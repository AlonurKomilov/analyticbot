...
class TestTelegramWebhookSimulation:
    ...
    async def test_webhook_rate_limiting(self, mock_bot, mock_redis):
        ...
        for webhook in webhooks:
            result = await self._process_telegram_webhook_with_rate_limit(webhook, mock_bot, mock_redis)
...
class TestPaymentWebhookSimulation:
    ...
    async def test_webhook_idempotency(self, mock_db_pool, mock_redis):
        ...
        result1 = await self._process_webhook_with_idempotency(webhook_data, mock_redis, mock_db_pool)
        result2 = await self._process_webhook_with_idempotency(webhook_data, mock_redis, mock_db_pool)
        result3 = await self._process_webhook_with_idempotency(webhook_data, mock_redis, mock_db_pool)
    ...
    async def _process_stripe_webhook(self, payload, headers, webhook_secret):
        ...
        try:
            ...
            if not timestamp or not signature:
                return {"valid_signature": False, "processed": False, "error": "Invalid signature format"}
    ...
    async def _process_webhook_with_idempotency(self, webhook_data, redis, db_pool):
        ...
        # Process webhook
        if db_pool:
            await db_pool.execute("INSERT INTO webhook_events (id, processed_at) VALUES ($1, NOW())", webhook_id)
