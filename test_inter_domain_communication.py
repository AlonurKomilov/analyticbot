...
async def test_event_bus_basic():
    ...
    try:
        ...
        user_event = UserRegisteredEvent(
            user_id="user_123", email="test@example.com", username="testuser"
        )

        payment_event = PaymentCompletedEvent(
            payment_id="pay_123", customer_id="user_123", amount="29.99", currency="USD"
        )
...
async def test_inter_domain_handlers():
    ...
    try:
        ...
        user_event = UserRegisteredEvent(
            user_id="user_456", email="newuser@example.com", username="newuser"
        )
...
async def test_event_publishers():
    ...
    try:
        ...
        await identity_publisher.publish_user_registered(
            user_id="user_789", email="publisher@example.com", username="publisher"
        )

        await analytics_publisher.publish_tracking_enabled(
            user_id="user_789", channel_id="channel_123"
        )

        await payments_publisher.publish_payment_completed(
            payment_id="pay_789", customer_id="user_789", amount="49.99", currency="USD"
        )

        await payments_publisher.publish_subscription_created(
            subscription_id="sub_789", customer_id="user_789", plan_name="premium"
        )
...
async def test_complete_workflow():
    ...
    try:
        ...
        await identity_publisher.publish_user_registered(
            user_id="workflow_user", email="workflow@example.com", username="workflow"
        )

        print("  → Simulating email verification...")
        await identity_publisher.publish_user_email_verified(
            user_id="workflow_user", email="workflow@example.com"
        )
