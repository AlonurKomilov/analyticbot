...
class AIChatService:
    ...
    async def process_user_question(
        ...
        try:
            ...
            relevant_data = await self._fetch_relevant_analytics(
                channel_id, intent, context
            )
            ...
            if intent["type"] == "performance_question":
                ...
                response = await self._explain_trends(
                    user_question, relevant_data, intent
                )
            elif intent["type"] == "timing_question":
                response = await self._answer_timing_question(
                    user_question, relevant_data, intent
                )
            else:
                response = await self._general_analytics_response(
                    user_question, relevant_data
                )
    ...
    def _parse_user_intent(self, question: str) -> dict:
        ...
        if any(word in question_lower for word in ["why", "drop", "decline", "down", "low", "poor", "falling"]):
        ...
        if any(word in question_lower for word in ["should", "recommend", "suggest", "improve", "optimize", "how to"]):
            return {"type": "recommendation_request", "focus": "optimization", "confidence": 0.9}
        
        # Trend inquiries (trend, growing, trending, pattern)
        if any(word in question_lower for word in ["trend", "growing", "trending", "pattern", "direction"]):
            return {"type": "trend_inquiry", "focus": "trends", "confidence": 0.8}
        
        # Best time questions (when, time, best time, schedule)
        if any(word in question_lower for word in ["when", "time", "best time", "schedule", "timing"]):
        ...
        if any(word in question_lower for word in ["who", "audience", "followers", "viewers", "demographic"]):
    ...
    async def _fetch_relevant_analytics(
        self, channel_id: int, intent: dict, context: Optional[dict]
    ) -> dict:
        """
        ...
        try:
            ...
            if hasattr(self._ai_insights_service, '_gather_ai_analysis_data'):
                ...
                data["overview"] = {
                    ...
                    "daily_metrics": {
                        "views": await self._daily.series_data(channel_id, "views", start_date, now),
                        "followers": await self._daily.series_data(channel_id, "followers", start_date, now)
                    }
                }
            ...
            if intent["type"] in ["performance_question", "trend_inquiry"]:
                ...
                data["historical"] = {
                    "posts": await self._posts.top_by_views(channel_id, extended_start, start_date, 50),
                    "daily_metrics": {
                        "views": await self._daily.series_data(channel_id, "views", extended_start, start_date)
                    }
                }
            ...
            if intent["type"] == "recommendation_request":
                ...
                if hasattr(self._ai_insights_service, '_identify_optimization_opportunities'):
                    data["optimization"] = await self._ai_insights_service._identify_optimization_opportunities(
    ...
    async def _answer_performance_question(
        self, question: str, data: dict, intent: dict
    ) -> str:
                ...
                if posts:
                    ...
                    if recent_views:
                        ...
                        if hist_posts:
                            ...
                            change = ((avg_views - hist_avg) / hist_avg * 100) if hist_avg > 0 else 0
    ...
    async def _provide_ai_recommendations(
        self, question: str, data: dict, intent: dict
    ) -> str:
        ...
        try:
            ...
            if optimization:
                ...
                if post_views:
                    top_performers = sorted(posts, key=lambda x: x.get("views", 0), reverse=True)[:3]
                    ...
                    if top_performers:
                        ...
                        recommendations = []
                        recommendations.append(f"Study your top-performing posts for patterns")
                        recommendations.append(f"Your successful content averages {avg_length:.0f} characters in titles")
                        recommendations.append("Maintain consistent posting schedule for best results")
            ...
            if posts:
                ...
                if len(post_views) >= 5:
                    ...
                    if trend_slope > 0:
                        ...
                        return f"Your content shows a declining trend with views decreasing over time. Consider refreshing your content approach, analyzing successful competitors, or adjusting your posting schedule."
                    else:
                        return f"Your content performance is relatively stable with no strong upward or downward trend. This consistency is good, but you might want to experiment with new formats to boost growth."
    ...
    async def _answer_timing_question(
        self, question: str, data: dict, intent: dict
    ) -> str:
    ...
    def _generate_follow_up_questions(self, intent: dict) -> list:
        ...
        return follow_ups.get(intent["type"], [
