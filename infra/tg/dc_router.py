...
class DCRouter:
    ...
    async def get_cached_dc(self, peer_id: str, request_type: str = "default") -> Optional[int]:
    ...
    async def cache_dc_mapping(self, peer_id: str, dc_id: int, request_type: str = "default", 
        ...
        logger.debug(f"Cached DC mapping: {peer_id} -> DC {dc_id} "
                    f"(type: {request_type}, success: {success})")
    
    def parse_migrate_error(self, error: Exception) -> Tuple[Optional[str], Optional[int]]:
    ...
    async def ensure_client_dc(self, client: Any, target_dc: int, client_id: str = "default") -> bool:
        ...
        try:
            ...
            if hasattr(client, 'connect_to_dc') or hasattr(client, 'switch_dc'):
                try:
                    ...
                    logger.warning(f"Failed to switch client {client_id} to DC {target_dc}: {switch_error}")
            ...
            if hasattr(client, 'disconnect') and hasattr(client, 'connect'):
                try:
                    ...
                    logger.error(f"Failed to reconnect client {client_id} to DC {target_dc}: {reconnect_error}")
            
            logger.warning(f"Could not switch client {client_id} to DC {target_dc} - will retry on next request")
            return False
    ...
    async def run_with_dc_retry(
        ...
        **kwargs
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if target_dc and attempt < self.max_retries - 1:
                    logger.info(f"Attempt {attempt + 1}: {error_type} migration to DC {target_dc} "
                               f"for peer {peer_id}")
        ...
        # All retries failed
        logger.error(f"Request failed after {self.max_retries} attempts for peer {peer_id}: {last_error}")
        raise last_error
    
    async def run_with_stats_dc(self, client: Any, request_callable: Callable, 
                               peer_id: str = "stats", client_id: str = "default", 
                               *args, **kwargs) -> Any:
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        valid_entries = sum(1 for entry in self._dc_cache.values() if entry.is_valid(self.cache_ttl))
    ...
    async def clear_cache(self, peer_id: Optional[str] = None) -> int:
...
def parse_stats_migrate(exc: Exception) -> Optional[int]:
...
_global_router: Optional[DCRouter] = None
