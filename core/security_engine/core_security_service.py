...
class CoreSecurityService(SecurityService):
    ...
    def create_refresh_token(self, user_id: str, session_id: str) -> str:
        ...
        expire = datetime.utcnow() + timedelta(
            days=self.config.get_refresh_token_expire_days()
        )
        ...
        refresh_token = self.token_generator.generate_jwt_token(
            claims, 
            timedelta(days=self.config.get_refresh_token_expire_days())
        )
    ...
    def create_session(
        ...
        expires_at = datetime.utcnow() + timedelta(
            hours=self.config.get_session_expire_hours()
        )
        ...
        try:
            ...
            session = SessionInfo(
                ...
                created_at=datetime.fromisoformat(session_data["created_at"]) 
                    if session_data.get("created_at") else None,
                last_activity=datetime.fromisoformat(session_data["last_activity"]) 
                    if session_data.get("last_activity") else None,
    ...
    def revoke_token(self, token: str) -> bool:
        ...
        try:
            ...
            if claims.expires_at:
                seconds_until_exp = int(
                    (claims.expires_at - datetime.utcnow()).total_seconds()
                )
    ...
    def _dict_to_token_claims(self, data: Dict[str, Any]) -> TokenClaims:
        ...
        return TokenClaims(
            ...
            issued_at=datetime.fromisoformat(data["issued_at"]) 
                if data.get("issued_at") else None,
