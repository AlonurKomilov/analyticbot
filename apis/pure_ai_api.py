...
class PureContentAnalyzer:
    ...
    def _generate_optimization_tips(
        ...
        if word_count < 20:
            tips.append(
                "📏 Add more content (aim for 20-100 words for optimal engagement)"
            )
        elif word_count > 150:
            tips.append(
                "✂️ Consider shortening content - shorter posts often get more engagement"
            )
        ...
        if emoji_count == 0:
            tips.append(
                "😊 Add 1-2 emojis to make content more engaging and visually appealing"
            )
        elif emoji_count > 5:
        ...
        if readability_score < 60:
            tips.append(
                "📚 Simplify sentences and use shorter words for better readability"
            )
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "pure_ai_api:app", host="0.0.0.0", port=8003, reload=False, log_level="info"
    )
