...
async def _get_asyncpg_pool():
    ...
    try:
        ...
        # ✅ UNIFIED DI: Use dependency-injector based container
class Container:
    """Legacy container compatibility layer - delegates to dependency-injector container"""
    
    def __init__(self):
        self._di_container = configure_bot_container()
        self.config = Singleton(Settings)
    
    async def asyncpg_pool(self):
        """Get asyncpg pool from new DI container"""
        try:
            pool = self._di_container.asyncpg_pool()
            return await pool
