...
def upgrade():
    ...
    expected_old_roles = {"guest", "user", "moderator", "admin", "super_admin", "support", "readonly", "analyst"}
    ...
    for old_role, new_role, description in migrations:
        if old_role in role_counts:
            ...
            if remaining == 0:
                ...
                raise Exception(f"❌ Migration failed! {remaining} users still have role '{old_role}'")
        else:
    ...
    print(f"✅ Old roles removed: guest, super_admin")
    print(f"✅ New 5-tier system: viewer < user < moderator < admin < owner")
    print("="*80 + "\n")
