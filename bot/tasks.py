...
from bot.celery_app import resilient_task
from bot.container import container
from bot.services.prometheus_service import prometheus_timer
...
def send_post_task(self, scheduler_id: int):  # type: ignore[override]
    async def _run():
        context = (
            ErrorContext()
            .add("task", "send_post_task")
            .add("scheduler_id", scheduler_id)
        )
...
def maintenance_cleanup(self):  # type: ignore[override]
    ...
    async def _run():
        ...
        try:
            ...
            requeued = await scheduler_repo.requeue_stuck_sending_posts(
                max_age_minutes=15
            )
            if requeued > 0:
