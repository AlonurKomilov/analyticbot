...
def mock_fusion_service():
    ...
    service = Mock()
    service.get_last_updated_at = AsyncMock(return_value=datetime(2025, 8, 31, 12, 0, 0, tzinfo=timezone.utc))
...
class TestAnalyticsV2ETag:
    ...
    async def test_etag_generated_correctly(self, client: AsyncClient, mock_fusion_service, mock_cache, monkeypatch):
        """Test that ETag is generated and included in response headers"""
        # Mock the dependencies
        monkeypatch.setattr("apps.api.di_analytics_v2.get_analytics_fusion_service", lambda: mock_fusion_service)
    ...
    async def test_304_response_with_matching_etag(self, client: AsyncClient, mock_fusion_service, mock_cache, monkeypatch):
    ...
    async def test_200_response_with_different_etag(self, client: AsyncClient, mock_fusion_service, mock_cache, monkeypatch):
    ...
    async def test_cache_control_header_present(self, client: AsyncClient, mock_fusion_service, mock_cache, monkeypatch):
    ...
    async def test_etag_changes_with_different_params(self, client: AsyncClient, mock_fusion_service, mock_cache, monkeypatch):
    ...
    async def test_etag_includes_last_updated_timestamp(self, client: AsyncClient, mock_cache, monkeypatch):
        """Test that ETag incorporates last_updated timestamp"""
        # Mock fusion service with different last_updated times
        service1 = Mock()
        service1.get_last_updated_at = AsyncMock(return_value=datetime(2025, 8, 31, 12, 0, 0, tzinfo=timezone.utc))
        service1.get_overview = AsyncMock(return_value={"channel_id": 123, "meta": {"cache_hit": False}})
        
        service2 = Mock()  
        service2.get_last_updated_at = AsyncMock(return_value=datetime(2025, 8, 31, 13, 0, 0, tzinfo=timezone.utc))
        service2.get_overview = AsyncMock(return_value={"channel_id": 123, "meta": {"cache_hit": False}})
        
        monkeypatch.setattr("apps.api.di_analytics_v2.get_cache", lambda: mock_cache)
        
        # First request
        monkeypatch.setattr("apps.api.di_analytics_v2.get_analytics_fusion_service", lambda: service1)
        response1 = await client.get(
            "/api/v2/analytics/channels/123/overview",
            params={"from": "2025-08-30T00:00:00Z", "to": "2025-08-31T23:59:59Z"}
        )
        ...
        monkeypatch.setattr("apps.api.di_analytics_v2.get_analytics_fusion_service", lambda: service2)
        response2 = await client.get(
            "/api/v2/analytics/channels/123/overview",
            params={"from": "2025-08-30T00:00:00Z", "to": "2025-08-31T23:59:59Z"}
        )
