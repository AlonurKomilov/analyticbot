...
from .roles import AdministrativeRole, ApplicationRole, has_role_or_higher
...
def require_permission(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            logger.debug(
                f"Permission granted: {current_user.get('username')} accessing {func.__name__}"
            )
            return await func(*args, **kwargs)
...
def require_role(min_role: str | ApplicationRole | AdministrativeRole, allow_equal: bool = True):
    """
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            logger.debug(
                f"Role access granted: {current_user.get('username')} accessing {func.__name__}"
            )
            return await func(*args, **kwargs)
...
def permission_check(
    user_dict: dict, required_permission: Permission | str, raise_on_fail: bool = True
) -> bool:
    """
...
def check_user_role(user_dict: dict, min_role: str | ApplicationRole | AdministrativeRole) -> bool:
    """
