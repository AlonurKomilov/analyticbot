...
class TelegramAnalyticsAdapter(AnalyticsAdapter):
    ...
    async def get_best_posting_times(self, channel_id: str) -> dict[str, Any]:
        """
        Analyze best posting times using Telegram API data
        Note: Limited by Telegram API capabilities, may need to use post history analysis
        """
        await self.rate_limiter.acquire()

        try:
            # This is a placeholder - real implementation would analyze post history
            # Telegram API doesn't provide direct "best posting times" analytics
            return {
                "channel_id": channel_id,
                "analysis_period": "limited",
                "message": "Telegram API has limited analytics capabilities",
                "recommendations": [
                    "Use MTProto for detailed message history analysis",
                    "Consider using post history to determine engagement patterns",
                ],
                "best_hours": [],
                "best_days": [],
                "metadata": {
