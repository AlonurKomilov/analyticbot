...
class CSVExporter:
    ...
    def overview_to_csv(self, data: OverviewResponse) -> io.StringIO:
        ...
        # Header
        writer.writerow([
            "metric", "value", "period_days", "channel_id", 
        ...
        base_data = [
            data.period, data.channel_id,
            "|".join(data.data_sources), data.last_updated.isoformat()
        ]
    ...
    def growth_to_csv(self, data: GrowthResponse) -> io.StringIO:
        ...
        # Header
        writer.writerow([
            "date", "subscribers", "change", "growth_rate",
            "period_days", "channel_id", "data_sources", "last_updated"
        ])
    ...
    def reach_to_csv(self, data: ReachResponse) -> io.StringIO:
        ...
        writer.writerow([
            "hour", "views", "total_views", "unique_viewers",
            "view_reach_ratio", "peak_concurrent", "period_days",
            "channel_id", "data_sources", "last_updated"
        ])
        
        reach = data.reach
        base_data = [
            reach.total_views, reach.unique_viewers,
            reach.view_reach_ratio, reach.peak_concurrent,
            data.period, data.channel_id,
            "|".join(data.data_sources), data.last_updated.isoformat()
        ]
    ...
    def top_posts_to_csv(self, data: TopPostsResponse) -> io.StringIO:
        ...
        # Header
        writer.writerow([
            "rank", "post_id", "message_preview", "views", "forwards",
            "reactions", "engagement_score", "published_at",
            "period_days", "channel_id", "data_sources", "last_updated"
        ])
        ...
        for rank, post in enumerate(top_posts, 1):
            ...
            writer.writerow([
                rank, post.post_id, message_preview,
                post.views, post.forwards, post.reactions,
                post.engagement_score, post.published_at.isoformat()
            ] + base_data)
    ...
    def sources_to_csv(self, data: SourcesResponse) -> io.StringIO:
        ...
        # Header
        writer.writerow([
            "source_type", "views", "percentage", "channel_id",
            "period_days", "data_sources", "last_updated"
        ])
        
        sources = data.sources
        base_data = [
            data.channel_id, data.period,
            "|".join(data.data_sources), data.last_updated.isoformat()
        ]
    ...
    def trending_to_csv(self, data: TrendingResponse) -> io.StringIO:
        ...
        # Header
        writer.writerow([
            "metric", "value", "is_trending", "trend_direction",
            "confidence", "period_days", "channel_id",
            "data_sources", "last_updated"
        ])
        
        trending = data.trending
        base_data = [
            trending.is_trending, trending.trend_direction,
            trending.confidence, data.period, data.channel_id,
            "|".join(data.data_sources), data.last_updated.isoformat()
        ]
