...
async def test_metrics_endpoint():
    ...
    try:
        ...
        if hasattr(response, 'body'):
            ...
            content = response.content.decode() if hasattr(response.content, 'decode') else str(response.content)
        else:
        ...
        if found_patterns < 2:
            print(f"❌ Content doesn't look like Prometheus metrics (found {found_patterns}/4 patterns)")
...
async def test_alembic_upgrade_simulation():
    ...
    try:
        ...
        migration_path = "/workspaces/analyticbot/infra/db/alembic/versions/0006_deliveries_observability.py"
