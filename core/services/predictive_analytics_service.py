...
class PredictiveAnalyticsService:
    ...
    async def generate_predictive_analytics(
        ...
        try:
            ...
            if prediction_type in ["engagement", "comprehensive"]:
                ...
                predictions["confidence_score"] += predictions["engagement_predictions"].get("confidence", 0.0) * 0.4
            ...
            if prediction_type in ["growth", "comprehensive"]:
                ...
                predictions["confidence_score"] += predictions["growth_predictions"].get("confidence", 0.0) * 0.3
            
            # Optimization predictions
            if prediction_type in ["optimization", "comprehensive"]:
                predictions["optimization_predictions"] = await self._predict_optimization_opportunities(
                    training_data, use_ml_models
                )
                predictions["confidence_score"] += predictions["optimization_predictions"].get("confidence", 0.0) * 0.3
            ...
            predictions["recommendations"] = await self._generate_predictive_recommendations(predictions)
            
            # Normalize confidence score
            predictions["confidence_score"] = min(1.0, max(0.0, predictions["confidence_score"]))
            
            logger.info(f"Predictive analytics generated for channel {channel_id} with confidence {predictions['confidence_score']:.2f}")
            return predictions
    ...
    async def _prepare_ml_training_data(self, channel_id: int, days: int = 90) -> dict:
        ...
        try:
            ...
            # Get follower data
            follower_series = await self._daily.series_data(channel_id, "followers", start_date, now)
            if not follower_series:
                follower_series = await self._daily.series_data(channel_id, "subscribers", start_date, now)
    ...
    async def _get_daily_views(self, channel_id: int, start_date: datetime, end_date: datetime) -> list:
    ...
    async def _get_daily_engagement(self, channel_id: int, start_date: datetime, end_date: datetime) -> list:
        """Get daily engagement time series"""
        try:
            daily_data = await self._daily.series_data(channel_id, "engagement", start_date, end_date)
            return [item.get("value", 0) for item in daily_data]
        except Exception as e:
            logger.error(f"Daily engagement retrieval failed: {e}")
            return []

    async def _get_daily_growth(self, channel_id: int, start_date: datetime, end_date: datetime) -> list:
    ...
    def _extract_post_features(self, post: dict, index: int, all_posts: list) -> dict:
        ...
        try:
            ...
            avg_recent_views = np.mean([p.get("views", 0) for p in recent_posts]) if recent_posts else views
    ...
    async def _predict_engagement_patterns(self, training_data: dict, forecast_horizon: int, use_ml: bool) -> dict:
        ...
        try:
            ...
            if posts:
                ...
                # Find best hour
                hour_avg = {hour: np.mean(engagements) for hour, engagements in posting_times.items()}
    ...
    async def _predict_growth_trajectory(self, training_data: dict, forecast_horizon: int, use_ml: bool) -> dict:
    ...
    async def _generate_predictive_recommendations(self, predictions: dict) -> list:
        ...
        try:
            ...
            return [{"category": "error", "recommendation": "Failed to generate recommendations", "priority": "low"}]