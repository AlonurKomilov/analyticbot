...
def mock_payme_workflow():
    ...
    client.check_perform_transaction.return_value = {"result": {"allow": True}}
...
class TestSubscriptionPurchaseWorkflow:
    ...
    async def test_complete_stripe_subscription_purchase(
        ...
        confirmed_payment = await mock_stripe_workflow.confirm_payment_intent(payment_intent["id"])
    ...
    async def test_payme_subscription_purchase_workflow(
        ...
        check_result = await mock_payme_workflow.check_perform_transaction(
            amount=plan_data["data"]["amount"], account={"user_id": str(user_id)}
        )
        ...
        await mock_payme_workflow.perform_transaction(id=transaction_id)
...
class TestPaymentFailureWorkflow:
    ...
    async def test_card_declined_recovery_workflow(
        ...
        await mock_stripe_workflow.create_payment_intent(amount=2999, currency="usd")
    ...
    async def test_multiple_payment_failures_workflow(
        self, mock_stripe_workflow, mock_payment_bot, mock_payment_redis
    ):
        ...
        update_payment_workflow_state(
            workflow_id, {"stage": "max_retries_exceeded", "escalation_required": True}
        )
                ...
                return_value={
                    ...
                    "data": {
                        ...
                        "next_renewal": datetime.fromtimestamp(
                            renewed_subscription["current_period_end"]
                        ).isoformat(),
                    },
                }
            ),
        ...
        await mock_payment_bot.send_message(
            ...
            reply_markup={
                "inline_keyboard": [
                    [
                        {
                            "text": "📋 View Refund Details",
                            "callback_data": f"refund_details_{refund_result['id']}",
                        }
                    ],
