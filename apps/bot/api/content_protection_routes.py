...
async def add_image_watermark(
    ...
    try:
        ...
        return ContentProtectionResponse(
            ...
            protected=True,
            protection_level=ProtectionLevel.PREMIUM if user_tier != UserTier.FREE else ProtectionLevel.BASIC,
...
async def add_video_watermark(
    ...
    if user_tier == UserTier.FREE:
        raise HTTPException(
            status_code=403, 
            detail="Video watermarking requires premium subscription"
        )
...
async def format_custom_emoji_message(
    ...
    if user_tier == UserTier.FREE:
        raise HTTPException(
            status_code=403,
            detail="Custom emoji features require premium subscription"
        )
...
async def get_feature_usage(
    ...
    return {
        ...
        "remaining": {
            "watermarks": (limits.watermarks_per_month - usage["watermarks"]) if limits.watermarks_per_month else "unlimited",
            "custom_emojis": (limits.custom_emojis_per_month - usage["custom_emojis"]) if limits.custom_emojis_per_month else "unlimited",
            "theft_scans": (limits.theft_scans_per_month - usage["theft_scans"]) if limits.theft_scans_per_month else "unlimited"
        }
    }
