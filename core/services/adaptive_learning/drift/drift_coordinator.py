...
from .statistical_analyzer import StatisticalDriftAnalyzer, FeatureAnalysisResult
...
class DriftCoordinatorConfig:
    ...
    drift_severity_thresholds: Optional[Dict[str, float]] = None
...
class DriftCoordinator:
    ...
    def __init__(
        ...
        config: Optional[DriftCoordinatorConfig] = None
    ):
        ...
        try:
            ...
            if reference_data is not None:
                if reference_data.shape[0] < self.config.min_samples_required:
                    logger.warning(f"⚠️ Insufficient reference data: {reference_data.shape[0]} < {self.config.min_samples_required}")
                    return False
    ...
    async def detect_data_drift(
    ...
    ) -> Optional[ComprehensiveDriftAnalysis]:
        ...
        try:
            ...
            if current_data.shape[1] != reference_data.shape[1]:
                logger.error(f"❌ Feature dimension mismatch: {current_data.shape[1]} vs {reference_data.shape[1]}")
                return None
            ...
            if cache_key in self.analysis_cache:
                ...
                if (datetime.utcnow() - cached.timestamp).total_seconds() < self.config.cache_ttl_minutes * 60:
                    return cached
            ...
            logger.info(f"🔍 Data drift analysis completed: {'DETECTED' if analysis.overall_detected else 'NO DRIFT'}")
            return analysis
    ...
    async def detect_concept_drift(
        ...
        try:
            ...
            logger.info(f"🔍 Concept drift analysis completed: {'DETECTED' if analysis.overall_detected else 'NO DRIFT'}")
            return analysis
    ...
    async def get_drift_status(self, model_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            active_alerts = [a for a in self.active_alerts.values() if a.model_id == model_id and not a.resolved]
            
            # Calculate trends
            data_drift_scores = [a.overall_drift_score for a in recent_analyses if a.drift_type == DriftType.DATA_DRIFT]
            concept_drift_scores = [a.overall_drift_score for a in recent_analyses if a.drift_type == DriftType.CONCEPT_DRIFT]
            ...
            return {
                ...
                'drift_trends': {
                    'data_drift_avg': sum(data_drift_scores) / len(data_drift_scores) if data_drift_scores else 0.0,
                    'concept_drift_avg': sum(concept_drift_scores) / len(concept_drift_scores) if concept_drift_scores else 0.0
                },
    ...
    async def _aggregate_drift_results(
        ...
        multivariate_confidence = sum(m.confidence for m in multivariate_results.values()) / len(multivariate_results) if multivariate_results else 0.5
    ...
    async def _analyze_performance_drift(
        ...
        for metric, current_value in current.items():
            if metric in baseline:
                ...
                change_ratio = (current_value - baseline_value) / baseline_value if baseline_value != 0 else 0.0
            ...
            try:
                ...
                for model_id in list(self.monitored_models):
                    try:
                        ...
                        current_metrics = await self.monitoring_service.get_current_metrics(model_id)
                        if current_metrics:
                            performance_dict = {
