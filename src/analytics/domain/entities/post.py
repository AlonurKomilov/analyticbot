...
from ..value_objects.analytics_value_objects import (
    PostId, ChannelId, MessageId, ViewCount, PostContent
)
from ..events import PostScheduled, PostPublished, ViewsUpdated
...
class Post(AggregateRoot[PostId]):
    ...
    def __init__(
        ...
        views: Optional[ViewCount] = None,
        last_view_update: Optional[datetime] = None,
        view_history: Optional[List[Dict[str, Any]]] = None,
        peak_views_per_hour: Optional[int] = None,
    ...
    def schedule_post(self, scheduled_time: datetime) -> None:
        ...
        # Emit domain event
        self.add_domain_event(PostScheduled(
    ...
    def mark_as_published(self, message_id: MessageId) -> None:
        ...
        # Emit domain event
        self.add_domain_event(PostPublished(
            post_id=self.id.value,
            channel_id=self.channel_id.value,
            user_id=self.user_id.value,
    ...
    def update_views(self, new_view_count: ViewCount) -> None:
        ...
        if view_increase > 0:
            ...
            if self.published_at:
                hours_since_publish = (self.last_view_update - self.published_at).total_seconds() / 3600
            ...
            # Emit domain event
            self.add_domain_event(ViewsUpdated(
                post_id=self.id.value,
                channel_id=self.channel_id.value,
                user_id=self.user_id.value,
                old_views=old_views,
    ...
    def get_analytics_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_view_update": self.last_view_update.isoformat() if self.last_view_update else None
        }
