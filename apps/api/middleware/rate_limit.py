    ...
    async def _cleanup_old_buckets(self):
        ...
        while True:
            try:
                ...
                expired_creation = [
                    ip for ip, bucket in self.creation_buckets.items()
                ...
                expired_access = [
                    ip for ip, bucket in self.access_buckets.items()
                    if bucket.last_refill < cutoff_time
                ]
                ...
                if expired_creation or expired_access:
                    logger.info(f"Cleaned up rate limit buckets: {len(expired_creation)} creation, {len(expired_access)} access")
...
_rate_limiter: Optional[ShareLinkRateLimiter] = None
...
def check_creation_rate_limit(request: Request):
    ...
    if not rate_limiter.check_creation_limit(request):
        ...
        logger.warning(f"Rate limit exceeded for share creation from IP {rate_limiter.get_client_ip(request)}")
...
def check_access_rate_limit(request: Request):
    ...
    if not rate_limiter.check_access_limit(request):
        ...
        logger.warning(f"Rate limit exceeded for share access from IP {rate_limiter.get_client_ip(request)}")
