...
class TrendAnalysisService:
    ...
    async def _analyze_trend_components(self, time_series: np.ndarray) -> dict:
        ...
        try:
            ...
            poly_coeffs = np.polyfit(x, time_series, 2) if len(time_series) > 10 else [0, slope, intercept]
            ...
            # Volatility analysis
            volatility = np.std(time_series) / np.mean(time_series) if np.mean(time_series) > 0 else 0
    ...
    async def _detect_change_points(self, time_series: np.ndarray) -> dict:
        ...
        try:
            ...
            return {
                ...
                "most_significant": max(change_points, key=lambda x: x["change_magnitude"]) if change_points else None
            }
    ...
    async def _generate_trend_forecasts(self, time_series: np.ndarray, forecast_days: int = 7) -> dict:
        ...
        try:
            ...
            recent_std = np.std(time_series[-14:]) if len(time_series) >= 14 else np.std(time_series)
    ...
    async def _get_daily_growth(self, channel_id: int, start_date: datetime, end_date: datetime) -> list:
        ...
        try:
            followers_data = await self._daily.series_data(channel_id, "followers", start_date, end_date)
            
            if not followers_data:
                # Try subscribers metric as fallback
                followers_data = await self._daily.series_data(channel_id, "subscribers", start_date, end_date)
        ...
        try:
            ...
            while current_date <= end_date:
                ...
                if current_subs is None:
                    current_subs = await self._daily.series_value(channel_id, "subscribers", current_date)
