...
class SuperAdminService:
    ...
    async def get_audit_logs(
        self, admin_id: Optional[UUID] = None, page: int = 1, limit: int = 50
    ) -> Dict[str, Any]:
    ...
    async def create_audit_log(
        self, admin_id: UUID, action: str, resource_type: str, resource_id: str, details: Dict[str, Any]
    ) -> None:
        """Create audit log entry via repository"""
        await self.admin_repo.create_audit_log(admin_id, action, resource_type, resource_id, details)
    ...
    async def get_admin_by_id(self, admin_id: UUID) -> Optional[Dict[str, Any]]:
