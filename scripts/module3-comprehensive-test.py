...
class Module3TestSuite:
        ...
        try:
            ...
            for dashboard in dashboards:
                ...
                if not dashboard_path.exists():
                    self.log_test(f"Grafana Dashboard - {dashboard}", False, 
                ...
                try:
                    ...
                    self.log_test(f"Grafana Dashboard - {dashboard}", True,
    ...
    def test_prometheus_alerts(self) -> bool:
        ...
        try:
            ...
            if rule_validity >= 90:
                self.log_test("Prometheus Alerts", True,
                            f"{valid_rules}/{total_rules} rules valid ({rule_validity:.1f}%)",
                            duration)
                return True
            else:
                self.log_test("Prometheus Alerts", False,
    ...
    def test_cicd_workflows(self) -> bool:
        ...
        try:
            ...
            if missing_keys:
                self.log_test("CI/CD Workflows", False,
                            f"Missing workflow keys: {missing_keys}")
                return False
            ...
            if job_validity >= 80:
                self.log_test("CI/CD Workflows", True,
        ...
        try:
            ...
            try:
                ...
                if result.returncode != 0:
                    self.log_test("Backup System", False,
            ...
            try:
                result = subprocess.run(
                    ...
                    env={**os.environ, 'DB_HOST': 'nonexistent', 'BACKUP_ROOT': str(tempfile.gettempdir())}
                )
            ...
            duration = time.time() - start_time
            self.log_test("Backup System", True,
    ...
    def test_monitoring_integration(self) -> bool:
        ...
        try:
            ...
            helm_monitoring = self.project_root / "infrastructure" / "helm" / "templates" / "monitoring.yaml"
            if helm_monitoring.exists():
                score += 1
                total_components += 1
                print(f"      ✅ helm_monitoring: Found")
            else:
                print(f"      ❌ helm_monitoring: Missing")
                total_components += 1
    ...
    def test_kubernetes_resources(self) -> bool:
        ...
        try:
                ...
                for file_path in template_files:
                    try:
                        with open(file_path) as f:
                            ...
                            if 'resources:' in content and ('requests:' in content or 'limits:' in content):
                                monitoring_found = True
            ...
            if total_score >= 50:
                self.log_test("Kubernetes Resources", True,
                            f"Resource optimization score: {total_score:.1f}%",
                            duration)
                return True
            else:
                self.log_test("Kubernetes Resources", False,
        ...
        try:
            ...
            if helm_dir.exists():
                ...
                for values_file in values_files:
                    try:
                        with open(values_file) as f:
                            content = f.read().lower()
                            if 'prometheus' in content or 'grafana' in content or 'monitoring' in content:
                ...
                if monitoring_in_helm:
                    integration_score += 1
                    print(f"      ✅ Module 1 Integration: Monitoring integrated into Helm")
                else:
                    print(f"      ❌ Module 1 Integration: No monitoring in Helm values")
            
            # Check Module 2 (Testing) integration  
            module2_script = self.project_root / "scripts" / "module2_local_test.py"
            if module2_script.exists():
                integration_score += 1
                print(f"      ✅ Module 2 Integration: Test framework available")
            else:
                print(f"      ❌ Module 2 Integration: Test framework missing")
            
            # Check CI/CD integration with existing workflows
            existing_workflows = list(self.workflows_dir.glob("*.yml")) if self.workflows_dir.exists() else []
            ...
            if self.backup_script.exists():
                try:
                    with open(self.backup_script) as f:
                        ...
                        if 'infrastructure/helm' in backup_content and 'infrastructure/k8s' in backup_content:
                            integration_score += 1
                            print(f"      ✅ Backup Integration: Infrastructure backup included")
                        else:
                            print(f"      ❌ Backup Integration: Limited infrastructure backup")
                except Exception:
                    print(f"      ❌ Backup Integration: Could not validate")
            
            integration_percentage = integration_score / total_checks * 100
            duration = time.time() - start_time
            
            if integration_percentage >= 75:
                self.log_test("Module Integration", True,
                            f"Integration score: {integration_percentage:.1f}%",
                            duration)
                return True
            else:
                self.log_test("Module Integration", False,
    ...
    async def run_all_tests(self):
        ...
        for test_method in test_methods:
            try:
                ...
                self.log_test(f"Test Method {test_method.__name__}", False, 
        ...
        self.results['success_rate'] = (self.results['passed_tests'] / 
        ...
        results_file = results_dir / f"module3_test_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
