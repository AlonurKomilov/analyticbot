...
class ContentOptimizer:
    ...
    def __init__(self, cache_service=None, analytics_service=None):
        ...
        self.positive_words = {
            ...
            'great', 'incredible', 'outstanding', 'perfect', 'wonderful',
            'best', 'top', 'super', 'love', 'beautiful', 'stunning',
            'impressive', 'remarkable', 'exceptional', 'magnificent'
        }
        
        self.negative_words = {
            'awful', 'terrible', 'horrible', 'disgusting', 'worst',
            'bad', 'poor', 'disappointing', 'annoying', 'frustrating',
            'boring', 'stupid', 'hate', 'ugly', 'pathetic', 'useless',
            'failed', 'wrong', 'broken', 'disaster'
        }
    ...
    async def optimize_hashtags(
        ...
        try:
            ...
            suggestions.sort(
                key=lambda x: x.relevance_score * x.popularity_score, 
                reverse=True
            )
    ...
    async def score_content_realtime(self, text: str) -> Dict[str, float]:
        ...
        try:
            ...
            positive_count = sum(1 for word in text.lower().split() 
                                if word in self.positive_words)
            negative_count = sum(1 for word in text.lower().split()
    ...
    async def _generate_optimization_tips(
        ...
        if hashtag_count == 0:
            ...
            tips.append(f"⚠️ Reduce hashtags to 5-7 for better performance (currently {hashtag_count})")
