...
class StorageContainer(containers.DeclarativeContainer):
    ...
    channel_daily_repo = providers.Factory(
        lambda repos: repos.channel_daily,
        repos=repositories
    )
    
    channel_repo = providers.Factory(
        lambda repos: repos.channel,
        repos=repositories
    )
    
    post_metrics_repo = providers.Factory(
        lambda repos: repos.post_metrics, 
        repos=repositories
    )
    
    post_repo = providers.Factory(
        lambda repos: repos.post,
        repos=repositories  
    )
    
    stats_raw_repo = providers.Factory(
        lambda repos: repos.stats_raw,
        repos=repositories
    )