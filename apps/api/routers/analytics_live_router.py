...
async def get_realtime_metrics(
    ...
    **Note**: No caching - always returns fresh real-time data
    ...
    try:
        ...
        overview_data = await analytics_client.overview(str(channel_id), 1)  # 1 day minimum
        growth_data = await analytics_client.growth(str(channel_id), 1)
        reach_data = await analytics_client.reach(str(channel_id), 1)
...
async def get_performance_score(
    ...
    try:
        ...
        period_days = max(1, period)  # Ensure minimum 1 day

        overview_data = await analytics_client.overview(str(channel_id), period_days)
        growth_data = await analytics_client.growth(str(channel_id), period_days)
        reach_data = await analytics_client.reach(str(channel_id), period_days)
...
async def get_live_monitoring_data(
    ...
    """
    try:
        # Get very recent data (minimum 1 day)
        recent_overview = await analytics_client.overview(str(channel_id), 1)  # 1 day
        recent_growth = await analytics_client.growth(str(channel_id), 1)
        recent_reach = await analytics_client.reach(str(channel_id), 1)
...
async def get_live_metrics_optimized(
    ...
    """
    try:
        # Use comprehensive analysis with appropriate time range
        analysis_result = await analytics_service.coordinate_comprehensive_analysis(
        ...
        # Extract live metrics from comprehensive analysis
        live_metrics = {
            "timestamp": datetime.utcnow().isoformat(),
            "channel_id": channel_id,
