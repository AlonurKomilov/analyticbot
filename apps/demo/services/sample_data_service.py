...
class SampleDataService:
    ...
    async def generate_post_dynamics_data(channel_id: int, days: int = 30) -> Dict[str, Any]:
        ...
        for i in range(days):
            ...
            engagement_rate = ((daily_reactions + daily_shares + daily_comments) / daily_views * 100) if daily_views > 0 else 0
        ...
        avg_engagement = sum(d["engagement_rate"] for d in dynamics) / len(dynamics) if dynamics else 0
        ...
        return {
            ...
            "summary": {
                ...
                "trend": "increasing" if trend_factor > 1 else "stable" if trend_factor == 1 else "decreasing",
    ...
    @staticmethod
    async def generate_top_posts_data(channel_id: int, limit: int = 15, period: str = "week") -> List[Dict[str, Any]]:
        ...
        for i in range(limit):
            ...
            post = {
                ...
                "published_at": (datetime.now() - timedelta(days=i+1, hours=random.randint(0, 23))).isoformat(),
                "metrics": {
                    "views": max(100, views),
                ...
                "tags": random.sample(["analytics", "development", "ai", "automation", "insights"], 3)
            }
            posts.append(post)
    ...
    async def generate_best_times_data(channel_id: int, timezone: str = "UTC") -> Dict[str, Any]:
        ...
        for day, base_score in zip(days, base_scores):
    ...
    async def generate_ai_recommendations(channel_id: int, category: str = "all") -> Dict[str, Any]:
        ...
        # Generate insights
        insights = {
            "channel_performance": random.choice(["excellent", "above_average", "good", "needs_improvement"]),
            "growth_trend": random.choice(["rapid_growth", "steady_increase", "stable", "slight_decline"]),
    ...
    async def get_initial_data(user_id: int | str, demo_type: str = "standard") -> Dict[str, Any]:
        ...
        if demo_type == "showcase":
            base_data["user"]["features_available"] = [
                "Full Analytics Dashboard", "AI Recommendations", 
                "Content Optimization", "Advanced Reports"
            ]
            # Generate rich sample channels
            base_data["channels"] = [
                {
                    "id": 1, "name": "Tech Insights", "subscribers": 15000,
                    "growth_rate": 12.5, "engagement": 8.9
                },
                {
                    "id": 2, "name": "AI Updates", "subscribers": 8500,
                    "growth_rate": 18.2, "engagement": 9.4
                },
                {
                    "id": 3, "name": "Developer Tips", "subscribers": 12000,
                    "growth_rate": 6.8, "engagement": 7.8
                }
            ]
        elif demo_type == "limited":
            base_data["user"]["features_available"] = [
                "Basic Analytics", "Sample Reports"
            ]
            # Generate basic sample data
            base_data["channels"] = [
                {
                    "id": 1, "name": "Sample Channel", "subscribers": 5000,
                    "growth_rate": 5.0, "engagement": 6.5
                }
            ]
        else:  # standard
            base_data["user"]["features_available"] = [
                "Analytics Dashboard", "Basic AI Insights", "Performance Reports"
            ]
            base_data["channels"] = [
                {
                    "id": 1, "name": "Demo Channel", "subscribers": 10000,
                    "growth_rate": 8.5, "engagement": 7.5
                },
                {
                    "id": 2, "name": "Sample Content", "subscribers": 6500,
                    "growth_rate": 10.2, "engagement": 8.1
                }
            ]
        ...
        base_data["recent_analytics"] = {
            ...
            "avg_engagement": sum(c["engagement"] for c in base_data["channels"]) / len(base_data["channels"]) if base_data["channels"] else 0,
            "growth_trend": "positive",
            "top_performing_channel": base_data["channels"][0]["name"] if base_data["channels"] else "N/A"
        }
