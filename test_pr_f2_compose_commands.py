...
def test_compose_explicit_commands():
    ...
    if worker_command != expected_worker_command:
        print(f"❌ Worker command mismatch. Expected: {expected_worker_command}, Got: {worker_command}")
        return False
...
def main():
    ...
    print("\n" + "=" * 60)
    print(f"📊 PR-F2 TEST SUMMARY")
    print("=" * 60)
    print(f"Explicit commands in docker-compose.yml... {'✅ PASSED' if tests_passed >= 1 else '❌ FAILED'}")
    print(f"Module imports without errors.......... {'✅ PASSED' if tests_passed >= 2 else '❌ FAILED'}")
    print("-" * 60)
