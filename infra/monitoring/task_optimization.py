    ...
    def decorator(func):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            should_process = (
                ...
                current_time - last_execution >= max_wait_time
            )
            ...
            if should_process:
                ...
                logger.info(f"📦 Processing batch of {len(batch_to_process)} items for {func.__name__}")
    ...
    def get_performance_report() -> Dict:
        ...
        if metrics:
            ...
            report['summary'] = {
                ...
                'most_used_task': max(metrics.keys(), key=lambda k: metrics[k]['total_executions']) if metrics else None,
                'slowest_task': max(metrics.keys(), key=lambda k: metrics[k]['average_duration']) if metrics else None
            }
