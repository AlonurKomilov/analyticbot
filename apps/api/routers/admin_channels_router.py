...
async def get_all_channels(
    ...
    channel_service: ChannelManagementService = Depends(
        lambda: container.channel_management_service()
    ),
):
    """
    ...
    **Returns:**
    ...
    try:
        ...
        with performance_timer("admin_all_channels_fetch"):
            channels = await channel_service.get_all_channels_admin(limit=limit, offset=offset)
    ...
    try:
        ...
        with performance_timer("admin_channel_deletion"):
            result = await channel_service.admin_delete_channel(
                channel_id=channel_id, admin_user_id=current_user["id"]
            )
