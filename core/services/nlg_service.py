...
class NaturalLanguageGenerationService:
    ...
    def _extract_key_metrics(self, analytics_data: Dict, insight_type: InsightType) -> Dict[str, Any]:
    ...
    async def _generate_narrative(
        ...
        try:
            ...
            template = self._narrative_templates.get(
                insight_type.value, {}
            ).get(
                style.value,
                "Analytics data shows {metric} patterns in your channel."
            )
    ...
    def _generate_title(self, insight_type: InsightType, key_metrics: Dict, style: NarrativeStyle) -> str:
        ...
        try:
            ...
            return f"📊 Analytics Insights"
    
    def _calculate_narrative_confidence(self, analytics_data: Dict, insight_type: InsightType) -> float:
    ...
    def _generate_fallback_narrative(self, insight_type: InsightType, analytics_data: Dict) -> InsightNarrative:
    ...
    def _extract_executive_kpis(self, analytics: Dict) -> Dict:
        ...
        if performance == "needs_improvement" or (trend == "declining" and kpis.get("trend_strength", 0) > 0.7):
    ...
    async def _generate_engagement_narrative(self, engagement_data: Dict, style: NarrativeStyle) -> str:
    ...
    async def _generate_predictions_narrative(self, predictions_data: Dict, style: NarrativeStyle) -> str:
        """Generate narrative for predictions section"""
        confidence = predictions_data.get("confidence_score", 0.5)
        
        if confidence > 0.8:
            return "Predictive models show high confidence in forecasted trends and recommendations."
        elif confidence > 0.6:
    ...
    def _generate_conversational_anomaly_explanation(
        self, anomaly_type: str, metric: str, value: float, baseline: float, change_percent: float, severity: str
    ) -> str:
    ...
    def _generate_positive_trend_story(self, metric: str, strength: float, duration: int, style: NarrativeStyle) -> str:
    ...
    def _generate_negative_trend_story(self, metric: str, strength: float, duration: int, style: NarrativeStyle) -> str:
    ...
    def _generate_stable_trend_story(self, metric: str, duration: int, style: NarrativeStyle) -> str:
