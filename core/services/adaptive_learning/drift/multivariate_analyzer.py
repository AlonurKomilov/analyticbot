...
class MultivariateDriftAnalyzer:
    ...
    async def energy_distance(
        ...
        try:
            ...
            confidence = min(0.95, energy_dist / threshold) if detected else max(0.1, 1.0 - energy_dist)
    ...
    async def wasserstein_distance(
        ...
        try:
            ...
            confidence = min(0.95, avg_wasserstein / threshold) if detected else max(0.1, 1.0 - avg_wasserstein)
    ...
    async def hotelling_t2_test(
        ...
        try:
            ...
            confidence = min(0.95, f_stat / threshold) if detected else max(0.1, threshold / max(f_stat, 1.0))
    ...
    async def pca_based_drift_detection(
        ...
        try:
            ...
            confidence = min(0.95, projected_drift / threshold) if detected else max(0.1, 1.0 - projected_drift)
    ...
    async def _calculate_mmd_kernel(
        self,
        X: np.ndarray,
        Y: np.ndarray,
        gamma: float
    ...
    async def _calculate_energy_distance(
        self,
        X: np.ndarray,
        Y: np.ndarray
    ) -> float:
    ...
    def _sample_data(self, data: np.ndarray, max_size: Optional[int] = None) -> np.ndarray:
