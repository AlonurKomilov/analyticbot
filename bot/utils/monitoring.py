...
class MetricsCollector:
    ...
    def __init__(self, max_metrics_age_hours: int = 24):
        ...
        self._performance_stats: dict[str, PerformanceStats] = defaultdict(
            PerformanceStats
        )
    ...
    def get_performance_stats(
        ...
        with self._lock:
            if endpoint:
                return {
                    endpoint: self._performance_stats.get(endpoint, PerformanceStats())
                }
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            total_requests = sum(
                stats.total_requests for stats in self._performance_stats.values()
            )
            total_errors = sum(
                stats.failed_requests for stats in self._performance_stats.values()
            )
