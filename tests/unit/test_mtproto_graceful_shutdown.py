...
class TestUpdatesPollerGracefulShutdown:
    ...
    def mock_collector(self):
        ...
        collector.get_stats = Mock(return_value={
    ...
    async def test_graceful_shutdown_within_timeout(self, mock_settings, mock_tg_client, mock_collector):
        ...
        with patch('apps.mtproto.tasks.poll_updates.MTProtoSettings', return_value=mock_settings), \
    ...
    async def test_shutdown_stops_ongoing_collection(self, mock_settings, mock_tg_client, mock_collector):
    ...
    async def test_signal_handler_triggers_shutdown(self, mock_settings, mock_tg_client, mock_collector):
    ...
    async def test_shutdown_with_feature_flags_disabled(self):
        ...
        with patch('apps.mtproto.tasks.poll_updates.MTProtoSettings', return_value=settings):
            ...
            assert shutdown_time < 0.1, f"Disabled polling should return quickly, took {shutdown_time:.2f}s"
    ...
    async def test_shutdown_during_error_recovery(self, mock_settings, mock_tg_client, mock_collector):
    ...
    async def test_multiple_shutdown_calls_are_safe(self, mock_settings, mock_tg_client, mock_collector):
        ...
        with patch('apps.mtproto.tasks.poll_updates.MTProtoSettings', return_value=mock_settings), \
            ...
            assert shutdown_duration < 2.0, "Multiple shutdown calls should not increase shutdown time"
