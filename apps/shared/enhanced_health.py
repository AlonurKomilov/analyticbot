...
class EnhancedHealthChecker:
    ...
    async def get_comprehensive_health(self) -> SystemHealth:
        ...
        results = await asyncio.gather(
            *[check for _, check in component_checks], return_exceptions=True
        )
        total_check_time = (time.time() - start_time) * 1000

        # Process results
        for (name, _), result in zip(component_checks, results, strict=False):
            if isinstance(result, Exception):
                ...
                if (
                    ...
                    and result.response_time_ms > self.thresholds["response_time_critical_ms"]
                ):
                    alerts.append(f"{name} response time critical: {result.response_time_ms:.2f}ms")
                elif (
                    result.response_time_ms
                    and result.response_time_ms > self.thresholds["response_time_warning_ms"]
                ):
    ...
    def _calculate_overall_status(self, components: dict[str, ComponentHealth]) -> HealthStatus:
        ...
        critical_unhealthy = any(
            components.get(
                name,
                ComponentHealth(name=name, status=HealthStatus.UNKNOWN, last_check=datetime.now()),
            ).status
    ...
    def _calculate_avg_response_time(self, components: dict[str, ComponentHealth]) -> float:
        ...
        response_times = [
            c.response_time_ms for c in components.values() if c.response_time_ms is not None
        ]
