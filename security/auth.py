...
class SecurityManager:
    ...
    def create_user_session(
        ...
        logger.info(
            f"Session created for user {user.username} from IP {session.ip_address}"
        )
        return session
        ...
        try:
            ...
            if exp_timestamp:
                ...
                seconds_until_exp = int(
                    (expire_time - datetime.utcnow()).total_seconds()
                )

                if seconds_until_exp > 0:
                    self.redis_client.setex(
                        f"revoked_token:{token}", seconds_until_exp, "revoked"
                    )
    ...
    def _cache_token(
        self, token: str, payload: dict[str, Any], expire: datetime
    ) -> None:
        """Cache token in Redis for fast validation"""
