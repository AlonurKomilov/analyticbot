...
from apps.mtproto.di import get_repositories, create_tg_client, create_stats_loader
...
async def main():
    ...
    try:
        ...
        if not settings.MTPROTO_ENABLED or not settings.MTPROTO_STATS_ENABLED:
            logger.info("Stats task disabled by flags (MTPROTO_ENABLED=%s, MTPROTO_STATS_ENABLED=%s)", 
                       settings.MTPROTO_ENABLED, settings.MTPROTO_STATS_ENABLED)
            return
        ...
        logger.info("Starting stats loader for %d peers: %s", 
                   len(settings.MTPROTO_STATS_PEERS), settings.MTPROTO_STATS_PEERS)
        ...
        try:
            ...
            for result in results:
                if result.get("skipped"):
                    logger.warning("Skipped peer %s: %s", result["peer"], result.get("reason", "unknown"))
                else:
                    logger.info("Processed peer %s: %d graphs, %d daily points", 
                               result["peer"], result.get("graphs", 0), result.get("daily_points", 0))
