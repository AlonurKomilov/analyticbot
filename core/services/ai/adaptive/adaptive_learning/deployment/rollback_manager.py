...
from ..protocols.learning_protocols import ModelMetadata
...
class RollbackManager:
    ...
    async def add_model_monitoring(
        self, model_id: str, current_version: str, version_history: list[str]
    ) -> bool:
        ...
        try:
            ...
            logger.info(
                f"📊 Added model {model_id} to rollback monitoring (current: {current_version})"
            )
            return True
    ...
    async def trigger_manual_rollback(
        ...
        try:
            ...
            if (
                model_id not in self.model_versions
                or target_version not in self.model_versions[model_id]
            ):
            ...
            # Get current version
            current_version = (
                self.model_versions[model_id][0] if self.model_versions[model_id] else "unknown"
            )
                    ...
                    try:
                        ...
                        for rule in triggered_rules:
                            if rule.auto_execute:
                                ...
                                logger.warning(
                                    f"⚠️ Rollback condition met but auto-execute disabled: {rule.rule_id}"
                                )
