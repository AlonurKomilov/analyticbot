...
from typing import List, Optional, AsyncContextManager, Protocol, Any, Dict
...
class AccountLeaseContext:
    ...
    async def __aenter__(self):
        ...
        logger.debug(f"Acquired lease for account {self.account.name} "
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        logger.debug(f"Released lease for account {self.account.name} "
                    f"(inflight: {self.account.inflight_requests})")
...
class AccountPool:
    ...
    async def _select_best_account(self) -> Optional[AccountState]:
        ...
        logger.debug(f"Selected account {selected.name} "
    ...
    async def _check_quarantine_recovery(self) -> None:
        ...
        for account in self.accounts:
            if (account.status == AccountStatus.QUARANTINED and 
                account.last_failure and 
                current_time - account.last_failure > self.quarantine_duration):
    ...
    async def _handle_account_error(self, account: AccountState, error: Exception) -> None:
        ...
        if account.fail_count >= self.fail_threshold:
            ...
            logger.warning(f"Account {account.name} marked unhealthy "
