...
from apps.workers.celery_app import enhanced_retry_task
...
async def smart_collect_post_metrics(
    ...
    try:
        async with get_db_connection() as db:
            ...
            posts = await _get_posts_to_check(db, min_age_hours, max_age_hours, max_age_days)
...
async def _get_posts_to_check(
    ...
    if min_age_hours > 0:
        age_conditions.append(f"p.date < NOW() - INTERVAL '{min_age_hours} hours'")

    if max_age_hours:
        age_conditions.append(f"p.date >= NOW() - INTERVAL '{max_age_hours} hours'")

    if max_age_days:
        age_conditions.append(f"p.date >= NOW() - INTERVAL '{max_age_days} days'")
...
async def _smart_collect_single_post(
    ...
    if should_save:
        ...
        stats["skipped"] += 1
        logger.debug(
            f"Skipped snapshot for {channel_id}/{msg_id} (no change, check #{check_count + 1})"
        )
...
async def _fetch_telegram_metrics(
    ...
    result = await db.execute(query, {"channel_id": channel_id, "msg_id": msg_id})
    row = result.fetchone()
...
async def cleanup_duplicate_post_metrics(
    ...
    try:
        async with get_db_connection() as db:
            ...
            count_result = await db.execute(text("SELECT COUNT(*) as count FROM post_metrics"))
