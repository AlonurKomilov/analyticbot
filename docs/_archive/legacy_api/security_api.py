...
async def register(request: Request, registration_data: RegisterRequest):
    """
    📝 User Registration
...
async def verify_mfa_setup(token: str, current_user: dict[str, Any] = Depends(get_current_user)):
    """
    ✅ Verify MFA Setup
...
async def update_user_role(
    ...
    try:
        ...
        logger.info(
            f"User {safe_user_id} role updated to {new_role.value} by admin {safe_admin_username}"
        )
