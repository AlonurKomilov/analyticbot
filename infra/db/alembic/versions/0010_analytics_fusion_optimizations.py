...
def upgrade():
    ...
    op.create_index('idx_posts_date_desc', 'posts', ['date'], postgresql_using='btree', postgresql_ops={'date': 'DESC'})
    
    # Add indexes for post_metrics table for analytics queries
    op.create_index('idx_post_metrics_channel_time', 'post_metrics', ['channel_id', 'snapshot_time'])
    op.create_index('idx_post_metrics_channel_msg_time', 'post_metrics', ['channel_id', 'msg_id', 'snapshot_time'])
    op.create_index('idx_post_metrics_views_desc', 'post_metrics', ['views'], postgresql_using='btree', postgresql_ops={'views': 'DESC'})
    ...
    op.create_index('idx_stats_raw_fetched_desc', 'stats_raw', ['fetched_at'], postgresql_using='btree', postgresql_ops={'fetched_at': 'DESC'})
    ...
    op.create_index('idx_mv_channel_daily_recent', 'mv_channel_daily_recent', ['channel_id', 'metric', 'day'])
    ...
    op.create_index('idx_mv_post_metrics_recent_views', 'mv_post_metrics_recent', ['views'], postgresql_using='btree', postgresql_ops={'views': 'DESC'})
