"""
Analytics adapter pattern for mock/real data source separation
"""
...
class MockAnalyticsAdapter(AnalyticsAdapter):
    ...
    def _generate_realistic_data(self, base_value: int, days: int, trend: float = 0.0) -> List[Dict[str, Any]]:
    ...
    async def get_channel_analytics(
        ...
        analytics = {
            ...
            "overview": {
                ...
                "avg_views_per_post": round(sum(item["value"] for item in views_data) / max(sum(item["value"] for item in posts_data), 1)),
            },
            "time_series": {
    ...
    async def get_post_analytics(
        ...
        analytics = {
            ...
            "metrics": {
                ...
                "engagement_rate": round((base_likes + base_shares + base_comments) / base_views * 100, 2),
    ...
    async def get_engagement_metrics(
        ...
        metrics = {
            ...
            "overview": {
                ...
                "average_daily_engagement": round(sum(item["value"] for item in engagement_data) / days, 2),
                "engagement_growth": round(
                    (engagement_data[-1]["value"] - engagement_data[0]["value"]) / engagement_data[0]["value"] * 100, 2
                ),
                "peak_engagement_day": max(engagement_data, key=lambda x: x["value"])["date"]
            },
            "breakdown": {
                "likes": {
                    "total": sum(item["value"] for item in engagement_data) * random.uniform(0.6, 0.8),
                    "percentage": random.randint(60, 75)
                },
                "shares": {
                    "total": sum(item["value"] for item in engagement_data) * random.uniform(0.1, 0.2),
                    "percentage": random.randint(10, 20)
                },
                "comments": {
                    "total": sum(item["value"] for item in engagement_data) * random.uniform(0.1, 0.15),
                    "percentage": random.randint(8, 18)
                },
                "saves": {
                    "total": sum(item["value"] for item in engagement_data) * random.uniform(0.05, 0.1),
        ...
        metrics = {
            ...
            "overview": {
                ...
                "growth_percentage": round(
                    (followers_data[-1]["value"] - followers_data[0]["value"]) / followers_data[0]["value"] * 100, 2
                ),
                "average_daily_growth": round(
                    (followers_data[-1]["value"] - followers_data[0]["value"]) / days, 2
                ),
                "best_growth_day": max(growth_rates, key=lambda x: x["new_followers"])["date"] if growth_rates else None
            },
