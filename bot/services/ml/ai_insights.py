...
class AIInsightsGenerator:
    ...
    def _analyze_data_overview(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        try:
            ...
            if overview["duplicated_records"] > 0:
                quality_issues.append(
                    f"Duplicated records found: {overview['duplicated_records']}"
                )
                ...
                for col in numeric_cols:
                    ...
                    if len(col_data) > 0:
                        ...
                        if abs(col_insights["distribution"]["skewness"]) > 2:
                            col_insights["distribution_note"] = (
        ...
        try:
            ...
            try:
                ...
                anomalies["statistical_zscore"] = {
                    ...
                    "anomaly_percentage": (len(z_anomaly_indices) / len(clean_df))
                    * 100,
    ...
    def _analyze_time_series_trend(
        self, dates: pd.Series, values: pd.Series
    ) -> dict[str, Any]:
                ...
                try:
                    ...
                    trend_info["seasonal_decomposition"] = {
                        "trend_strength": float(
                            np.var(decomposition.trend.dropna()) / np.var(ts)
                        ),
                        ...
                        strong_correlations.append(
                            {
                                ...
                                "strength": (
                                    "very_strong" if abs(corr_val) > 0.9 else "strong"
                                ),
                            }
                        )
    ...
    async def _generate_insight_summary(self, insights: dict[str, Any]) -> str:
        ...
        try:
            ...
            if patterns:
                ...
                summary_parts.append(
                    f"Identified {pattern_count} distinct data patterns."
                )
            ...
            if "strong_correlations" in correlations:
                ...
                if strong_corr_count > 0:
                    summary_parts.append(
                        f"Found {strong_corr_count} strong variable correlations."
                    )
