...
from core.services.ai.adaptive.adaptive_learning.protocols.learning_protocols import (
    LearningStrategy,
    LearningTask,
    UpdateStatus,
)
...
class TaskCreator:
    ...
    def __init__(self, config: TaskCreationConfig | None = None):
    ...
    async def create_task(
        ...
        try:
            ...
            full_config = await self._prepare_task_config(model_id, learning_strategy, task_config)
    ...
    async def estimate_task_resources(self, task_config: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "estimated_duration_minutes": await self._estimate_duration_from_config(
                    task_config
                ),
            }
    ...
    async def start_creation_service(self) -> bool:
        """Start the task creation service"""
        try:
            logger.info("✅ TaskCreator service started")
            return True
        except Exception as e:
            logger.error(f"❌ Failed to start TaskCreator: {e}")
            return False

    async def stop_creation_service(self) -> bool:
        """Stop the task creation service"""
        try:
            logger.info("✅ TaskCreator service stopped")
            return True
        except Exception as e:
            logger.error(f"❌ Failed to stop TaskCreator: {e}")
            return False

    async def get_task_info(self, task_id: str) -> dict[str, Any] | None:
        """Get information about a task (placeholder implementation)"""
        # This would typically query a task registry or database
        logger.info(f"Getting task info for {task_id}")
        return {
            "task_id": task_id,
            "status": "unknown",
            "message": "TaskCreator does not maintain task registry",
        }

    async def health_check(self) -> bool:
        """Check the health of the task creator"""
        try:
            # Basic health check - verify configuration is valid
            return (
                self.config.max_retries > 0
                and self.config.default_batch_size > 0
                and self.config.memory_limit_mb > 0
                and self.config.task_timeout_minutes > 0
            )
        except Exception as e:
            logger.error(f"❌ TaskCreator health check failed: {e}")
            return False
