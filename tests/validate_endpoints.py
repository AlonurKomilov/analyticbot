...
def test_endpoint_imports():
    ...
    try:
        ...
        print("📋 Available routes in analytics_v2:")
        for route in routes:
            print(f"   - {route}")
...
def test_request_models():
    ...
    try:
        ...
        request1 = ChannelDataRequest(channel_id="123", include_real_time=True, format="detailed")
        print("✅ ChannelDataRequest model works")
        print(f"   Example: {request1.dict()}")

        # Test PerformanceMetricsRequest
        request2 = PerformanceMetricsRequest(channels=["123", "456"], period="30d")
