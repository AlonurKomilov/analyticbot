...
class TelegramStatsService:
    ...
    async def get_channel_stats(
        self, user_id: int, channel_id: int, dark: bool = False
        ...
        try:
            ...
            if not user_client:
                result.error_message = (
                    "MTProto client not available. Please configure MTProto in settings."
                )
            ...
            try:
                ...
                logger.info(
                    f"🔍 Attempting to get channel entity: raw={channel_id}, actual_channel_id={actual_channel_id}"
                )
                ...
                if not entity:
                    ...
                    result.error_message = "Channel not found. The MTProto client may not have access to this channel. Please ensure the channel is in your dialogs."
            ...
            try:
                if is_megagroup:
                    ...
                    stats = await client(GetMegagroupStatsRequest(channel=entity, dark=dark))
                else:
                    # Get broadcast channel stats
                    from telethon.tl.functions.stats import GetBroadcastStatsRequest

                    stats = await client(GetBroadcastStatsRequest(channel=entity, dark=dark))
                ...
                if "broadcast_required" in error_str:
                    ...
                    result.error_message = (
                        "Statistics require at least 500 subscribers and admin access"
                    )
    ...
    async def _parse_stats_response(
        self, stats: Any, channel_id: int, is_megagroup: bool
    ) -> TelegramChannelStats:
        """
        ...
        try:
            ...
            growth_graph = getattr(stats, "growth_graph", None) or getattr(
                stats, "members_graph", None
            )
            if growth_graph:
            ...
            if json_data:
                ...
                if x_column and y_columns:
                    for i, timestamp in enumerate(x_column):
                        ...
                        subscribers = (
                            y_columns[0][i + 1]
                            if len(y_columns) > 0 and i + 1 < len(y_columns[0])
                            else 0
                        )
                        joined = (
                            y_columns[1][i + 1]
                            if len(y_columns) > 1 and i + 1 < len(y_columns[1])
                            else 0
                        )
                        left = (
                            y_columns[2][i + 1]
                            if len(y_columns) > 2 and i + 1 < len(y_columns[2])
                            else 0
                        )
