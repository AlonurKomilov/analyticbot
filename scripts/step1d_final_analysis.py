...
def count_current_violations():
    ...
    for violation_group in sorted_violations[:5]:  # Top 5
        ...
        for violation in violation_group["violations"][:3]:  # First 3 examples
            print(
                f"      📄 {violation['file']}:{violation['line']} → {violation['target_module']}"
            )
...
def show_fixes_applied():
    ...
    if exceptions_file.exists():
        print("🎯 Created shared exception classes:")
        print("   📄 exceptions.py")
...
if __name__ == "__main__":
    ...
    print(
        f"   🎯 Priority recommendations: {len([r for r in recommendations if r['priority'] == 'HIGH'])}"
    )
    print()
