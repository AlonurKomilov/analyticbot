...
class OptimizedContainer(containers.DeclarativeContainer):
    ...
    performance_manager = providers.Singleton(lambda: performance_manager)

    # Database components
    database_manager = providers.Singleton(lambda: db_manager)

    database_pool = providers.Resource(providers.Coroutine(db_manager.create_pool))
    ...
    bot = providers.Singleton(
        Bot, token=settings.BOT_TOKEN.get_secret_value(), session=bot_session
    )

    dispatcher = providers.Singleton(Dispatcher)

    # Repository layer with connection pooling
    user_repository = providers.Singleton(UserRepository, db_pool=database_pool)

    channel_repository = providers.Singleton(ChannelRepository, db_pool=database_pool)

    analytics_repository = providers.Singleton(
        AnalyticsRepository, db_pool=database_pool
    )

    scheduler_repository = providers.Singleton(
        SchedulerRepository, db_pool=database_pool
    )

    plan_repository = providers.Singleton(PlanRepository, db_pool=database_pool)

    # Optimized service layer
    optimized_analytics_service = providers.Singleton(
        OptimizedAnalyticsService, bot=bot, analytics_repository=analytics_repository
    )
    ...
    scheduler_service = providers.Singleton(
        SchedulerService, scheduler_repository=scheduler_repository, bot=bot
    )

    # Monitoring services
    prometheus_service = providers.Singleton(lambda: prometheus_service)
    ...
    async def _check_ml_services_health(self) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                ml_health[
                    "prediction_service"
                ] = await prediction_service.health_check()
            except Exception as e:
            ...
            try:
                ...
                ml_health[
                    "engagement_analyzer"
                ] = await engagement_analyzer.health_check()
            except Exception as e:
    ...
    async def get_performance_metrics(self) -> dict[str, Any]:
        ...
        try:
            ...
            if performance_manager:
                metrics[
                    "performance"
                ] = await performance_manager.get_performance_stats()
