...
class SimplifiedPerformanceTester:
    ...
    async def measure_async_function(
        self, name: str, func, iterations: int = 10
    ) -> dict[str, Any]:
    ...
    def measure_sync_function(
        self, name: str, func, iterations: int = 100
    ) -> dict[str, Any]:
...
async def test_basic_async_operations():
    ...
    await tester.measure_async_function(
        "async_computation", async_computation_test, 100
    )
    await tester.measure_async_function(
        "concurrent_operations", concurrent_operations_test, 20
    )
    ...
    try:
        ...
        for category, results in all_results.items():
            if "summary" in results:
                ...
                logger.info(
                    f"   ⚡ Avg throughput: {summary['avg_throughput']:.1f} ops/sec"
                )
        ...
        if all_results and "async_operations" in all_results:
            ...
            if system_metrics:
                logger.info("\n🖥️ SYSTEM HEALTH:")
                logger.info(
                    f"   🔥 CPU Usage: {system_metrics.get('cpu_percent', 'N/A')}%"
                )
                logger.info(
                ...
                logger.info(
                    f"   💿 Disk Usage: {system_metrics.get('disk_usage_percent', 'N/A')}%"
                )
