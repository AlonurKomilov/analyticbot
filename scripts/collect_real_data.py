...
async def collect_from_channel(client, repos, channel_username, limit=50):
    ...
    try:
        ...
        print(f"   🛡️ Processing with rate limiting protection...")
        ...
        async for message in client._client.iter_messages(entity, limit=limit):
            if message.text:
                ...
                if processed_count % 20 == 0:
                    print(f"   🛡️ Processed {processed_count} messages, extended sleep 10 seconds...")
                    await asyncio.sleep(10)
        
        print(f"✅ Collected {messages_collected} messages from {channel_username}")
        print(f"   🛡️ Applied {processed_count * 0.5 + (processed_count // 5) * 3 + (processed_count // 20) * 10:.1f}s of rate limiting delays")
    ...
    try:
        ...
        for channel in settings.MTPROTO_PEERS:
            try:
                ...
                print(f"🛡️ Sleeping 8 seconds between channels for safety...")
        ...
        print(f"   🛡️ Rate limiting protected your account")
