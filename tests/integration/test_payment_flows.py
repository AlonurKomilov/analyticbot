...
class TestPaymentFlowIntegration:
    ...
    async def test_successful_payment_workflow(self, mock_db_pool, mock_redis):
        ...
        with patch('infra.db.repositories.payment_repository.AsyncpgPaymentRepository') as mock_payment_repo:
    ...
    async def _process_payment_with_retry(self, payment, payment_repo, max_retries=3):
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                await payment_repo._pool.execute("UPDATE payments SET status = 'completed' WHERE id = $1", payment["id"])
                return {
                    "success": True,
                    "retry_count": attempt
                }
