...
# Auth
from src.api_service.middleware.auth import get_current_user, require_channel_access

# Schemas
# Schemas
from src.api_service.schemas.analytics import PostListResponse
...
async def get_channel_engagement_insights(
    channel_id: int,
    period: str = Query("24h", regex="^(24h|7d|30d|90d)$", description="Time period for engagement analysis"),
    metrics_type: str = Query("comprehensive", regex="^(basic|comprehensive|advanced)$", description="Level of engagement metrics"),
    ...
    try:
        ...
        with performance_timer("channel_engagement_insights"):
            ...
            if metrics_type == "advanced":
                engagement_trends = await analytics_service.get_engagement_trends(channel_id, period)
...
async def get_channel_audience_insights(
    channel_id: int,
    analysis_depth: str = Query("standard", regex="^(basic|standard|detailed)$", description="Depth of audience analysis"),
    ...
    try:
        ...
        with performance_timer("channel_audience_insights"):
            ...
            if include_behavior and analysis_depth == "detailed":
                behavior_patterns = await analytics_service.get_audience_behavior_patterns(channel_id)
...
async def get_trending_content_analysis(
    ...
    method: str = Query(default="zscore", regex="^(zscore|ewma|hybrid)$", description="Trending detection method"),
...
async def get_engagement_patterns_analysis(
    ...
    pattern_type: str = Query("temporal", regex="^(temporal|content|user)$", description="Type of pattern analysis"),
    current_user: dict = Depends(get_current_user),
    analytics_service: AnalyticsFusionService = Depends(get_analytics_fusion_service),
    cache=Depends(get_cache),
):
    ...
    try:
        ...
        with performance_timer("engagement_patterns_analysis"):
            ...
            if pattern_type == "temporal":
                patterns_data = await analytics_service.get_temporal_engagement_patterns(channel_id, days)
            elif pattern_type == "content":
                patterns_data = await analytics_service.get_content_engagement_patterns(channel_id, days)
            else:  # user patterns
                patterns_data = await analytics_service.get_user_engagement_patterns(channel_id, days)
