...
def check_file_for_i18n(filepath: Path) -> dict:
    ...
    uses_i18n = bool(
        re.search(r"useTranslation|import.*i18next|import.*i18n", content)
        or re.search(r'\bt\(["\']', content)  # t() function calls
    )
...
def analyze_frontend():
    ...
    base_path = Path(
        "/home/abcdev/projects/analyticbot/apps/frontend/apps/app_analyticbot/user/src"
    )
    ...
    print(
        f"  Files NOT using i18n: {files_without_i18n} ({files_without_i18n / files_checked * 100:.1f}%)"
    )
