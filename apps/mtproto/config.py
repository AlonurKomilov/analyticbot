...
class MTProtoSettings(BaseSettings):
    ...
    MTPROTO_PROXY_ENABLED: bool = Field(
        default=False,
        description="Enable proxy pool rotation for enhanced reliability"
    )
    ...
    MTPROTO_MAX_CONCURRENCY_PER_ACCOUNT: int = Field(
        default=2,
        description="Maximum concurrent requests per account"
    )
    
    MTPROTO_GLOBAL_RPS: float = Field(
        default=2.5,
        description="Global requests per second limit across all accounts"
    )
    ...
    MTPROTO_PROXY_FAIL_SCORE_LIMIT: int = Field(
        default=3,
        description="Number of failures before marking proxy as unhealthy"
    )
    ...
    MTPROTO_CONCURRENCY: int = Field(
        default=4,
        description="Number of concurrent requests to Telegram API"
    )
    
    MTPROTO_SLEEP_THRESHOLD: float = Field(
        default=1.5,
        description="Sleep threshold in seconds for rate limiting"
    )
