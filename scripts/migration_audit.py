...
def audit_migration():
    ...
    if remaining_files.get('core'):
        issues.append(f"• {len(remaining_files['core'])} files still in core/ - may cause import conflicts")
    
    if remaining_files.get('infra'):
        issues.append(f"• {len(remaining_files['infra'])} files still in infra/ - database/cache logic not migrated")
    
    if remaining_files.get('apps'):
        issues.append(f"• {len(remaining_files['apps'])} files still in apps/ - service logic not migrated")
    
    # Check for legacy patterns
    if os.path.exists('core/di_container.py'):
        issues.append("• DI Container still in core/ - should be in src/shared/ or src/infrastructure/")
