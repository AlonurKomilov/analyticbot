...
class TestSecuritySystem:
    ...
    def test_session_management(self):
        ...
        # Create session
        session = self.security_manager.create_user_session(
            self.test_user, mock_request
        )
    ...
    def test_rbac_permissions(self):
        ...
        # Test role hierarchy
        admin_user = User(
            email="admin@test.com", username="adminuser", role=UserRole.ADMIN
        )
        ...
        assert (
            self.rbac_manager.has_permission(admin_user, Permission.USER_DELETE) == True
        )
        assert (
            self.rbac_manager.has_permission(admin_user, Permission.SYSTEM_ADMIN)
            == True
        )

        # Test user permissions
        assert (
            self.rbac_manager.has_permission(user_user, Permission.ANALYTICS_READ)
            == True
        )
        assert (
            self.rbac_manager.has_permission(user_user, Permission.USER_DELETE) == False
        )
    ...
    def test_permission_matrix(self):
        ...
        analyst_user = User(
            email="analyst@test.com", username="analystuser", role=UserRole.ANALYST
        )
