...
class OverviewDTO(BaseModel):
    ...
    err: float | None = Field(None, description="Engagement rate ratio (avg_reach/followers * 100)")
    followers: int | None = Field(None, description="Current follower/subscriber count")
...
class PointDTO(BaseModel):
    ...
    t: str | datetime = Field(..., description="Timestamp (ISO string or datetime)")
    y: float | int = Field(..., description="Value at this time point")
...
class SeriesDTO(BaseModel):
    ...
    points: list[PointDTO] = Field(
        default_factory=list, description="Data points in chronological order"
    )
...
class PostDTO(BaseModel):
    ...
    reactions: dict | None = Field(default_factory=dict, description="Reactions data")
    title: str | None = Field(None, description="Post title or excerpt")
    permalink: str | None = Field(None, description="Direct link to the post")
...
class TrendingQuery(BaseModel):
    ...
    method: Literal["zscore", "ewma"] = Field(
        default="zscore", description="Trending detection method"
    )
