...
class MonitoringInfrastructureService:
    ...
    def __init__(self, config: MonitoringConfig | None = None):
        ...
        self.thresholds_cache: dict[
            str, dict[PerformanceMetricType, tuple[float, AlertSeverity]]
        ] = {}
        ...
        try:
            ...
            for model_id in list(self.metrics_cache.keys()):
                ...
                self.metrics_cache[model_id] = [
                    metric
                    for metric in self.metrics_cache[model_id]
            ...
            for model_id in list(self.alerts_cache.keys()):
                ...
                self.alerts_cache[model_id] = [
                    alert for alert in self.alerts_cache[model_id] if alert.timestamp > cutoff_date
                ]
    ...
    async def set_alert_threshold(
        ...
        try:
            ...
            logger.info(
                f"📊 Set alert threshold for {model_id}/{metric_type.value}: {threshold_value} ({severity.value})"
            )
            return True
