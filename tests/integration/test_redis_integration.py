...
def sample_cache_data():
    ...
    return {
        ...
        "analytics_data": {
            ...
            "metrics": {"views": 1000, "subscribers": 50, "engagement_rate": 0.85},
...
def rate_limit_config():
    ...
    return {
        "api_calls": {"window_seconds": 60, "max_requests": 100},
        "message_sending": {"window_seconds": 3600, "max_requests": 50},
        "payment_attempts": {"window_seconds": 86400, "max_requests": 5},
    }
...
class TestRedisSessionManagement:
    ...
    async def test_user_session_creation(self, mock_redis_client):
        ...
        await mock_redis_client.hset(
            session_id,
            mapping={
                k: json.dumps(v) if isinstance(v, dict) else str(v) for k, v in session_data.items()
            },
        )
    ...
    async def test_session_state_transitions(self, mock_redis_client):
        ...
        for i, next_state in enumerate(states[1:], 1):
            ...
            await mock_redis_client.hset(
                session_key, f"state_changed_{i}", datetime.now().isoformat()
            )
...
class TestRedisRateLimiting:
    ...
    async def test_rate_limit_reset(self, mock_redis_client):
        ...
        for i in range(5):
            await mock_redis_client.zadd(rate_limit_key, {f"req_{i}": current_time + i})
...
class TestRedisCacheInvalidation:
    ...
    async def test_tag_based_cache_invalidation(self, mock_redis_client):
        ...
        cache_entries = [
            {
                "key": "analytics:channel:123",
                "data": {"views": 1000},
                "tags": ["analytics", "channel:123"],
            },
            {
                "key": "analytics:channel:456",
                "data": {"views": 2000},
        ...
        for entry in cache_entries:
            # Store actual data
            await mock_redis_client.set(entry["key"], json.dumps(entry["data"]), ex=3600)
    ...
    async def test_time_based_cache_invalidation(self, mock_redis_client):
        ...
        for entry in ttl_entries:
            await mock_redis_client.set(entry["key"], f"data_for_{entry['key']}", ex=entry["ttl"])
