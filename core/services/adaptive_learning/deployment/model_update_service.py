...
class ModelUpdateService(ModelUpdateProtocol):
    ...
    async def get_rollback_status(self, rollback_id: str) -> dict[str, Any] | None:
        """Get rollback status"""
        try:
    ...
    async def list_deployments(self, model_id: str | None = None) -> list[dict[str, Any]]:
    ...
    async def list_rollbacks(self, model_id: str | None = None) -> list[dict[str, Any]]:
    ...
    async def get_model_deployment_status(self, model_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            active_deployments = [
                progress
                for progress in await self.executor.get_active_deployments()
            ...
            active_rollbacks = [
                rollback
                for rollback in await self.rollback_manager.list_active_rollbacks()
