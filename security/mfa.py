...
class MFAManager:
    ...
    def setup_mfa(self, user: User) -> MFASetupResponse:
        ...
        self.redis_client.setex(
            f"mfa_setup:{user.id}",
            3600,
            json.dumps(setup_data),  # 1 hour expiration
        )
    ...
    def verify_setup_token(self, user: User, token: str) -> bool:
        ...
        try:
            ...
            if is_valid:
                logger.info(
                    f"MFA setup verification successful for user {user.username}"
                )
    ...
    def verify_backup_code(self, user: User, backup_code: str) -> bool:
        ...
        if not self._check_mfa_rate_limit(user.id):
            logger.warning(
                f"Backup code verification rate limited for user {user.username}"
            )
            return False
        ...
        try:
            ...
            for i, stored_code in enumerate(backup_codes):
                if stored_code == normalized_code:
                    ...
                    logger.info(
                        f"Backup code used successfully for user {user.username}"
                    )
                    return True
    ...
    def _record_mfa_attempt(self, user_id: str) -> None:
        ...
        if (
            not self.redis_client.ttl(attempts_key)
            or self.redis_client.ttl(attempts_key) == -1
        ):
