...
from infra.celery.celery_app import celery_app, enhanced_retry_task, critical_message_task
    ...
    async def _send_message():
        try:
            ...
            try:
                from apps.bot.utils.monitoring import metrics
                metrics.record_metric(
                    "message_sent_failure", 1.0,
                    {"chat_id": str(chat_id), "retry_count": self.request.retries, "error_type": type(e).__name__}
                )
            except ImportError:
                pass
...
def process_analytics(self, channel_id: str, post_id: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    async def _process():
        try:
            ...
            logger.info(f"Analytics processed for {channel_id}: {result.get('processed_count', 0)} items")
            ...
            logger.warning(f"Analytics processing failed for {channel_id}, retrying in {delay}s: {e}")
...
def scheduled_broadcast(self, message: str, target_channels: list, schedule_time: str) -> Dict[str, Any]:
    """
