...
class K8sService:
    ...
    def _load_config(self) -> bool:
        ...
        try:
            if self.in_cluster:
                ...
                logger.info(
                    f"Loaded kubeconfig from {self.kubeconfig_path}, context: {self._cluster_name}"
                )
    ...
    def connect(self) -> dict[str, Any]:
        ...
        if self._load_config():
            try:
                ...
                self._connected = False
                return {"status": K8sConnectionStatus.ERROR.value, "error": str(e)}
        else:
            return {
            ...
            return [
                {
                    ...
                    "memory_usage": 62.3,
                    "created_at": (
                        nodes.items[0].metadata.creation_timestamp.isoformat()
                        if nodes.items
                        else None
                    ),
                }
    ...
    def get_ingresses(self, namespace: str | None = None) -> list[dict[str, Any]]:
        """Get all ingresses."""
        if not self._connected and not self._load_config():
            return []
