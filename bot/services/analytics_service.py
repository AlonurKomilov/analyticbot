...
from bot.database.performance import performance_manager, cache_result, performance_timer, PerformanceConfig
...
class AnalyticsService:
    ...
    async def _smart_group_posts(self, posts: List[Dict]) -> Dict[int, List[Dict]]:
        ...
        for channel_id, channel_posts in grouped.items():
            ...
            sorted_grouped[channel_id] = sorted(channel_posts, key=lambda x: x["_priority"], reverse=True)
        
        # Return channels sorted by urgency
        return dict(sorted(sorted_grouped.items(), key=lambda x: sum(p["_priority"] for p in x[1]), reverse=True))
    
    async def _process_channels_concurrent(self, grouped: Dict[int, List[Dict]], stats: Dict[str, int]):
        ...
        for channel_id, channel_posts in grouped.items():
            if hasattr(self, '_process_channel_optimized'):
                ...
                task = asyncio.create_task(
                    self._process_channel_posts(channel_id, channel_posts)
                )
            tasks.append(task)
    ...
    async def _process_channels_sequential(self, grouped: Dict[int, List[Dict]], stats: Dict[str, int]):
    ...
    async def _process_channel_optimized(self, channel_id: int, posts: List[Dict]) -> Dict[str, int]:
        ...
        async with self._semaphore:
            ...
            try:
                ...
                for i in range(0, len(posts), micro_batch_size):
                    ...
                    success_rate = batch_stats.get("updated", 0) / len(micro_batch) if micro_batch else 1
                    delay = self._rate_limit_delay * (2 - success_rate)  # Less delay for successful batches
    ...
    async def _get_post_views_with_cache(self, channel_id: int, post: Dict) -> Optional[int]:
    ...
    async def _get_single_post_views(self, channel_id: int, post: Dict) -> Optional[int]:
    ...
    async def _sequential_update_views(self, updates: List[Dict]) -> int:
        ...
        for update in updates:
            try:
                await self.analytics_repository.update_post_views(
                    scheduled_post_id=update["post_id"],
    ...
    async def get_posts_ordered_by_views(self, channel_id: int) -> Optional[List[Dict]]:
        """
    ...
    async def get_post_views(self, scheduled_post_id: int, user_id: Optional[int] = None) -> Optional[int]:
        """
    ...
    async def get_analytics_data(
        ...
        try:
            ...
            context = (ErrorContext()
                      .add("operation", "get_analytics_data")
                      .add("channel_id", channel_id))
            ErrorHandler.handle_database_error(e, context)
            return []
    ...
    async def get_dashboard_data(self, channel_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            context = (ErrorContext()
                      .add("operation", "get_dashboard_data")
                      .add("channel_id", channel_id))
            ErrorHandler.handle_database_error(e, context)
            return {}
        ...
        try:
            if hasattr(performance_manager, 'pool'):
                async with performance_manager.pool.acquire_connection() as conn:
                    ...
                    insert_params = [
                        (item["channel_id"], item["message_id"], item["view_count"], 
