...
class AnalyticsService:
    ...
    @cache_result(
        "posts_to_track", ttl=getattr(PerformanceConfig, "CACHE_ANALYTICS_TTL", 300)
    )
            ...
            try:
                ...
                for i in range(0, len(posts), micro_batch_size):
                    ...
                    if hasattr(self, "_process_micro_batch"):
                        ...
                        batch_stats = await self._process_post_batch(
                            channel_id, micro_batch
                        )
        ...
        try:
            ...
            if updates_to_execute:
                if hasattr(self, "_batch_update_views"):
                    ...
                    updated_count = await self._sequential_update_views(
                        updates_to_execute
                    )
            ...
            logger.error(
                f"❌ Micro-batch processing failed for channel {channel_id}: {e}"
            )
    ...
    async def _get_single_post_views(self, channel_id: int, post: dict) -> int | None:
        ...
        try:
            message = await self.bot.get_message(
                chat_id=channel_id, message_id=post["message_id"]
            )
            ...
            if hasattr(performance_manager, "pool"):
                async with performance_manager.pool.acquire_connection() as conn:
                    ...
                    if hasattr(performance_manager, "query_optimizer"):
                        results = (
                            await performance_manager.query_optimizer.execute_batched(
    ...
    @cache_result(
        "channel_analytics", ttl=getattr(PerformanceConfig, "CACHE_ANALYTICS_TTL", 300)
    )
    async def get_channel_analytics_cached(
        ...
        try:
            return await self.analytics_repository.get_channel_analytics(
                channel_id, days
            )
        except Exception:
    ...
    @cache_result(
        "top_posts", ttl=getattr(PerformanceConfig, "CACHE_ANALYTICS_TTL", 300)
    )
    ...
    async def refresh_channel_analytics(self, channel_id: int):
        ...
        try:
            ...
            posts = await self.analytics_repository.get_channel_posts_for_tracking(
                channel_id
            )
            if posts:
