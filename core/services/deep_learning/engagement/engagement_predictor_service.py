...
from ..protocols import EngagementPredictorProtocol
...
class EngagementPredictorService:
    ...
    def __init__(
        ...
        model_config: Optional[LSTMEngagementModelConfig] = None
    ):
        ...
        # Model state
        self.model: Optional[LSTMEngagementModel] = None
    ...
    async def _initialize_model(self) -> None:
        ...
        try:
            ...
            if self.model is not None:
                ...
                logger.info(f"🚀 Engagement model ready with {self.model.get_param_count()} parameters")
        ...
        try:
            ...
            with torch.no_grad():
                if include_confidence:
                    ...
                    confidence_score = float(1.0 / (1.0 + confidence_std.item()))  # Convert std to confidence
                else:
            ...
            logger.info(f"✅ Engagement prediction completed: {predicted_engagement:.3f} (confidence: {confidence_score:.3f})")
            return result
    ...
    async def batch_predict_engagement(self, requests: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            if not self.model_loaded or self.model is None:
                return [{"error": "Model not loaded", "service": "engagement_predictor"} for _ in requests]
            ...
            if not batch_tensors:
                return [{"error": "No valid requests", "service": "engagement_predictor"} for _ in requests]
