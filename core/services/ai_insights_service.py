    ...
    async def generate_ai_insights(
        ...
        try:
            ...
            if analysis_type in ["content", "comprehensive"]:
                insights_report["content_insights"] = await self._analyze_content_patterns(
                    analysis_data
                )
            ...
            if analysis_type in ["audience", "comprehensive"]:
                insights_report["audience_insights"] = await self._analyze_audience_behavior(
                    analysis_data
                )
                insights_report["confidence_score"] += 0.3
            ...
            if analysis_type in ["performance", "comprehensive"]:
                insights_report["performance_insights"] = await self._analyze_performance_patterns(
                    analysis_data
                )
            ...
            if analysis_type in ["comprehensive"]:
                insights_report["ai_predictions"] = await self._generate_ai_predictions(
            ...
            insights_report["key_patterns"] = await self._extract_key_patterns(
                analysis_data, insights_report
            )
            ...
            logger.info(
                f"AI insights generated for channel {channel_id} with confidence {insights_report['confidence_score']:.2f}"
            )
    ...
    async def _analyze_content_patterns(self, data: dict) -> dict:
        ...
        try:
            ...
            if length_performance:
                ...
                content_insights["content_length_analysis"] = {
                    ...
                    "recommendation": self._get_length_recommendation(
                        optimal_length, length_correlation
                    ),
                }
            ...
            if timing_performance:
                ...
                content_insights["timing_patterns"] = {
                    ...
                    "performance_lift": float(
                        (best_performance - avg_performance) / avg_performance * 100
                    ),
            ...
            if engagement_factors:
                ...
                # Question marks
                with_questions = [
                    f["engagement_score"] for f in engagement_factors if f["has_question"]
                ]
                without_questions = [
                    f["engagement_score"] for f in engagement_factors if not f["has_question"]
                ]

                if with_questions and without_questions:
                    question_boost = (
                        (np.mean(with_questions) - np.mean(without_questions))
                        / np.mean(without_questions)
                        * 100
                    )
                    drivers["questions"] = {
                ...
                without_emoji = [
                    f["engagement_score"] for f in engagement_factors if not f["has_emoji"]
                ]

                if with_emoji and without_emoji:
                    emoji_boost = (
                        (np.mean(with_emoji) - np.mean(without_emoji))
                        / np.mean(without_emoji)
                        * 100
                    )
                    drivers["emojis"] = {
            ...
            if (
                content_insights.get("content_length_analysis", {}).get("length_correlation", 0)
                > 0.3
            ):
                ...
                if engagement_scores:
                    audience_insights["engagement_patterns"] = {
                        ...
                        "consistency_score": float(
                            1 - (np.std(engagement_scores) / max(np.mean(engagement_scores), 0.001))
                        ),
            ...
            if followers_data and len(followers_data) > 1:
                ...
                for i in range(1, len(follower_counts)):
                    if follower_counts[i - 1] > 0:
                        rate = (follower_counts[i] - follower_counts[i - 1]) / follower_counts[
                            i - 1
                        ]
                ...
                if growth_rates:
                    audience_insights["growth_patterns"] = {
                        ...
                        "recent_growth": (
                            float(np.mean(growth_rates[-7:]))
                            if len(growth_rates) >= 7
                            else float(np.mean(growth_rates))
                        ),
                    }
                ...
                if views:
                    ...
                    performance_insights["outlier_analysis"] = {
                        ...
                        "outlier_characteristics": (
                            self._analyze_outlier_characteristics(outliers) if outliers else {}
                        ),
                    }
    ...
    async def _extract_key_patterns(self, data: dict, insights_report: dict) -> list:
        ...
        try:
            ...
            if abs(length_analysis.get("length_correlation", 0)) > 0.3:
                correlation_type = (
                    "positive" if length_analysis["length_correlation"] > 0 else "negative"
                )
                patterns.append(
                    {
                        ...
                        "strength": (
                            "strong"
                            if abs(length_analysis["length_correlation"]) > 0.5
                            else "moderate"
                        ),
                        "actionable": True,
                    }
