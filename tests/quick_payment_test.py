...
def main():
    ...
    print("\n🌐 FRONTEND COMPONENTS:")
    frontend_files = [
        ("/home/alonur/analyticbot/apps/frontend/src/components/payment/PaymentForm.jsx", "Payment Form Component"),
        ("/home/alonur/analyticbot/apps/frontend/src/components/payment/SubscriptionDashboard.jsx", "Subscription Dashboard"),
        ("/home/alonur/analyticbot/apps/frontend/src/components/payment/PlanSelector.jsx", "Plan Selector Component"),
        ("/home/alonur/analyticbot/apps/frontend/src/services/paymentAPI.js", "Payment API Service"),
    ...
    print(f"Backend Infrastructure: {backend_completion:.1f}% complete ({sum(backend_results)}/{len(backend_results)})")
    print(f"Frontend Components: {frontend_completion:.1f}% complete ({sum(frontend_results)}/{len(frontend_results)})")
    print(f"Configuration: {config_completion:.1f}% complete ({sum(config_results)}/{len(config_results)})")
    print(f"Module Imports: {import_completion:.1f}% complete ({sum(import_results)}/{len(import_results)})")
    print(f"Dependencies: {'✅ Configured' if deps_result else '❌ Missing'}")
    
    overall_completion = (sum(backend_results) + sum(frontend_results) + sum(config_results) + sum(import_results)) / (len(backend_results) + len(frontend_results) + len(config_results) + len(import_results)) * 100
