...
async def mock_analytics_redis():
    ...
    await client.hset("channel_stats:-123456789", mapping={
        "subscribers": "1500",
        "posts_today": "5",
...
class TestAnalyticsDataCollectionWorkflow:
    ...
    async def test_channel_data_collection_workflow(
        ...
        update_analytics_workflow_state(workflow_id, {
        ...
        growth_metrics = {
            ...
            "growth_rate": ((current_metrics["subscribers"] - previous_data["subscribers"]) / previous_data["subscribers"]) * 100
        }
    ...
    async def test_multi_channel_data_aggregation_workflow(
        ...
        aggregate_metrics.update({
            ...
            "avg_engagement_rate": aggregate_metrics["total_engagement"] / aggregate_metrics["total_views"],
        ...
        mock_analytics_api.get.return_value = AsyncMock(
            ...
            json=AsyncMock(return_value={
                ...
                "data": {
                    ...
                    "historical_data": [
                        {"date": "2024-01-01", "subscribers": 1200, "views": 18000, "engagement": 2700},
                        {"date": "2024-01-15", "subscribers": 1350, "views": 21500, "engagement": 3200},
                        {"date": "2024-01-30", "subscribers": 1500, "views": 25000, "engagement": 3750}
                    ],
                    "insights": {
                        "growth_rate": 25.0,
                        "best_posting_time": "19:00",
...
class TestScheduledAnalyticsWorkflow:
    ...
    async def test_daily_scheduled_analytics_workflow(
        ...
        daily_summary["total_metrics"] = {
            ...
            "total_engagement": sum(data["engagement_today"] for data in daily_summary["channels"].values())
        }
        ...
        next_delivery = datetime.now().replace(hour=9, minute=0, second=0, microsecond=0) + timedelta(days=1)
        await mock_analytics_redis.zadd(
...
class TestRealTimeAnalyticsWorkflow:
    ...
    async def test_real_time_engagement_monitoring_workflow(
        ...
        realtime_data_points = [
            ...
            {"timestamp": datetime.now() + timedelta(minutes=5), "views": 1250, "engagement": 200},  # Spike
            {"timestamp": datetime.now() + timedelta(minutes=10), "views": 1800, "engagement": 290},  # Continued spike
        ]
        ...
        for data_point in realtime_data_points:
            ...
            # Check for spikes
            is_spike = (
                data_point["views"] > baseline_metrics["avg_views_per_hour"] * baseline_metrics["spike_threshold_multiplier"] or
                engagement_rate > baseline_metrics["avg_engagement_rate"] * baseline_metrics["spike_threshold_multiplier"]
            )
            ...
            if is_spike:
                update_analytics_workflow_state(workflow_id, {
                    ...
                    "spike_magnitude": data_point["views"] / baseline_metrics["avg_views_per_hour"]
                })
                ...
                await mock_telegram_analytics.send_message(
                    ...
                    reply_markup={"inline_keyboard": [
                        [{"text": "📱 View Post", "callback_data": f"view_spiking_post_{channel_id}"}],
        ...
        stored_data = await mock_analytics_redis.zrange("realtime_data:" + str(channel_id), 0, -1, withscores=True)
            ...
            "platform_breakdown": {
                "telegram": {
                    ...
                    "engagement_rate": platform_data["telegram"]["total_engagement"] / 
                                     sum(data.get("views", data.get("reach", data.get("total_views", 25000))) 
