...
def test_basic_input_sanitization():
    ...
    assert (
        sanitize_username("<script>alert('xss')</script>") == "scriptalert(xss)script"
    )  # Single quotes removed
    assert (
        sanitize_username("'; DROP TABLE users; --") == "DROP TABLE users"
    )  # Dangerous chars removed and trimmed
...
def test_webhook_signature_validation():
    ...
    valid_signature = (
        "sha256=" + hmac.new(webhook_secret.encode(), payload.encode(), hashlib.sha256).hexdigest()
    )
