...
class AsyncpgPostRepository:
        ...
        async with self.pool.acquire() as conn:
            ...
            if existing:
                ...
                await conn.execute(
                    ...
                    """,
                    channel_id, msg_id, text, json.dumps(links_json)
                )
                ...
                await conn.execute(
                    ...
                    """,
                    channel_id, msg_id, date, text, json.dumps(links_json)
                )
                return {"inserted": True, "updated": False}

    async def max_msg_id(self, channel_id: int) -> Optional[int]:
        ...
        async with self.pool.acquire() as conn:
            result = await conn.fetchval(
                "SELECT MAX(msg_id) FROM posts WHERE channel_id = $1",
                channel_id
            )
            return result

    async def get_post(self, channel_id: int, msg_id: int) -> Optional[dict[str, Any]]:
    ...
    async def top_by_views(self, channel_id: int, from_dt: datetime, to_dt: datetime, limit: int) -> list[dict[str, Any]]:
        ...
        async with self.pool.acquire() as conn:
            records = await conn.fetch(
                ...
                LIMIT $4
                """,
                channel_id, from_dt, to_dt, limit
            )
            ...
            for record in records:
                ...
                post_dict["title"] = text[:100] + "..." if len(text) > 100 else text or f"Post {post_dict['msg_id']}"
