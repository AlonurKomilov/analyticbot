...
from .workflow_models import AdaptiveLearningWorkflow, WorkflowStage, OrchestrationStrategy
...
class WorkflowManager:
    ...
    async def create_workflow(
        ...
        try:
            ...
            if existing_workflow:
                logger.warning(f"⚠️ Active workflow already exists for model {model_id}: {existing_workflow.workflow_id}")
            ...
            logger.info(f"📋 Created workflow {workflow_id} for model {model_id} (strategy: {strategy.value})")
            return workflow_id
    ...
    async def get_workflow(self, workflow_id: str) -> Optional[AdaptiveLearningWorkflow]:
    ...
    async def update_workflow_status(
        ...
        try:
            ...
            if results:
                if workflow.current_stage in [WorkflowStage.LEARNING_TASK_CREATION, WorkflowStage.MODEL_TRAINING]:
    ...
    async def get_workflow_history(self, model_id: str, limit: int = 10) -> List[AdaptiveLearningWorkflow]:
    ...
    def _find_active_workflow_for_model(self, model_id: str) -> Optional[AdaptiveLearningWorkflow]:
    ...
    async def get_status(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                'total_historical_workflows': sum(len(history) for history in self.workflow_history.values())
            }
