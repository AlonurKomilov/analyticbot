...
class TestDatabaseErrorHandling:
    ...
    def test_classify_error_types(self):
        ...
        assert (
            handler.classify_error(Exception("Connection failed"))
            == DatabaseErrorType.CONNECTION_FAILED
        )
        assert handler.classify_error(Exception("Query timeout")) == DatabaseErrorType.TIMEOUT
        assert (
            handler.classify_error(Exception("Pool exhausted")) == DatabaseErrorType.POOL_EXHAUSTED
        )
        assert (
            handler.classify_error(Exception("No rows found")) == DatabaseErrorType.DATA_NOT_FOUND
        )
        assert (
            handler.classify_error(Exception("Unique constraint violation"))
            == DatabaseErrorType.CONSTRAINT_VIOLATION
        )
        assert (
            handler.classify_error(Exception("SQL query failed")) == DatabaseErrorType.QUERY_FAILED
        )
