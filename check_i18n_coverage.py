...
def analyze_coverage():
    ...
    for file_name in translation_files:
        ...
        keys_by_lang = {
            lang: get_all_keys(translations[lang])
            for lang in languages
        }
        ...
        for lang in languages:
            ...
            print(f"{status} {lang.upper()}: {len(lang_keys)}/{total_keys} keys ({coverage_percent:.1f}%)")
    ...
    for lang in languages:
        ...
        lang_keys_count = sum(file_coverage[f]['by_language'][lang]['count'] for f in translation_files)
        missing_count = sum(file_coverage[f]['by_language'][lang]['missing'] for f in translation_files)
