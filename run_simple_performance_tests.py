...
from bot.test_config import test_settings
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        return {
            "summary": {
                ...
                "avg_response_time": statistics.mean([r["avg_time"] for r in successful_tests]) if successful_tests else 0,
                "avg_throughput": statistics.mean([r["throughput"] for r in successful_tests]) if successful_tests else 0
            },
...
def test_cpu_intensive_operations():
    ...
    def fibonacci_calculation():
        ...
        return fib(20)  # Small fibonacci to avoid too long execution
...
async def test_cache_simulation():
    ...
    async def cache_get_operation():
        ...
        if not cache or random.random() < 0.1:  # 10% miss rate
            cache_stats["misses"] += 1
            await asyncio.sleep(0.0001)  # Miss penalty
            return None
        else:
            cache_stats["hits"] += 1
            key = random.choice(list(cache.keys()))
            await asyncio.sleep(0.00005)  # Fast hit
            return cache[key]
    ...
    try:
        ...
        for category, results in all_results.items():
            if "summary" in results:
                ...
                if summary["success_rate"] < 100:
                    recommendations.append(f"Check {category} - {100-summary['success_rate']:.1f}% failure rate")
                
                if summary["avg_response_time"] > 0.01:  # 10ms threshold
                    recommendations.append(f"{category} response time high: {summary['avg_response_time']:.3f}s")
                
                if summary["avg_throughput"] < 100:  # 100 ops/sec threshold
                    recommendations.append(f"{category} throughput could be improved: {summary['avg_throughput']:.1f} ops/sec")
        ...
        if all_results and "async_operations" in all_results:
            ...
            if system_metrics:
                ...
                logger.info(f"   💾 Available Memory: {system_metrics.get('memory_available_mb', 'N/A'):.0f}MB")
