...
class ExternalContainer(containers.DeclarativeContainer):
    ...
    health_server = providers.Singleton(
        HealthCheckServer,
        settings=settings
    )
    ...
    tracer = providers.Singleton(
        MTProtoTracer,
        enabled=settings.provided.enable_tracing.as_(bool)
    )
    ...
    fault_injector = providers.Singleton(
        lambda: get_global_injector()
    )
    
    # Global tracer initialization
    global_tracer = providers.Resource(
