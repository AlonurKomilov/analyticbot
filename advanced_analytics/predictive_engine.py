...
class PredictiveAnalyticsEngine:
        ...
        try:
            ...
            if method == 'auto' or method == 'prophet':
                ...
                if PROPHET_AVAILABLE:
                    try:
                        prophet_result = await self._forecast_prophet(ts_df.reset_index(), date_column, value_column, periods)
    ...
    async def cluster_analysis(
        ...
        try:
            ...
            logger.info(f"Clustering complete. Method: {best_method}, Silhouette Score: {best_result['silhouette_score']:.3f}")
            return best_result
    ...
    async def _forecast_prophet(
        ...
        validation_error = mean_absolute_error(
            val_df['y'], 
            val_forecast.tail(val_size)['yhat']
        )
...
if __name__ == "__main__":
    ...
    y_reg = (X_reg['feature1'] * 2 + X_reg['feature2'] * 1.5 + 
    ...
    ts_values = 100 + np.cumsum(np.random.normal(0, 2, 365)) + 10 * np.sin(np.arange(365) * 2 * np.pi / 30)
    ...
    async def test_engine():
        ...
        print(f"Best regression model: {reg_results['best_model']} (R²: {reg_results['best_model_score']:.3f})")
        ...
        # Test clustering
        cluster_results = await engine.cluster_analysis(reg_df.drop('target', axis=1), ['feature1', 'feature2'])
        print(f"Clustering method: {cluster_results['method_used']} (Silhouette: {cluster_results['silhouette_score']:.3f})")
