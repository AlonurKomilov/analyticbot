...
class ScheduledPostFactory(factory.Factory):
    ...
    media_url = factory.Maybe("media_type", yes_declaration=Faker("url"), no_declaration=None)
...
class PaymentFactory(factory.Factory):
    ...
    metadata = factory.LazyFunction(
        lambda: {"user_agent": "Test Browser", "ip_address": "127.0.0.1"}
    )
    created_at = Faker("past_datetime", start_date="-30d")
...
class AnalyticsDataFactory(factory.Factory):
    ...
    engagement_rate = factory.LazyAttribute(
        lambda obj: (obj.clicks + obj.shares) / max(obj.views, 1)
    )
...
class RelatedDataFactory:
    ...
    def create_channel_with_posts(post_count: int = 5) -> dict[str, Any]:
        ...
        posts = [
            ScheduledPostFactory(channel_id=channel["id"], user_id=user["id"])
            for _ in range(post_count)
        ]

        return {"user": user, "channel": channel, "posts": posts}
    ...
    def create_payment_flow_data() -> dict[str, Any]:
        ...
        return {"user": user, "payment": payment, "webhook": webhook}
    ...
    def create_analytics_scenario() -> dict[str, Any]:
        ...
        high_performance_post = ScheduledPostFactory(
            channel_id=channel["id"], user_id=user["id"], views=5000, status="sent"
        )

        low_performance_post = ScheduledPostFactory(
            channel_id=channel["id"], user_id=user["id"], views=100, status="sent"
        )
