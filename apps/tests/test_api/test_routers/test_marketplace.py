...
class TestMarketplaceItems:
    ...
    async def test_search_items(self, marketplace_repo):
        ...
        assert any("dark" in i["name"].lower() or (i["description"] and "dark" in i["description"].lower()) for i in results)
...
class TestMarketplaceAPI:
    ...
    async def test_get_items_with_filters(self, api_client: AsyncClient):
        ...
        response = await api_client.get("/marketplace/items", params={
            "category": "themes",
            "is_featured": True,
            "limit": 5,
        })
