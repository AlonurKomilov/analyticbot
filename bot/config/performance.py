...
class PerformanceConfig(BaseSettings):
    ...
    HTTP_CONNECTOR_LIMIT_PER_HOST: int = Field(
        default=30, description="HTTP limit per host"
    )
    ...
    CACHE_MAX_CONNECTIONS: int = Field(
        default=100, description="Maximum cache connections"
    )

    # Async Settings
    ASYNCIO_TASK_LIMIT: int = Field(
        default=1000, description="Maximum concurrent asyncio tasks"
    )
    ASYNCIO_SEMAPHORE_LIMIT: int = Field(
        default=50, description="Asyncio semaphore limit"
    )
    WORKER_CONCURRENCY: int = Field(default=10, description="Worker concurrency level")

    # Memory Management
    MEMORY_THRESHOLD_WARNING: float = Field(
        default=0.8, description="Memory warning threshold"
    )
    MEMORY_THRESHOLD_CRITICAL: float = Field(
        default=0.95, description="Memory critical threshold"
    )
    GC_THRESHOLD_0: int = Field(
        default=700, description="Garbage collection threshold 0"
    )
    GC_THRESHOLD_1: int = Field(
        default=10, description="Garbage collection threshold 1"
    )
    GC_THRESHOLD_2: int = Field(
        default=10, description="Garbage collection threshold 2"
    )
    ...
    PERFORMANCE_SAMPLING_RATE: float = Field(
        default=0.1, description="Performance sampling rate"
    )
...
def get_performance_config(environment: str | None = None) -> PerformanceConfig:
    """
