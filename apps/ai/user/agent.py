...
from apps.ai.shared.models import Action, ActionResult, ActionStatus, ActionType
...
class UserAIAgent:
    ...
    def __init__(
        self,
        config: UserAIConfig,
        providers_repo: Optional[UserAIProvidersRepository] = None,
        usage_repo: Optional[UserAIUsageRepository] = None,
    ):
        self.config = config
        ...
        logger.info(f"🤖 User AI Agent initialized for user {self.user_id} (tier: {config.tier.value})")
        ...
        if self.providers_repo:
            try:
                ...
                if user_provider:
                    ...
                    logger.info(
                        f"Using user's {provider_name} provider with model {model}"
                    )
    ...
    async def _track_ai_usage(
        ...
        if self.usage_repo:
            try:
                ...
                logger.info(
                    f"Tracked usage: {tokens_used} tokens, ${cost_usd:.4f} "
    ...
    async def analyze_channel(
        ...
        try:
            ...
            logger.info(f"📊 Analyzing channel {channel_id} with AI provider {provider.provider_info.name}")
    ...
    async def execute_marketplace_service(
        ...
        try:
            ...
            from apps.ai.user.marketplace import get_marketplace_registry, ServiceExecutionContext
