...
class IdempotencyGuard:
    ...
    def __init__(self, redis_url: Optional[str] = None, key_prefix: str = "idempotency"):
    ...
    async def is_duplicate(self, idempotency_key: str) -> tuple[bool, Optional[IdempotencyStatus]]:
        """
    ...
    async def mark_operation_start(self, idempotency_key: str, ttl_seconds: int = 300) -> bool:
        ...
        try:
            ...
            if result:
                logger.info(f"Idempotency operation started: key='{idempotency_key}', ttl={ttl_seconds}s")
                return True
            else:
    ...
    async def mark_operation_failed(
        ...
        try:
            await redis_client.set(key, status.model_dump_json(), ex=ttl_seconds)
            logger.warning(f"Idempotency operation failed: key='{idempotency_key}', error='{error}'")
