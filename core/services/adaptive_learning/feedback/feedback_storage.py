...
class FeedbackStorageService:
    ...
    def __init__(self, config: Optional[FeedbackStorageConfig] = None):
    ...
    async def get_storage_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'cached_feedback': sum(len(feedback_list) for feedback_list in self.feedback_cache.values()),
    ...
    async def store_feedback_batch(self, batch: FeedbackBatch) -> bool:
        ...
        try:
            ...
            logger.info(f"📦 Stored feedback batch {batch.batch_id}: {success_count}/{len(batch.feedback_list)} items")
        ...
        try:
            for user_id in list(self.feedback_cache.keys()):
                ...
                self.feedback_cache[user_id] = [
                    feedback for feedback in self.feedback_cache[user_id]
