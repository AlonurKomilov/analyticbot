...
class MTProtoPerformanceTest:
    ...
    async def test_concurrent_requests(self) -> None:
        ...
        self.results.append(metrics)
        logger.info(
            f"Concurrent requests test: {concurrency} requests, "
    ...
    async def test_graceful_shutdown(self) -> None:
        ...
        if shutdown_time <= self.slos["max_graceful_shutdown_time"]:
            ...
            metrics.errors.append(
                f"Shutdown took {shutdown_time:.1f}s, exceeds {self.slos['max_graceful_shutdown_time']}s SLO"
            )
    ...
    async def test_fault_resilience(self) -> None:
        ...
        self.results.append(metrics)
        logger.info(
            f"Fault resilience test: {metrics.success_rate:.1f}% success rate under fault injection"
        )
    ...
    async def analyze_results(self) -> dict[str, Any]:
        ...
        for metrics in self.results:
            ...
            if metrics.success_rate < self.slos["min_success_rate"]:
                compliance["success_rate"] = (
                    f"FAIL: {metrics.success_rate:.1f}% < {self.slos['min_success_rate']}%"
                )
            ...
            if metrics.requests_per_second < self.slos["min_rps"]:
                compliance["rps"] = (
                    f"FAIL: {metrics.requests_per_second:.1f} < {self.slos['min_rps']}"
                )
                analysis["overall_status"] = "FAIL"
            else:
        ...
        analysis["summary"] = {
            ...
            "tests_passed": sum(
                1
                for test_name, compliance in analysis["slo_compliance"].items()
