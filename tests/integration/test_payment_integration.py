...
def mock_stripe_client():
    ...
    mock_client.confirm_payment_intent.return_value = MOCK_STRIPE_RESPONSES["payment_intent_success"]
    mock_client.retrieve_payment_intent.return_value = MOCK_STRIPE_RESPONSES["payment_intent_success"]
...
def mock_payme_client():
    ...
    mock_client.check_perform_transaction.return_value = MOCK_PAYME_RESPONSES["check_perform_success"]
    mock_client.create_transaction.return_value = MOCK_PAYME_RESPONSES["create_transaction_success"]
    mock_client.perform_transaction.return_value = MOCK_PAYME_RESPONSES["perform_transaction_success"]
...
class TestLocalPaymentProviders:
    ...
    async def test_payme_create_transaction(self, mock_payme_client):
        ...
        result = await mock_payme_client.create_transaction(
            id=transaction_id,
            amount=amount
        )
...
class TestPaymentWebhookProcessing:
    ...
    def test_payme_webhook_signature_validation(self):
        ...
        signature = hashlib.md5(
            (signature_string + webhook_key).encode()
        ).hexdigest()
...
class TestPaymentErrorHandling:
    ...
    async def test_stripe_card_declined_handling(self, mock_stripe_client):
        ...
        mock_stripe_client.confirm_payment_intent.return_value = MOCK_STRIPE_RESPONSES["payment_intent_failed"]
    ...
    async def test_network_timeout_handling(self, mock_stripe_client):
        ...
        mock_stripe_client.create_payment_intent.side_effect = httpx.TimeoutException("Payment request timed out")
        
        with pytest.raises(httpx.TimeoutException):
            await mock_stripe_client.create_payment_intent(
                amount=5000,
                currency="usd"
            )
