    ...
    async def store_raw_stats(
        ...
        async with self.pool.acquire() as conn:
            await conn.execute(
                ...
                VALUES ($1, $2, $3, $4)
                """,
                channel_id, key, json.dumps(data), fetched_at
            )
            
            return {"stored": True, "channel_id": channel_id, "key": key, "fetched_at": fetched_at}

    async def get_latest_stats(self, channel_id: int, key: str) -> Optional[dict[str, Any]]:
    ...
    async def get_all_keys(self, channel_id: int) -> list[str]:
        ...
        async with self.pool.acquire() as conn:
            records = await conn.fetch(
                "SELECT DISTINCT key FROM stats_raw WHERE channel_id = $1",
                channel_id
            )
    ...
    async def cleanup_old_stats(self, days_to_keep: int = 90) -> int:
        ...
        async with self.pool.acquire() as conn:
            result = await conn.execute(
                "DELETE FROM stats_raw WHERE fetched_at < $1",
                cutoff_date
            )
