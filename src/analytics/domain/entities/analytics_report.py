...
from ..value_objects.analytics_value_objects import AnalyticsMetric
...
class AnalyticsReport(AggregateRoot[ReportId]):
    ...
    def __init__(
        ...
        total_posts: int = 0,
        metrics: dict[str, AnalyticsMetric] | None = None,
        insights: list[ReportInsight] | None = None,
        channel_summaries: list[dict[str, Any]] | None = None,
        top_performing_posts: list[dict[str, Any]] | None = None,
        previous_period_metrics: dict[str, float] | None = None,
    ...
    def _generate_default_title(
        cls, report_type: ReportType, period_start: datetime, period_end: datetime
    ) -> str:
    ...
    def complete_generation(self, generation_duration: float | None = None) -> None:
        """
    ...
    def add_channel_data(self, channel_id: str, channel_data: dict[str, Any]) -> None:
        ...
        if self.previous_period_metrics:
            for metric_name, metric in self.metrics.items():
                if metric_name in self.previous_period_metrics:
                    ...
                    change_pct = (
                        ((metric.value - previous_value) / previous_value) * 100
                        if previous_value > 0
                        else 0
                    )
    ...
    def get_metric(self, metric_name: str) -> AnalyticsMetric | None:
