...
class MarketplaceRepository:
        ...
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                ...
                await conn.execute("""
                    ...
                    VALUES ($1, $2, 'spend', $3)
                """, user_id, -price, f"Marketplace purchase: item #{item_id}")
                ...
                purchase_id = await conn.fetchval("""
                    ...
                    VALUES ($1, $2, $3, $4)
                    RETURNING id
                """, user_id, item_id, price, expires_at)
    ...
    async def add_review(
        ...
        async with self.pool.acquire() as conn:
            ...
            """, user_id, item_id, rating, review_text, is_verified)
        ...
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                ...
                gift_id = await conn.fetchval("""
                    ...
                    VALUES ($1, $2, $3, $4)
                    RETURNING id
                """, sender_id, recipient['id'], amount, message)
                
                # Record transactions
                await conn.execute("""
                    INSERT INTO credit_transactions (user_id, amount, transaction_type, description)
                    VALUES ($1, $2, 'gift_sent', $3)
                """, sender_id, -amount, f"Gift to @{recipient_username}")
                
                await conn.execute("""
                    INSERT INTO credit_transactions (user_id, amount, transaction_type, description)
                    VALUES ($1, $2, 'gift_received', $3)
                """, recipient['id'], amount, f"Gift from user #{sender_id}")
        ...
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                ...
                await conn.execute("""
                    UPDATE users SET credit_balance = credit_balance - $1
                    WHERE id = $2
                """, bundle['price_credits'], user_id)
                
                # Record transaction
                await conn.execute("""
                    INSERT INTO credit_transactions (user_id, amount, transaction_type, description)
                    VALUES ($1, $2, 'spend', $3)
                """, user_id, -bundle['price_credits'], f"Bundle purchase: {bundle['name']}")
                ...
                user_bundle_id = await conn.fetchval("""
                    ...
                    VALUES ($1, $2, $3, $4, $5)
                    RETURNING id
                """, user_id, bundle_id, bundle['price_credits'], expires_at, json.dumps(remaining_uses))
