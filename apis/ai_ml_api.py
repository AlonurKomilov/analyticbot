...
class PerformanceReportRequest(BaseModel):
    ...
    include_churn_analysis: bool = Field(
        True, description="Include churn risk analysis"
    )
...
class RealTimeInsightsRequest(BaseModel):
    ...
    lookback_hours: int = Field(
        24, description="Lookback period in hours", ge=1, le=168
    )
...
async def health_check(services: dict = Depends(get_ml_services)):
    ...
    try:
        ...
        # Overall health
        all_healthy = all(
            health.get("status") == "healthy" for health in service_health.values()
        )
...
async def analyze_churn_risk(
    request: ChurnRiskRequest, services: dict = Depends(get_ml_services)
):
    """
...
async def generate_performance_report(
    ...
    try:
        logger.info(
            f"📊 Generating performance report for channel {request.channel_id}"
        )
        ...
        return {
            ...
            "summary": {
                "critical_insights": len(
                    [i for i in insights if i.impact_level == "critical"]
                ),
...
async def optimize_content_real_time(
    text: str, services: dict = Depends(get_ml_services)
):
        ...
        return {
            ...
            "recommendations": (
                await content_optimizer._generate_optimization_tips(
                    ...
                    scores.get("sentiment_score", 0.5) * 2
...
if __name__ == "__main__":
    uvicorn.run(
        "ai_ml_api:app", host="0.0.0.0", port=8002, reload=True, log_level="info"
    )
