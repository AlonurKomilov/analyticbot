...
class AsyncpgChannelDailyRepository:
    ...
    async def series_value(self, channel_id: int, metric: str, target_date: datetime) -> Optional[int]:
        ...
        async with self.pool.acquire() as conn:
            result = await conn.fetchval(
                ...
                channel_id, metric, target_day
            )
            return result

    async def series_data(self, channel_id: int, metric: str, from_dt: datetime, to_dt: datetime) -> list[dict[str, Any]]:
        ...
        async with self.pool.acquire() as conn:
            records = await conn.fetch(
                ...
                ORDER BY day ASC
                """,
                channel_id, metric, from_day, to_day
            )
            return [{"day": record["day"], "value": record["value"]} for record in records]

    async def upsert_metric(self, channel_id: int, target_date: datetime, metric: str, value: int) -> None:
        ...
        async with self.pool.acquire() as conn:
            await conn.execute(
                ...
                """,
                channel_id, target_day, metric, value
            )

    async def get_latest_metric(self, channel_id: int, metric: str) -> Optional[dict[str, Any]]:
    ...
    async def get_metrics_for_day(self, channel_id: int, target_date: datetime) -> list[dict[str, Any]]:
    ...
    async def delete_old_metrics(self, days_to_keep: int = 365) -> int:
        ...
        async with self.pool.acquire() as conn:
            result = await conn.execute(
                "DELETE FROM channel_daily WHERE day < $1",
                cutoff_date
            )
