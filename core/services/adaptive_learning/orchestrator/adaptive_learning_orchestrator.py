...
from .workflow_models import OrchestrationStrategy, OrchestrationConfig, OrchestrationMetrics
...
class AdaptiveLearningOrchestrator:
    ...
    async def trigger_adaptive_learning(
        ...
        try:
            ...
            if workflow_id:
                ...
                logger.info(f"🚀 Triggered adaptive learning workflow {workflow_id} for model {model_id}")
                return workflow_id
    ...
    async def get_orchestration_metrics(self) -> Dict[str, Any]:
        ...
        try:
            # Update metrics
            self.orchestration_metrics.active_workflows = len(self.workflow_manager.active_workflows)
    ...
    async def _execute_workflow(self, workflow) -> None:
        ...
        try:
            ...
            while current_stage not in [workflow.current_stage.COMPLETED, workflow.current_stage.FAILED]:
                ...
                if not stage_result.get('success', False):
                    ...
                    logger.error(f"❌ Stage {current_stage.value} failed for workflow {workflow_id}: {error_msg}")
                    
                    # Check if rollback is needed
                    if stage_result.get('needs_rollback', False):
                        rollback_result = await self.stage_executor.execute_stage(workflow, workflow.current_stage.ROLLBACK)
