...
class AdminCommandHandler:
    ...
    async def handle_unwarn(self, message: Message) -> None:
        ...
        cleared = await self.service.clear_user_warnings(self.user_id, chat_id, target_id)

        await message.answer(f"✅ Cleared {cleared} warning(s) for {target_name}")
    ...
    async def handle_warnings(self, message: Message) -> None:
        ...
        warnings = await self.service.get_user_warnings(self.user_id, chat_id, target_id)
    ...
    async def handle_mute(self, message: Message, command: CommandObject) -> None:
        ...
        await message.answer(f"🔇 {target_name} has been muted for {duration} minutes.")
    ...
    async def handle_modlog(self, message: Message) -> None:
        ...
        chat_id = message.chat.id
        logs = await self.service.get_moderation_log(self.user_id, chat_id, limit=20)
