...
class CNNFeatureExtractor(nn.Module):
    ...
    def __init__(
        ...
        for i, (out_channels, kernel_size, pool_size) in enumerate(
            zip(self.feature_channels, self.kernel_sizes, self.pool_sizes)
        ):
            # Convolution layer
            conv = nn.Conv1d(
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        for i, (conv, norm, pool) in enumerate(zip(self.conv_layers, self.norm_layers, self.pool_layers)):
            x = conv(x)
...
class CNNTransformerModel(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: Optional[torch.Tensor] = None) -> Dict[str, torch.Tensor]:
        ...
        projected_features = self.feature_projection(cnn_features)  # (batch_size, reduced_seq_len, embed_dim)
        ...
        if attention_mask is not None:
            ...
            pooled = (transformer_output * mask_expanded).sum(dim=1) / mask_expanded.sum(dim=1).clamp(min=1e-8)
        else:
    ...
    def predict_with_confidence(
        ...
        for task_name, predictions_list in all_predictions.items():
            stacked_preds = torch.stack(predictions_list, dim=0)  # (mc_samples, batch_size, num_classes)
    ...
    def analyze_content_features(
        ...
        with torch.no_grad():
            ...
            analysis['risk_assessment'] = {
                'toxicity_risk': 'high' if toxicity_prob.mean() > 0.7 else 'medium' if toxicity_prob.mean() > 0.3 else 'low',
                'quality_level': 'high' if quality_score.float().mean() >= 4 else 'medium' if quality_score.float().mean() >= 3 else 'low',
