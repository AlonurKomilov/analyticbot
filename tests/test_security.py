        ...
        class PaymentRateLimiter:
            ...
            def can_attempt_payment(self, user_id: int) -> bool:
                ...
                self.payment_attempts[user_id] = [
                    attempt_time
                    for attempt_time in self.payment_attempts[user_id]
...
class TestWebhookSecurity:
    ...
    def test_webhook_signature_validation(self):
        ...
        valid_signature = (
            "sha256="
            + hmac.new(webhook_secret.encode(), valid_payload.encode(), hashlib.sha256).hexdigest()
        )
        ...
        assert (
            validate_webhook_signature(valid_payload, "invalid_signature", webhook_secret) is False
        )
        assert (
            validate_webhook_signature(valid_payload, "sha256=wrong_hash", webhook_secret) is False
        )
