...
class MTProtoMetrics:
    ...
    def __init__(self, enabled: bool = False, registry: Any | None = None):
        ...
        if not self.enabled:
            logger.warning(
                "Prometheus metrics disabled (prometheus_client not available or disabled)"
            )
            return
        ...
        self.pool_accounts_total = Gauge(
            "mtproto_pool_accounts_total", "Total accounts in pool", registry=registry
        )
        ...
        self.proxy_pool_total = Gauge(
            "mtproto_proxy_pool_total", "Total proxies in pool", registry=registry
        )

        self.proxy_pool_healthy = Gauge(
            "mtproto_proxy_pool_healthy", "Healthy proxies in pool", registry=registry
        )
    ...
    def record_db_operation(
        self, operation: str, table: str, status: str, duration: float | None = None
    ) -> None:
    ...
    async def time_channel_sync(self, channel: str, sync_type: str):
        ...
        try:
            ...
            duration = time.time() - start_time
            self.channel_sync_duration.labels(channel=channel, sync_type=sync_type).observe(
                duration
            )
...
_metrics: MTProtoMetrics | None = None
...
def initialize_metrics(
    enabled: bool = False, port: int = 9108, registry: Any | None = None
) -> MTProtoMetrics:
