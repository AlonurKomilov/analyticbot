...
class Price:
    ...
    """
    current: Credits
    original: Optional[Credits] = None
...
class UsageQuota:
    ...
    def daily_remaining(self, current_usage: int) -> Optional[int]:
        """Get remaining daily usage"""
        if not self.daily:
            return None
        return max(0, self.daily - current_usage)
    
    def monthly_remaining(self, current_usage: int) -> Optional[int]:
