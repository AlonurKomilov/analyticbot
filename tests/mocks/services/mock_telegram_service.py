...
Provides mock implementations of telegram operations for development and testing.
Simulates realistic telegram bot interactions without requiring actual API calls.
"""
...
from tests.mocks.constants import DEFAULT_MOCK_CHANNEL_NUMERIC_ID, MOCK_POSTS_COUNT

# Demo constants
DEFAULT_DEMO_CHANNEL_ID = DEFAULT_MOCK_CHANNEL_NUMERIC_ID
DEMO_POSTS_COUNT = MOCK_POSTS_COUNT

import asyncio
import uuid

from core.protocols import TelegramAPIServiceProtocol
from tests.mocks.constants import DEMO_API_DELAY_MS, MOCK_POSTS_COUNT
...
class MockTelegramService(TelegramAPIServiceProtocol):
    ...
    async def get_channel_posts(self, channel_id: str, limit: int = 100) -> list[dict[str, Any]]:
        ...
        channel_posts = [post for post in self.mock_posts if post["channel_id"] == channel_id][
            :limit
        ]

        if not channel_posts:
            # Generate posts if none exist
            channel_posts = self._generate_posts_for_channel(
                channel_id, min(limit, DEMO_POSTS_COUNT)
            )
    ...
    def _generate_mock_channels(self) -> dict[str, dict[str, Any]]:
        ...
        channels[DEFAULT_DEMO_CHANNEL_ID] = self._generate_channel_info(
            str(DEFAULT_DEMO_CHANNEL_ID)
        )
    ...
    def _generate_posts_for_channel(self, channel_id: str, count: int) -> list[dict[str, Any]]:
        ...
        for i in range(count):
            ...
            post = {
                ...
                "date": int(
                    (datetime.utcnow() - timedelta(hours=random.randint(1, 720))).timestamp()
                ),
