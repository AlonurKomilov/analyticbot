...
def get_chart_renderer() -> ChartRenderer:
    ...
    if not MATPLOTLIB_AVAILABLE:
        raise HTTPException(
            status_code=503,
            detail="PNG chart rendering not available"
        )
...
def check_share_enabled():
    ...
    if not settings.SHARE_LINKS_ENABLED:
        raise HTTPException(
            status_code=403,
            detail="Share functionality is disabled"
        )
...
async def create_share_link(
    ...
    try:
        ...
        if not data:
            raise HTTPException(
                status_code=404, 
                detail="No analytics data available for sharing"
            )
...
async def access_shared_report(
    ...
    try:
        ...
        if not data:
            raise HTTPException(
                status_code=404, 
                detail="Analytics data no longer available"
            )
        ...
        if format == 'csv':
            ...
            if not chart_renderer:
                raise HTTPException(
                    status_code=503,
                    detail="PNG rendering not available"
                )
