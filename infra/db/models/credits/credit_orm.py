...
class UserCreditsORM(Base):
    ...
    lifetime_earned: Mapped[Decimal] = mapped_column(Numeric(12, 2), server_default="0", nullable=False)
    lifetime_spent: Mapped[Decimal] = mapped_column(Numeric(12, 2), server_default="0", nullable=False)
    ...
    __table_args__ = (
        Index("ix_user_credits_user_id", "user_id", unique=True),
    )
...
class CreditPackageORM(Base):
    ...
    bonus_credits: Mapped[Decimal] = mapped_column(Numeric(12, 2), server_default="0", nullable=False)
...
class AchievementORM(Base):
    ...
    # Reward
    credit_reward: Mapped[Decimal] = mapped_column(Numeric(10, 2), server_default="0", nullable=False)
    
    # Categorization
    category: Mapped[str | None] = mapped_column(String(50), nullable=True)  # engagement, milestone, etc.
    
    # Requirements
    requirement_type: Mapped[str | None] = mapped_column(String(50), nullable=True)  # action, count, date
...
class UserReferralORM(Base):
    ...
    # Reward status
    reward_paid_referrer: Mapped[bool] = mapped_column(Boolean, server_default="false", nullable=False)
    reward_paid_referred: Mapped[bool] = mapped_column(Boolean, server_default="false", nullable=False)
