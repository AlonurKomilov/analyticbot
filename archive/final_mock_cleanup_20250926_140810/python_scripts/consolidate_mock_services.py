...
def analyze_imports() -> dict[str, list[str]]:
    ...
    for py_file in src_path.rglob("*.py"):
        try:
            ...
            if (
                "from src.api_service.infrastructure.testing.services" in content
                and "mock" in content
            ):
...
def generate_migration_plan():
    ...
    print(
        "find src/ -name '*.py' -exec sed -i 's|from src.api_service.__mocks__|from src.mock_services|g' {} \\;"
    )

    print("\n# Fix existing __mocks__ imports")
    print(
        "find src/ -name '*.py' -exec sed -i 's|from src.api_service.application.services.__mocks__|from src.mock_services|g' {} \\;"
    )

    print("\n# Fix testing services imports")
    print(
        "find src/ -name '*.py' -exec sed -i 's|from src.api_service.infrastructure.testing.services|from src.mock_services.services|g' {} \\;"
    )
