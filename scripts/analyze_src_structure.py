...
def analyze_src_structure():
    ...
    for item in sorted(os.listdir("src")):
        ...
        if module_path.is_dir() and not item.startswith("_"):
            ...
            module_info = {"layers": [], "files": [], "submodules": {}}
    ...
    if "infrastructure" not in shared_layers:
        recommendations.append(
            "Create src/shared_kernel/infrastructure/ for DI container and common infrastructure"
        )

    if "domain" not in shared_layers:
        recommendations.append(
            "Create src/shared_kernel/domain/ for service protocols and shared entities"
        )
    ...
    for module_name, info in modules.items():
        if module_name != "shared_kernel" and module_name != "migration_adapters":
            ...
            if missing_layers:
                recommendations.append(
                    f"Module {module_name} missing layers: {', '.join(missing_layers)}"
                )
