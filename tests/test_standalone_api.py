...
async def run_api_tests():
    ...
    async with APITester() as tester:
        ...
        success, response, time_ms = await tester.test_endpoint("POST", "/analyze/content", content_data)
        test_results["total_tests"] += 1
        if success:
            test_results["passed_tests"] += 1
            print(f"   ✅ Overall Score: {response.get('overall_score', 'N/A'):.3f}")
            print(f"   ✅ Sentiment: {response.get('sentiment_label', 'N/A')} ({response.get('sentiment_score', 0):.3f})")
        ...
        realtime_data = {
            "text": "Great product! Love the new features 😍 #amazing #product #love"
        }
        
        success, response, time_ms = await tester.test_endpoint("POST", "/score/realtime", realtime_data)
        test_results["total_tests"] += 1
        if success:
            test_results["passed_tests"] += 1
            print(f"   ✅ Overall Score: {response.get('overall_score', 'N/A'):.3f}")
