...
class CodeFixer:
    ...
    def create_ai_prompt(
        self, context_file: str | None = None, issues: dict | None = None
    ) -> str:
...
def main():
    ...
    parser.add_argument(
        "--context", type=str, help="Context file with additional information"
    )
    parser.add_argument(
        "--fix-type",
        choices=[
    ...
    try:
        ...
        elif fixer.client is None and any(
            count > 0 for count in result["issues_found"].values()
        ):
...
def main():
    ...
    ctx = open(args.context, encoding="utf-8", errors="ignore").read()
    prompt = (
        USER_INTRO
        + "\n\n=== CONTEXT START ===\n"
        + ctx[:180000]
