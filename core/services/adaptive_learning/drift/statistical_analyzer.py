...
class StatisticalDriftAnalyzer:
    ...
    async def anderson_darling_test(
        ...
        try:
            ...
            ad_statistic, critical_values, significance_level = stats.anderson_ksamp([reference_sample, current_sample])
    ...
    async def _run_all_tests(
        ...
        try:
            ...
            for test_name in ['kolmogorov_smirnov', 'mann_whitney', 'chi_square', 'anderson_darling']:
                tests[test_name] = self._create_failed_test_result(test_name)
    ...
    def _calculate_distribution_metrics(
        ...
        try:
            metrics = {
                ...
                'quantile_25_shift': float(abs(np.percentile(current_sample, 25) - np.percentile(reference_sample, 25))),
                'quantile_75_shift': float(abs(np.percentile(current_sample, 75) - np.percentile(reference_sample, 75))),
                'skewness_shift': float(abs(stats.skew(current_sample) - stats.skew(reference_sample))),
                'kurtosis_shift': float(abs(stats.kurtosis(current_sample) - stats.kurtosis(reference_sample)))
            }
        ...
        try:
            ...
            for test_name, result in test_results.items():
                if test_name in test_weights:
                    ...
                    drift_contribution = min(1.0, 1.0 - result.p_value) if result.p_value < 1.0 else 0.0
