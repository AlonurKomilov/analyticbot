...
type SpacingScale = keyof typeof SPACING_SCALE;
...
export const spacingUtils = {
   ...
   */
  space: (scale: SpacingScale) => SPACING_SCALE[scale],

  /**
   * Get semantic spacing
   */
  semantic: (category: string, property: string): number | undefined => {
    const cat = (SEMANTIC_SPACING as any)[category];
    return cat?.[property];
  },
   ...
   */
  responsive: (spacingConfig: number | ResponsiveSpacing) => {
    ...
    return {
      xs: spacingConfig.xs || 0,
      sm: spacingConfig.sm || spacingConfig.xs || 0,
      md: spacingConfig.md || spacingConfig.sm || spacingConfig.xs || 0,
      lg: spacingConfig.lg || spacingConfig.md || spacingConfig.sm || spacingConfig.xs || 0,
    };
  },
  ...
  patterns: {
    ...
    stack: (gap: SpacingScale = 'lg') => ({
      display: 'flex',
      flexDirection: 'column',
      gap: SPACING_SCALE[gap]
    }),

    // Horizontal row with gaps
    row: (gap: SpacingScale = 'lg') => ({
      display: 'flex',
      gap: SPACING_SCALE[gap],
      alignItems: 'center'
    }),
 ...
 */
export const MIGRATION_MAP: Record<string, number> = {
