...
class ContentOptimizer:
    ...
    async def optimize_hashtags(
        ...
        try:
            ...
            for topic in topics[:3]:  # Top 3 topics
                topic_hashtags = await self._generate_topic_hashtags(
            ...
            suggestions.sort(
                key=lambda x: x.relevance_score * x.popularity_score, reverse=True
            )
    ...
    async def score_content_realtime(self, text: str) -> dict[str, float]:
        ...
        try:
            ...
            positive_count = sum(
                1 for word in text.lower().split() if word in self.positive_words
            )
            negative_count = sum(
                1 for word in text.lower().split() if word in self.negative_words
            )
    ...
    async def _predict_engagement(
        ...
        if target_audience == "business":
            ...
            if any(
                word in text.lower() for word in ["inspiration", "motivation", "life"]
            ):
                engagement_score += 5
    ...
    async def _generate_optimization_tips(
        ...
        if word_count < 20:
            tips.append(
                "📝 Add more content - aim for 50-200 words for optimal engagement"
            )
        elif word_count > 300:
            tips.append(
                "✂️ Consider shortening content - posts under 200 words perform better"
            )
        ...
        if hashtag_count == 0:
            ...
            tips.append(
                f"📱 Add {3 - hashtag_count} more hashtags (currently {hashtag_count})"
            )
        ...
        if not has_cta:
            tips.append(
                "💬 Add a call-to-action (like, share, comment) to boost engagement"
            )
