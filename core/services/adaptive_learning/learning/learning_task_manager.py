...
from .task_creator import TaskCreator, TaskConfig
...
class LearningTaskManager:
    ...
    def __init__(
        self,
        config: Optional[LearningTaskManagerConfig] = None,
        task_creator_config: Optional[TaskConfig] = None,
    ...
    async def create_learning_task(
        ...
        try:
            ...
            task_id = await self.task_creator.create_task(
                model_id=model_id,
                strategy=strategy,
            ...
            if task_id:
                ...
                if scheduled:
                    # Start tracking
                    await self.progress_tracker.start_task_tracking(task_id)
                    self.manager_stats['successful_operations'] += 1
                    logger.info(f"✅ Created and scheduled learning task {task_id}")
                    return task_id
                else:
    ...
    async def cancel_task(self, task_id: str, reason: str = "") -> bool:
        ...
        try:
            ...
            # Cancel in scheduler
            scheduler_cancelled = await self.task_scheduler.cancel_task(task_id, reason)
    ...
    async def register_worker(self, worker_id: str, capabilities: Dict[str, Any]) -> bool:
        ...
        try:
            return await self.task_scheduler.register_worker(worker_id, capabilities)
    ...
    async def worker_heartbeat(self, worker_id: str, status: Dict[str, Any]) -> bool:
        ...
        try:
            return await self.task_scheduler.worker_heartbeat(worker_id, status)
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Overall health
            all_healthy = (
                creator_healthy.get('status') == 'healthy' and
                scheduler_healthy.get('status') == 'healthy' and
                'error' not in tracker_healthy
            )
            ...
            health_status = {
                ...
                'components': {
                    ...
                    'progress_tracker': {'status': 'healthy' if 'error' not in tracker_healthy else 'unhealthy'}
                },
