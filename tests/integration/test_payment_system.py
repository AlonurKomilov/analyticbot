...
class MockPaymentRepository:
    ...
    async def create_subscription(
        self, user_id: int, plan_id: int, billing_cycle: str, amount: Decimal, **kwargs
    ) -> str:
...
class MockPaymentAdapter:
    ...
    async def charge_payment_method(
        self, method_id: str, amount: Decimal, currency: str, **kwargs
    ) -> dict[str, Any]:
...
async def test_payment_system():
    ...
    for plan in MOCK_PLANS:
        ...
        print(
            f"  • {plan['name']}: ${plan['price_monthly']}/month, ${plan['price_yearly']}/year (Save: ${monthly_savings:.2f})"
        )
    ...
    for provider, adapter in adapters.items():
        ...
        result = await adapter.charge_payment_method("pm_test_001", amount, currency)
    ...
    for event in webhook_events:
        ...
        verified = adapter.verify_webhook_signature(
            b"test_payload", "test_signature", "test_secret"
        )
    ...
    total_revenue = sum(
        p["amount"] for p in repository.payments.values() if p["status"] == "succeeded"
    )
