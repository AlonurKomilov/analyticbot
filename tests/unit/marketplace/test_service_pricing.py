...
EXPECTED_PRICING = {
    ...
    'mtproto_bulk_export': ServicePricing(
        'mtproto_bulk_export', 200, category='mtproto_services'
    ),
...
class TestServiceQuotas:
    ...
    def test_ai_services_have_quotas(self):
        ...
        for key, pricing in ai_services.items():
            ...
            assert pricing.daily_quota > 0, \
                f"AI service {key} should have positive daily_quota"
    ...
    def test_bot_services_no_quotas(self):
        ...
        for key, pricing in bot_services.items():
            assert pricing.daily_quota is None, \
                f"Bot service {key} should not have daily_quota"
    ...
    def test_mtproto_services_no_quotas(self):
        ...
        for key, pricing in mtproto_services.items():
            assert pricing.daily_quota is None, \
                f"MTProto service {key} should not have daily_quota"
    ...
    def test_quota_order_makes_sense(self):
        ...
        assert EXPECTED_PRICING['ai_content_moderation'].daily_quota >= \
            EXPECTED_PRICING['ai_sentiment_analyzer'].daily_quota
        
        # Smart replies has higher quota than content optimizer
        assert EXPECTED_PRICING['ai_smart_replies'].daily_quota > \
