...
async def migrate_configs(dry_run: bool = False):
    ...
    async with async_session() as session:
        ...
        for service_key, config_data in redis_configs.items():
            ...
            try:
                if service_key in existing_keys:
                    ...
                    if updated:
                        logger.info(
                            f"✅ Updated: {service_key} = {config_data['limit']}/{config_data['period']}"
                        )
                        updated_count += 1

                        # Log to audit trail
                        old_config = next(
                            (c for c in existing_configs if c.service_key == service_key),
                            None,
                        )
                    ...
                    if created:
                        logger.info(
                            f"✅ Created: {service_key} = {config_data['limit']}/{config_data['period']}"
                        )
                        migrated_count += 1
        ...
        if dry_run:
            ...
            logger.info(
                f"Would create: {len([k for k in redis_configs.keys() if k not in existing_keys])}"
            )
            logger.info(
                f"Would update: {len([k for k in redis_configs.keys() if k in existing_keys])}"
            )
        else:
