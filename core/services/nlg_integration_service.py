...
from .nlg import InsightType, NarrativeStyle, NaturalLanguageGenerationService
...
class NLGIntegrationService:
        ...
        try:
            ...
            if insights.get("audience_insights"):
                audience_narrative = await self._nlg_service.generate_insight_narrative(
                    insights["audience_insights"], InsightType.AUDIENCE, narrative_style
                )
    ...
    async def generate_multi_style_narratives(
        self, insights: dict, styles: list[NarrativeStyle] | None = None
    ) -> dict:
        """
    ...
    async def generate_narrative_summary(
        ...
        try:
            ...
            if summary_type == "executive":
                ...
                summary = await self._generate_custom_summary(
                    insights, summary_type, narrative_style
                )
    ...
    async def _generate_custom_summary(
        ...
        try:
            if summary_type == "performance":
                ...
                narrative_obj = await self._nlg_service.generate_insight_narrative(
                    performance_data, InsightType.PERFORMANCE, narrative_style
                )
                ...
                if recommendations:
                    ...
                    narrative_obj = await self._nlg_service.generate_insight_narrative(
                        rec_data,
                        InsightType.PERFORMANCE,  # Use PERFORMANCE instead of STRATEGIES
                        narrative_style,
                    )
                    return narrative_obj.narrative
                else:
                ...
                if patterns:
                    ...
                    narrative_obj = await self._nlg_service.generate_insight_narrative(
                        pattern_data,
                        InsightType.TREND,  # Use TREND instead of TRENDS
                        narrative_style,
                    )
                    return narrative_obj.narrative
                else:
    ...
    async def health_check(self) -> dict:
        ...
        return {
            ...
            "dependencies": {"nlg_service": nlg_status == "healthy"},
