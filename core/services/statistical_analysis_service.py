...
class StatisticalAnalysisService:
    ...
    async def calculate_statistical_significance(
        ...
        try:
            ...
            if metric_type == "views":
                ...
                baseline_data = await self._get_daily_views(channel_id, baseline_start, baseline_end)
            elif metric_type == "engagement":
                comparison_data = await self._get_daily_engagement(channel_id, comparison_start, now)
                baseline_data = await self._get_daily_engagement(channel_id, baseline_start, baseline_end)
            elif metric_type == "growth":
                comparison_data = await self._get_daily_growth(channel_id, comparison_start, now)
                baseline_data = await self._get_daily_growth(channel_id, baseline_start, baseline_end)
            else:
            ...
            statistical_tests = await self._perform_statistical_tests(comparison_array, baseline_array)
            ...
            significance_report = {
                ...
                "practical_significance": {
                    "percent_change": float(
                        (np.mean(comparison_array) - np.mean(baseline_array)) / np.mean(baseline_array) * 100
                    ) if np.mean(baseline_array) != 0 else 0,
    ...
    async def _perform_statistical_tests(self, comparison_data: np.ndarray, baseline_data: np.ndarray) -> dict:
        """
        ...
        try:
            ...
            tests["recommendation"] = {
                ...
                "reason": "Normal distribution" if recommended_test == "t_test" else "Non-normal distribution"
            }
    ...
    def _calculate_effect_size(self, comparison_data: np.ndarray, baseline_data: np.ndarray) -> dict:
        ...
        try:
            ...
            glass_delta = (np.mean(comparison_data) - np.mean(baseline_data)) / np.std(baseline_data)
    ...
    def _calculate_confidence_intervals(self, comparison_data: np.ndarray, baseline_data: np.ndarray, confidence_level: float) -> dict:
    ...
    async def _get_daily_views(self, channel_id: int, start_date: datetime, end_date: datetime) -> list[float]:
    ...
    async def _get_daily_growth(self, channel_id: int, start_date: datetime, end_date: datetime) -> list[float]:
        """Get daily growth rates for statistical analysis"""
        try:
            # Get follower/subscriber data
            follower_data = await self._daily.series_data(channel_id, "followers", start_date, end_date)
            if not follower_data:
                follower_data = await self._daily.series_data(channel_id, "subscribers", start_date, end_date)
    ...
    async def _get_daily_engagement(self, channel_id: int, start_date: datetime, end_date: datetime) -> list[float]:
