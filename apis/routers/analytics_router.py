...
async def create_channel(
    ...
    try:
        ...
        existing_channel = await channel_repo.get_channel_by_telegram_id(
...
async def get_demo_post_dynamics(hours: int = Query(24, ge=1, le=168)):  # Max 1 week
...
async def get_analytics_summary(
    ...
    try:
        ...
        logger.error(
            f"Error generating analytics summary for channel {channel_id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
