...
type EntranceAnimation = 'zoom' | 'slide' | 'fade';

interface InteractiveButtonProps extends Omit<ButtonProps, 'size' | 'color'> {
  children?: ReactNode;
  loading?: boolean;
  success?: boolean;
  error?: boolean;
...
interface InteractiveIconButtonProps extends Omit<IconButtonProps, 'size' | 'color'> {
  children?: ReactNode;
  size?: ButtonSize;
  color?: ButtonColor;
  disabled?: boolean;
...
interface AnimatedFabProps extends Omit<ButtonProps, 'size' | 'color'> {
  children?: ReactNode;
  onClick?: () => void;
  color?: ButtonColor;
  size?: ButtonSize;
...
export const InteractiveButton: React.FC<InteractiveButtonProps> = ({
  ...
  const getHoverStyles = (): Record<string, any> => {
    if (disabled || loading) return {};

    const hoverStyles: Record<HoverEffect, Record<string, any>> = {
      lift: {
  ...
  const ButtonComponent: any = ripple ? TouchRipple : Box;
...
export const AnimatedFab: React.FC<AnimatedFabProps> = ({
  ...
  const getEntranceAnimation = (): { timeout: number; direction?: 'up' | 'down' | 'left' | 'right' } => {
    const animations = {
      zoom: { timeout: 200 },
      slide: { timeout: 300, direction: 'up' as const },
      fade: { timeout: 400 }
    };
    return animations[entrance] || animations.zoom;
  };

  const AnimationComponent: any = entrance === 'zoom' ? Zoom : entrance === 'slide' ? Zoom : Fade;
