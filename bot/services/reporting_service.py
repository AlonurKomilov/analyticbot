...
class AutomatedReportingSystem:
            ...
            for section in template.sections:
                if section["type"] == "header":
                    ...
                    if isinstance(section["content"], pd.DataFrame):
                        ...
                        table_data = [
                            df_section.columns.tolist()
                        ] + df_section.values.tolist()
    ...
    async def _generate_excel_report(
        ...
        try:
            ...
            if len(numeric_data.columns) > 0:
                ...
                for row_num, (stat_name, row_data) in enumerate(
                    summary_stats.iterrows(), 2
                ):
                    summary_sheet.cell(row=row_num, column=1, value=stat_name)
                    for col_num, value in enumerate(row_data, 2):
                        summary_sheet.cell(
                            row=row_num, column=col_num, value=float(value)
                        )
    ...
    def _generate_data_summary(self, data: pd.DataFrame) -> str:
        ...
        try:
            ...
            if missing_total > 0:
                ...
                summary_parts.append(
                    f"Missing data: {missing_total:,} cells ({missing_pct:.1f}%)."
                )
    ...
    async def schedule_report(
        ...
        try:
            ...
            def generate_scheduled_report():
                ...
                try:
                    ...
                    if not isinstance(data, pd.DataFrame):
                        logger.error(
                            f"Data source for {schedule_name} did not return DataFrame"
                        )
                        return
                    ...
                    asyncio.run(
                        self.create_report(data, template, output_format, filename)
                    )
                    ...
                    logger.info(
                        f"Scheduled report {schedule_name} generated successfully"
                    )
            ...
            if (
                not hasattr(self, "_scheduler_thread")
            ...
            return {
                ...
                "next_run": (
                    str(schedule.jobs[-1].next_run) if schedule.jobs else "Unknown"
                ),
            }
    ...
    async def _send_report_email(
        self, recipients: list[str], subject: str, attachment_path: str
    ):
...
if __name__ == "__main__":
    ...
    async def test_reporting():
        ...
        html_result = await reporting_system.create_report(
            sample_data, template, "html"
        )
        print(f"HTML Report: {html_result.get('status', 'failed')}")

        json_result = await reporting_system.create_report(
            sample_data, template, "json"
        )
