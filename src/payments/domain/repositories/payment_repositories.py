...
class IPaymentRepository(ABC):
    ...
    @abstractmethod
    async def get_by_id(self, payment_id: PaymentId) -> Payment | None:
        """Get payment by ID"""
    ...
    async def find_failed_payments_for_retry(
        self, max_retry_count: int = 3, limit: int = 50
    ) -> list[Payment]:
        """Find failed payments eligible for retry"""

    @abstractmethod
    async def find_by_payment_method_id(
        self, payment_method_id: PaymentMethodId, limit: int = 50, offset: int = 0
    ) -> list[Payment]:
    ...
    async def get_revenue_by_period(
        self,
        period: str,
        periods_count: int = 12,  # 'day', 'week', 'month', 'year'
    ) -> list[dict[str, Any]]:
        """Get revenue breakdown by time period"""

    @abstractmethod
    async def get_top_customers_by_revenue(
        self, limit: int = 10, start_date: datetime | None = None
    ) -> list[dict[str, Any]]:
...
class IPaymentMethodRepository(ABC):
    ...
    @abstractmethod
    async def get_by_id(self, payment_method_id: PaymentMethodId) -> PaymentMethod | None:
    ...
    @abstractmethod
    async def find_by_customer_id(
        self, customer_id: CustomerId, active_only: bool = True
    ) -> list[PaymentMethod]:
        """Find payment methods by customer ID"""

    @abstractmethod
    async def get_default_for_customer(self, customer_id: CustomerId) -> PaymentMethod | None:
        """Get customer's default payment method"""

    @abstractmethod
    async def find_by_provider(
        self, provider: PaymentProvider, customer_id: CustomerId | None = None
    ) -> list[PaymentMethod]:
    ...
    async def get_by_provider_method_id(
        self, provider: PaymentProvider, provider_method_id: str
    ) -> PaymentMethod | None:
    ...
    async def set_as_default(
        self, payment_method_id: PaymentMethodId, customer_id: CustomerId
    ) -> None:
        """Set payment method as default and unset others"""

    @abstractmethod
    async def deactivate_all_for_customer(
        self, customer_id: CustomerId, except_method_id: PaymentMethodId | None = None
    ) -> int:
...
class ISubscriptionRepository(ABC):
    ...
    @abstractmethod
    async def find_by_status(
        self, status: SubscriptionStatus, limit: int = 100, offset: int = 0
    ) -> list[Subscription]:
    ...
    async def find_expiring_trials(
        self, days_ahead: int = 3, limit: int = 100
    ) -> list[Subscription]:
        """Find trial subscriptions expiring soon"""

    @abstractmethod
    async def find_due_for_renewal(self, limit: int = 100) -> list[Subscription]:
        """Find subscriptions due for renewal"""

    @abstractmethod
    async def find_past_due(self, days_overdue: int = 1, limit: int = 100) -> list[Subscription]:
        """Find past due subscriptions"""

    @abstractmethod
    async def find_for_cancellation(
        self, max_failed_days: int = 30, limit: int = 100
    ) -> list[Subscription]:
    ...
    @abstractmethod
    async def get_churn_rate(self, period_days: int = 30) -> dict[str, Any]:
        """Calculate churn rate for given period"""

    @abstractmethod
    async def get_revenue_forecast(self, periods_ahead: int = 12) -> list[dict[str, Any]]:
    ...
    async def update_next_billing_date(
        self, subscription_id: SubscriptionId, next_billing_date: datetime
    ) -> None:
...
class IPaymentPlanRepository(ABC):
    ...
    @abstractmethod
    async def get_by_id(self, plan_id: PlanId) -> dict[str, Any] | None:
        """Get plan by ID"""
    ...
    @abstractmethod
    async def get_plan_pricing(self, plan_id: PlanId, billing_cycle: str) -> dict[str, Any] | None:
