...
class GrowthForecasterService:
    ...
    def __init__(
        ...
        self.task_client = task_client  # Injected dependency for background tasks
        self.model_config = model_config or GRUGrowthModelConfig()
        self.cache_predictions = cache_predictions
        self.max_cache_size = max_cache_size

        # Initialize model and processor
        self.model: GRUGrowthModel | None = None
        self.data_processor: GrowthDataProcessor | None = None
    ...
    def _prepare_input_data(self, data: pd.DataFrame | list[dict] | dict) -> pd.DataFrame:
    ...
    def _generate_cache_key(
        self, data: pd.DataFrame, forecast_horizon: int, include_uncertainty: bool
    ) -> str:
    ...
    def _process_prediction_results(
        ...
        if uncertainties:
            ...
            result["confidence_intervals"] = [
                {"lower": pred - 1.96 * unc, "upper": pred + 1.96 * unc}
                for pred, unc in zip(pred_values, uncertainty_values, strict=False)
            ]

        # Add attention if available
        if attention_weights:
            result["attention_patterns"] = [weights[0].tolist() for weights in attention_weights]
    ...
    async def predict_growth_via_celery(
        ...
        """
        Submit prediction task to background queue and return task ID.

        This is the RECOMMENDED method for API endpoints to avoid blocking.
        ...
        Returns:
            Dictionary with task_id for status checking

        Raises:
            RuntimeError: If task_client is not configured (dependency not injected)
        """
        try:
            # Ensure task client is available
            if self.task_client is None:
                raise RuntimeError(
                    "Task client not configured. Please inject TaskClientProtocol "
                    "implementation when initializing GrowthForecasterService."
                )

            # Convert DataFrame to dict for JSON serialization
            if isinstance(data, pd.DataFrame):
                training_data = {"data": data.to_dict(orient="records")}
            elif isinstance(data, list):
                training_data = {"data": data}
            else:
                training_data = data

            # Submit to background queue via injected task client (returns immediately)
            ...
            logger.info(f"📤 Growth prediction task submitted: {task_id}")

            return {
                "task_id": task_id,
                "channel_id": channel_id,
                "status": "processing",
                "message": "Prediction task submitted to background worker",
                "estimated_time": "30-60 seconds",
                "check_status_at": f"/api/v1/ml/task-status/{task_id}",
            }

        except Exception as e:
            logger.error(f"❌ Failed to submit prediction task: {e}")
            raise
    ...
    def predict_growth_sync(
        ...
        """
        Synchronous version of predict_growth for Celery tasks.

        This method performs blocking operations and should ONLY be called
        from Celery worker context, never from async API endpoints.
        ...
        try:
            ...
            # Process data for model (BLOCKING - OK in Celery worker)
            if not self.data_processor.is_fitted:
                # First time - fit and transform (CPU-intensive)
                X_scaled, _ = self.data_processor.fit_transform(df)
            else:
                # Already fitted - just transform
                X_sequences, _ = self.data_processor.create_sequences(df)
            ...
            logger.info(f"🎯 Sync growth prediction completed for horizon: {forecast_horizon}")
            return result

        except Exception as e:
            self.health_stats["errors"] += 1
            logger.error(f"❌ Sync growth prediction failed: {e}")
            raise
