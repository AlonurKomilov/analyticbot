 ...
 */
export const calculateEngagementRate = (post: Post | null | undefined): number => {
    if (!post) return 0;

    const views = parseInt(String(post.views)) || 0;
    const likes = parseInt(String(post.likes)) || 0;
    const shares = parseInt(String(post.shares)) || 0;
    const comments = parseInt(String(post.comments)) || 0;
...
type StatusColor = 'success' | 'info' | 'warning' | 'default' | 'error';
