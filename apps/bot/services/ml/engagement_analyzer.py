...
class EngagementAnalyzer:
    ...
    async def generate_performance_report(
        ...
        try:
            ...
            content_analysis = await self._analyze_content_performance(channel_id, analytics_data)

            # Analyze user segments and behavior
            user_analysis = await self._analyze_user_behavior(channel_id, period_start, period_end)
            ...
            if include_churn_analysis:
                churn_summary = await self._analyze_channel_churn_risk(channel_id, period_days)
    ...
    async def analyze_content_before_publishing(
        ...
        try:
            ...
            optimal_timing = None
            if channel_id:
                optimal_timing = await self.prediction_service.find_optimal_posting_time(channel_id)
    ...
    async def get_real_time_insights(
        ...
        try:
            ...
            recent_data = await self._get_recent_performance_data(channel_id, lookback_hours)
            ...
            user_insights = await self._analyze_recent_user_behavior(channel_id, lookback_hours)
            ...
            high_confidence_insights.sort(
                key=lambda x: impact_order.get(x.impact_level, 0), reverse=True
            )
    ...
    async def _calculate_core_metrics(self, analytics_data: dict[str, Any]) -> dict[str, float]:
        ...
        total_engagement = sum(
            post["views"] + post["likes"] + post["comments"] + post["shares"] for post in posts
        )
        ...
        if overall_score > 80:
            ...
            insights.append(
                EngagementInsight(
                    ...
                    data_points={"content_score": overall_score},
                    recommendations=[
                        "Review content optimization suggestions",
            ...
            if recent_engagement > older_engagement * 1.2:  # 20% improvement
                ...
                insights.append(
                    EngagementInsight(
                        ...
                        description=(
                            f"Recent posts showing {improvement_pct:.1f}% " f"engagement increase."
                        ),
                        impact_level="high",
                        confidence=0.8,
