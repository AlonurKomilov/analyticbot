...
class IntelligenceService:
    ...
    async def check_real_time_alerts(self, channel_id: int) -> dict:
        ...
        try:
            ...
            # Engagement alerts
            engagement_alerts = await self._check_engagement_alerts(channel_id, current_metrics, alert_config)
            all_alerts.extend(engagement_alerts)
            
            # Growth alerts
            growth_alerts = await self._check_growth_alerts(channel_id, current_metrics, alert_config)
            all_alerts.extend(growth_alerts)
            
            # Performance alerts
            performance_alerts = await self._check_performance_alerts(channel_id, current_metrics, alert_config)
            all_alerts.extend(performance_alerts)
            
            # Statistical anomaly detection
            anomaly_alerts = await self._check_statistical_anomalies(channel_id, current_metrics, alert_config)
            all_alerts.extend(anomaly_alerts)
            
            # Sort alerts by severity
            severity_scores = {"critical": 3, "warning": 2, "info": 1}
            all_alerts.sort(key=lambda x: severity_scores.get(x.get("severity", "info"), 0), reverse=True)
    ...
    async def _establish_alert_baselines(self, channel_id: int) -> dict:
        ...
        try:
            ...
            daily_followers = await self._daily.series_data(channel_id, "followers", start_date, now)
            if not daily_followers:
                daily_followers = await self._daily.series_data(channel_id, "subscribers", start_date, now)
            ...
            follower_values = [item.get("value", 0) for item in daily_followers] if daily_followers else []
            ...
            if follower_values:
                ...
                for i in range(1, len(follower_values)):
                    if follower_values[i-1] > 0:
                        rate = (follower_values[i] - follower_values[i-1]) / follower_values[i-1]
        ...
        try:
            ...
            if config.get("growth_rate_alerts") and baseline.get("followers"):
                ...
                rules.append({
                    ...
                    "threshold": followers_baseline.get("mean_growth", 0) - (2 * followers_baseline.get("std_growth", 0)),
                    "condition": "below",
                    "severity": "warning",
    ...
    async def _check_engagement_alerts(self, channel_id: int, current_metrics: dict, alert_config: dict) -> list:
        ...
        try:
            ...
            drop_threshold = alert_config.get("engagement_drop_threshold", 0.3)
    ...
    async def _check_growth_alerts(self, channel_id: int, current_metrics: dict, alert_config: dict) -> list:
    ...
    async def _check_performance_alerts(self, channel_id: int, current_metrics: dict, alert_config: dict) -> list:
    ...
    async def _check_statistical_anomalies(self, channel_id: int, current_metrics: dict, alert_config: dict) -> list:
    ...
    async def _trigger_alert_notifications(self, channel_id: int, alerts: list) -> bool:
        ...
        try:
            ...
            for alert in alerts:
                logger.warning(f"ALERT for channel {channel_id}: {alert.get('message', 'Unknown alert')}")
    ...
    async def generate_competitive_intelligence(
        ...
        try:
            ...
            if not competitor_ids:
                competitor_ids = await self._discover_competitor_channels(channel_id, max_competitors=5)
            ...
            analysis = {
                ...
                "performance_comparison": await self._compare_performance_metrics(channel_id, all_data),
            ...
            opportunities = await self._identify_market_opportunities(channel_id, all_data, analysis)
    ...
    async def _discover_competitor_channels(self, channel_id: int, max_competitors: int = 5) -> list:
    ...
    async def _analyze_market_position(self, channel_id: int, all_data: dict) -> dict:
        ...
        try:
            ...
            # Follower ranking
            follower_rank = len([c for c in all_channels if c.get("followers", 0) > channel_data.get("followers", 0)]) + 1
            
            # Engagement ranking
            engagement_rank = len([c for c in all_channels if c.get("engagement_rate", 0) > channel_data.get("engagement_rate", 0)]) + 1
            
            # Views ranking
            views_rank = len([c for c in all_channels if c.get("avg_views_per_post", 0) > channel_data.get("avg_views_per_post", 0)]) + 1
            
            # Overall score (normalized)
            total_channels = len(all_channels)
            overall_score = 1 - ((follower_rank + engagement_rank + views_rank) / (3 * total_channels))
            ...
            return {
                ...
                "market_position": "leader" if overall_score > 0.7 else "competitive" if overall_score > 0.4 else "challenger"
            }
        except Exception as e:
    ...
    async def _compare_performance_metrics(self, channel_id: int, all_data: dict) -> dict:
        ...
        try:
            ...
            avg_competitor_engagement = np.mean(competitor_engagement) if competitor_engagement else 0
            avg_competitor_views = np.mean(competitor_views) if competitor_views else 0
            
            # Calculate performance ratios
            follower_ratio = (channel_data.get("followers", 0) / avg_competitor_followers) if avg_competitor_followers > 0 else 0
            engagement_ratio = (channel_data.get("engagement_rate", 0) / avg_competitor_engagement) if avg_competitor_engagement > 0 else 0
            views_ratio = (channel_data.get("avg_views_per_post", 0) / avg_competitor_views) if avg_competitor_views > 0 else 0
    ...
    async def _identify_market_opportunities(self, channel_id: int, all_data: dict, analysis: dict) -> list:
