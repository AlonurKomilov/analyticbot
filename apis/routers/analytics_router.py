...
def generate_top_posts(count: int = 10) -> List[TopPost]:
    ...
    for i in range(count):
        ...
        posts.append(TopPost(
            ...
            thumbnail=f"https://picsum.photos/64/64?random={i}" if random.choice([True, False]) else None
        ))
...
async def get_ai_insights(
    channel_id: int,
    insights_generator: AIInsightsGenerator = Depends(get_ai_insights)
):
