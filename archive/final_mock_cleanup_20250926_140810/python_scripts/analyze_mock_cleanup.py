...
def find_mock_files(base_path: str) -> dict[str, list[str]]:
    ...
    mock_files = {"services": [], "data": [], "adapters": [], "other": []}

    # Patterns to identify mock files
    service_patterns = [r"mock_.*_service\.py$", r".*_mock_service\.py$"]

    data_patterns = [r"mock_.*_data\.py$", r".*_mock_data\.py$", r"mock_data\.py$"]

    adapter_patterns = [r"mock_.*_adapter\.py$", r".*_mock_adapter\.py$"]
...
def analyze_mock_imports(base_path: str, mock_files: dict[str, list[str]]) -> dict[str, list[str]]:
    ...
    imports = {"direct_imports": [], "from_imports": [], "mixed_imports": []}
                ...
                with open(file_path, encoding="utf-8") as f:
                    ...
                    for mock_module in mock_modules:
                        if f"import {mock_module}" in content:
                            ...
                            f"from .{mock_module}" in content or f"from ...{mock_module}" in content
                        ):
