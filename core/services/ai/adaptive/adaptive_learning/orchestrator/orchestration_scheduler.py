...
from .workflow_models import OrchestrationConfig, OrchestrationStrategy
    ...
    async def add_model_scheduling(
        ...
        try:
            ...
            self.scheduled_updates[model_id] = {
                ...
                "next_drift_check": datetime.utcnow()
                + timedelta(hours=self.config.drift_check_frequency_hours),
                "next_feedback_check": datetime.utcnow()
                + timedelta(minutes=self.config.monitoring_interval_minutes),
            }
    ...
    async def _check_model_status(self, model_id: str) -> None:
        ...
        try:
            ...
            if health_status in ["critical", "warning"]:
                ...
                active_workflows = await self.workflow_manager.get_active_workflows_for_model(
                    model_id
                )
                if not active_workflows:
                    ...
                    if workflow_id:
                        logger.info(
                            f"🎯 Created reactive workflow {workflow_id} for model {model_id}"
                        )
    ...
    async def _check_drift_trigger(self, model_id: str, schedule_config: dict[str, Any]) -> bool:
        ...
        try:
            ...
            if workflow_id:
                ...
                logger.info(
                    f"📅 Triggered scheduled drift check workflow {workflow_id} for model {model_id}"
                )
                return True
