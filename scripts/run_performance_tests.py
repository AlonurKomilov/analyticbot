...
try:
    from tests.test_performance_optimization import PerformanceTester, run_comprehensive_performance_test
except ImportError:
    raise ImportError("Performance optimization test module not found. Please ensure tests/test_performance_optimization.py exists.")
...
async def run_basic_performance_tests():
    ...
    try:
        ...
        logger.info(f"📊 DB Connection: {db_result.avg_time:.3f}s avg, {db_result.throughput:.1f} ops/sec")
        ...
        if performance_manager.cache._is_connected:
            ...
            cache_result = await tester.benchmark_function(
                "cache_operations_basic",
                cache_test,
                iterations=50,
                concurrent=False
            )
            
            logger.info(f"📦 Cache: {cache_result.avg_time:.3f}s avg, {cache_result.throughput:.1f} ops/sec")
        else:
        ...
        logger.info(f"🔄 Concurrent: {concurrent_result.avg_time:.3f}s avg, {concurrent_result.throughput:.1f} ops/sec")
...
async def run_load_test():
    ...
    try:
        ...
        logger.info(f"🔥 Load Test: {load_result.avg_time:.3f}s avg, {load_result.error_rate:.1f}% errors")
    ...
    try:
        ...
        if "load_test" in results:
            ...
            if "memory" in load:
                ...
                logger.info(f"   {'⚠️' if mem['potential_leak'] else '✅'} Memory Leak: {'Detected' if mem['potential_leak'] else 'None'}")
