    ...
    def _summarize_result(self, result: dict) -> dict:
        ...
        if "messages" in result:
            summary["message_count"] = (
                len(result["messages"]) if isinstance(result["messages"], list) else 0
            )

        if "media" in result:
            summary["media_count"] = (
                len(result["media"]) if isinstance(result["media"], list) else 0
            )

        if "participants" in result:
            summary["participant_count"] = (
                len(result["participants"]) if isinstance(result["participants"], list) else 0
            )
