...
class StorageChannelCreate(BaseModel):
    ...
    channel_username: Optional[str] = Field(None, description="Channel username if public")
...
async def get_storage_channels(
    ...
    **Note:** This feature is currently under development.
    """
    logger.info(f"User {current_user.get('id')} requested storage channels (active_only={only_active})")
...
async def upload_file_to_storage(
    ...
    storage_channel_id: Optional[int] = Form(None, description="Specific channel ID"),
    current_user: dict = Depends(get_current_user),
):
    """
...
@router.get(
    ...
    summary="List files in Telegram storage"
)
...
@router.post(
    "/files/{media_id}/forward",
    summary="Forward file to another channel"
)
