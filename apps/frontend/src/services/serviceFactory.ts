...
export const apiClient = {
    get: async (url: string, config?: any) => {
        if (isDemoUser()) {
            ...
            const realClient = await import('./apiClient');
...
class ServiceFactory {
    static async getChurnPredictorService() {
        if (isDemoUser()) {
            ...
            const { default: RealChurnService } = await import('./ChurnPredictorService');
    ...
    static async getPredictiveAnalyticsService() {
        if (isDemoUser()) {
            ...
            const { default: RealPredictiveService } = await import('./PredictiveAnalyticsService');
