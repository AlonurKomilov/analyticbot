...
class DatabaseLoadTester:
    ...
    async def test_query_performance(
        ...
        logger.info(f"🔍 Starting database load test: {concurrent_connections} connections, {total_queries} queries")
    ...
    async def test_connection_pool_performance(
        ...
        logger.info(f"🏊 Testing connection pool: {pool_size} pool size, {max_connections} max connections")
    ...
    def _calculate_metrics(self, results: List[LoadTestResult], total_duration: float) -> LoadTestMetrics:
...
class APILoadTester:
    ...
    async def run_load_test(self, config: LoadTestConfig) -> LoadTestMetrics:
        ...
        logger.info(f"🚀 Starting API load test: {config.concurrent_users} users, {config.total_requests} requests")
    ...
    def _calculate_api_metrics(self, results: List[LoadTestResult], total_duration: float) -> LoadTestMetrics:
...
class PerformanceBenchmark:
    ...
    async def benchmark_api_endpoints(self, endpoints: List[Dict[str, Any]]) -> Dict[str, LoadTestMetrics]:
        ...
        async with APILoadTester() as load_tester:
            for endpoint_config in endpoints:
                ...
                try:
                    ...
                    logger.info(f"✅ Endpoint {config.target_url}: "
    ...
    async def benchmark_database_queries(self, db_connection_func: Callable, queries: List[Dict[str, Any]]) -> Dict[str, LoadTestMetrics]:
    ...
    def generate_performance_report(self, results: Dict[str, LoadTestMetrics], output_file: Optional[str] = None) -> Dict:
        ...
        if results:
            ...
            report['summary']['overall_metrics'] = {
                ...
                'best_performing_test': min(results.keys(), key=lambda k: results[k].avg_response_time),
                'worst_performing_test': max(results.keys(), key=lambda k: results[k].avg_response_time)
            }
    ...
    def _generate_recommendations(self, results: Dict[str, LoadTestMetrics]) -> List[str]:
        ...
        for test_name, metrics in results.items():
            ...
            if metrics.avg_response_time > 2.0:
                recommendations.append(f"🐌 {test_name}: High response time ({metrics.avg_response_time:.2f}s) - Consider optimization")
            
            # Low success rate
            if metrics.success_rate < 0.95:
                recommendations.append(f"⚠️ {test_name}: Low success rate ({metrics.success_rate:.1%}) - Investigate errors")
            
            # High P99 response time
            if metrics.p99_response_time > metrics.avg_response_time * 3:
                recommendations.append(f"📈 {test_name}: High P99 latency - Check for outliers and bottlenecks")
            
            # Low throughput
            if metrics.requests_per_second < 10:
                recommendations.append(f"🔄 {test_name}: Low throughput ({metrics.requests_per_second:.1f} RPS) - Consider scaling")
...
async def run_api_performance_test():
    ...
    for endpoint, metrics in results.items():
        logger.info(f"  {endpoint}: {metrics.success_rate:.1%} success, {metrics.avg_response_time:.3f}s avg")
...
async def run_database_performance_test(db_connection_func):
    ...
    for query_name, metrics in results.items():
        logger.info(f"  {query_name}: {metrics.success_rate:.1%} success, {metrics.avg_response_time:.3f}s avg")
