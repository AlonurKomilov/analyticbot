...
class APITester:
    ...
    async def test_endpoint(
        self, method: str, endpoint: str, data=None, expected_status=200
    ):
...
async def run_comprehensive_tests():
    ...
    async with APITester() as tester:
        ...
        if success:
            ...
            print(
                f"   ✅ Readability: {response.get('readability_score', 'N/A'):.1f}/100"
            )
        ...
        for scenario in scenarios:
            ...
            if success:
                test_results["passed_tests"] += 1
                print(
                    f"      ✅ Overall Score: {response.get('overall_score', 'N/A'):.3f}"
                )
                print(
                    f"      ✅ Length Score: {response.get('length_score', 'N/A'):.3f}"
                )
                print(
                    f"      ✅ Hashtag Score: {response.get('hashtag_score', 'N/A'):.3f}"
                )
                print(
                    f"      ✅ Sentiment Score: {response.get('sentiment_score', 'N/A'):.3f}"
                )
        ...
        if success:
            ...
            print(
                f"   📝 Sample Content Length: {len(response.get('sample_content', ''))}"
            )
        else:
            test_results["failed_tests"] += 1
    ...
    response_times = [
        detail["response_time_ms"] for detail in test_results["test_details"]
    ]
