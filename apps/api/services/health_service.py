...
class HealthMonitoringService:
    ...
    def __init__(self, service_name: str = "analyticbot", version: str | None = None, thresholds: HealthThresholds | None = None):
        ...
        self.logger.info(f"🏥 HealthMonitoringService initialized for {service_name} v{self.version}")
    ...
    async def get_system_health(self, include_non_critical: bool = True) -> SystemHealth:
        ...
        for (name, _), check_result in zip(component_checks, results):
            if isinstance(check_result, Exception):
                ...
                if check_result.status == HealthStatus.UNHEALTHY:
                    ...
                    alerts.append(f"🟡 {name} is degraded: {check_result.error or 'High response time'}")
                
                # Performance-based alerts
                if check_result.response_time_ms:
                    if check_result.response_time_ms > self.thresholds.response_time_critical_ms:
                        alerts.append(f"⚠️ {name} critical response time: {check_result.response_time_ms:.1f}ms")
                    elif check_result.response_time_ms > self.thresholds.response_time_warning_ms:
                        alerts.append(f"⏱️ {name} slow response time: {check_result.response_time_ms:.1f}ms")
