...
class ContentAnalyzerService:
    ...
    def __init__(
        ...
        model_config: Optional[CNNTransformerConfig] = None,
        ...
        self.model: Optional[CNNTransformerModel] = None
        self.data_processor: Optional[ContentDataProcessor] = None
                    ...
                    with torch.no_grad():
                        ...
                        for task_name, task_pred in raw_predictions.items():
                            predictions[task_name] = {
                                ...
                                'confidence': torch.ones(task_pred.size(0)) * 0.8  # Default confidence
                            }
        ...
        try:
            ...
            if toxicity_data:
                ...
                if isinstance(all_probs, list) and len(all_probs) >= 2:
                    toxic_prob = all_probs[1] if len(all_probs) > 1 else 0  # Toxic class probability
                else:
                    toxic_prob = 0.0
                risk_scores['toxicity'] = {
                    'score': float(toxic_prob),
                    'level': 'high' if toxic_prob > 0.7 else 'medium' if toxic_prob > 0.3 else 'low',
                    'confidence': float(confidence_metrics.get('toxicity', {}).get('confidence', 0.5))
                }
            ...
            if quality_data:
                ...
                risk_scores['quality'] = {
                    ...
                    'level': 'high' if quality_risk > 0.6 else 'medium' if quality_risk > 0.4 else 'low',
                    'quality_rating': self.task_mappings['quality'].get(int(quality_class), 'unknown')
                }
            ...
            if sentiment_data:
                ...
                risk_scores['sentiment'] = {
                    ...
                    'sentiment': self.task_mappings['sentiment'].get(int(sentiment_class), 'unknown')
                }
            ...
            risk_report = {
                "overall_risk": {
                    ...
                    "level": "high" if overall_risk > 0.6 else "medium" if overall_risk > 0.3 else "low",
    ...
    async def _process_analysis_results(
        ...
        for i in range(batch_size):
            text_result = {
                ...
                "content_scores": {},
                "confidence_metrics": {},
                "text_preview": processed_batch['original_texts'][i][:100] + "..." if len(processed_batch['original_texts'][i]) > 100 else processed_batch['original_texts'][i]
            }
            
            # Process each task prediction
            for task_name, task_data in predictions.items():
                pred_probs = task_data['prediction'][i]
                confidence = task_data.get('confidence', torch.tensor(0.8))[i] if hasattr(task_data.get('confidence', torch.tensor(0.8)), '__getitem__') else task_data.get('confidence', torch.tensor(0.8))
                ...
                text_result['content_scores'][task_name] = {
                    ...
                    'predicted_label': self.task_mappings.get(task_name, {}).get(int(pred_class), 'unknown'),
                ...
                text_result['confidence_metrics'][task_name] = {
                    ...
                    'uncertainty': 1.0 - (confidence.item() if hasattr(confidence, 'item') else float(confidence))
                }
            ...
            if include_features:
                ...
                if self.data_processor:
                    complexity = self.data_processor.analyze_text_complexity(processed_batch['original_texts'][i])
                    text_result['complexity_analysis'] = complexity
            
            # Add risk assessment
            if risk_assessment:
                text_result['risk_indicators'] = self._calculate_risk_indicators(text_result['content_scores'])
    ...
    def _calculate_risk_indicators(self, content_scores: Dict[str, Dict]) -> Dict[str, str]:
        ...
        if toxicity_data and 'all_probabilities' in toxicity_data:
            ...
            risk_indicators['toxicity_risk'] = 'high' if toxic_prob > 0.7 else 'medium' if toxic_prob > 0.3 else 'low'
        
        # Quality risk
        quality_data = content_scores.get('quality', {})
        if quality_data and 'predicted_class' in quality_data:
            quality_class = quality_data['predicted_class']
            risk_indicators['quality_risk'] = 'high' if quality_class < 2 else 'medium' if quality_class < 3 else 'low'
        
        # Sentiment risk
        sentiment_data = content_scores.get('sentiment', {})
        if sentiment_data and 'predicted_class' in sentiment_data:
            sentiment_class = sentiment_data['predicted_class']
            risk_indicators['sentiment_risk'] = 'high' if sentiment_class == 2 else 'low'  # 2 = negative
        
        return risk_indicators
    
    def _generate_cache_key(self, content: str, include_confidence: bool, include_features: bool) -> str:
