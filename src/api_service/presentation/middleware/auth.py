...
using the existing SecurityManager from src.shared_kernel.domain.security_engine.
"""
...
from src.shared_kernel.domain.security_engine import SecurityManager, get_security_manager, get_rbac_manager
from src.shared_kernel.domain.security_engine.auth_utils import auth_utils, AuthError, security_scheme
from src.shared_kernel.domain.security_engine.rbac import RBACManager, Permission
from src.shared_kernel.domain.security_engine.models import User, UserRole
from src.shared_kernel.domain.repositories.interfaces import UserRepository, ChannelRepository
...
async def require_channel_access(
    ...
    try:
        ...
        if channel_id not in user_channel_ids:
            logger.warning(f"User {user_id} attempted to access channel {channel_id} without permission")
            raise HTTPException(
