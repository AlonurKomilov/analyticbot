...
class DeploymentPlanManager:
    ...
    async def create_deployment_plan(
        ...
        try:
            ...
            approval_required = await self._requires_approval(risk_assessment.overall_risk, strategy)
            ...
            plan = DeploymentPlan(
                ...
                rollback_strategy=await self._select_rollback_strategy(strategy, risk_assessment.overall_risk),
                performance_requirements=requirements or await self._generate_default_requirements(model_id),
            ...
            logger.info(f"📋 Created deployment plan {plan_id}: {strategy.value} strategy, {risk_assessment.overall_risk.value} risk")
            return plan
    ...
    async def list_active_plans(self, model_id: Optional[str] = None) -> List[DeploymentPlan]:
    ...
    async def _assess_deployment_risk(
                ...
                for metric, value in perf_metrics.items():
                    if metric in baseline:
                        ...
                        if degradation > 0.1:  # 10% degradation
                            ...
                            mitigation_strategies.append(f"Monitor {metric} closely during deployment")
        ...
        if hasattr(target_metadata, 'architecture') and hasattr(target_metadata, 'previous_architecture'):
    ...
    async def _generate_default_requirements(
        self,
        model_id: str
    ) -> List[PerformanceRequirement]:
    ...
    async def _validate_performance_requirements(self, plan: DeploymentPlan) -> Dict[str, Any]:
        ...
        for req in plan.performance_requirements:
            ...
            if req.max_degradation_percent < 0:
                errors.append(f"Invalid max_degradation_percent for {req.metric_name}: {req.max_degradation_percent}")
            
            if req.max_degradation_percent > 50:
                warnings.append(f"High degradation tolerance for {req.metric_name}: {req.max_degradation_percent}%")
    ...
    async def _validate_deployment_constraints(self, plan: DeploymentPlan) -> Dict[str, Any]:
        ...
        for constraint in plan.deployment_constraints:
            ...
            if constraint.is_blocking and not constraint.validation_function:
                errors.append(f"Blocking constraint {constraint.constraint_type} missing validation function")
    ...
    async def _validate_strategy_compatibility(self, plan: DeploymentPlan) -> Dict[str, Any]:
        ...
        if plan.risk_level == DeploymentRisk.CRITICAL and plan.strategy == DeploymentStrategy.DIRECT:
