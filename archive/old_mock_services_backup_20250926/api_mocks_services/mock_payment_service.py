...
from src.shared_kernel.domain.protocols import PaymentServiceProtocol
from src.bot_service.services.adapters.mock_payment_adapter import MockPaymentAdapter
from src.bot_service.models.payment import BillingCycle
...
class MockPaymentService(PaymentServiceProtocol):
    ...
    async def process_payment(self, amount: int, currency: str, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            method_result = await self.adapter.create_payment_method(
                customer_id, 
                {"type": "card"}
            )
