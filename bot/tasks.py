...
def remove_expired_schedulers(self):  # type: ignore[override]
    async def _run():
        ...
        try:
            repo = container.scheduler_repository()
            await repo.remove_expired(datetime.utcnow())
        finally:
            try:
                db = container.db_session()
                if hasattr(db, "close"):
...
def send_scheduled_message(self):  # type: ignore[override]
    async def _run():
        ...
        try:
            ...
            for post in due_posts:
                try:
                    await scheduler_service.send_post_to_channel(post)
                    # status updated to 'sent' inside service; if failure occurs we leave as 'sending' or 'error'
                except Exception as e:  # pragma: no cover - defensive
                    logger.exception("failed sending scheduled post %s", post.get("id"), exc_info=e)
        finally:
            try:
                db = container.db_session()
                if hasattr(db, "close"):
