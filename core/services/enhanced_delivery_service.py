...
class EnhancedDeliveryService(BaseDeliveryService):
    ...
    def __init__(self, delivery_repo, schedule_repo, redis_url: Optional[str] = None):
    ...
    async def send_with_reliability_guards(
        ...
        if is_duplicate and existing_status:
            if existing_status.status == "completed":
                logger.info(f"Duplicate delivery detected, returning cached result: {idempotency_key}")
                return {
                    "success": True,
        ...
        try:
            ...
            if not chat_allowed:
                raise Exception(f"Per-chat rate limit exceeded for {channel_id}, max wait time reached")
            ...
            is_rate_limit_error = any(phrase in error_msg.lower() for phrase in [
                "rate limit", "too many requests", "flood", "slow down"
            ])
    ...
    async def reset_rate_limits(self, bucket_type: str = "chat", bucket_id: Optional[str] = None) -> Dict[str, Any]:
