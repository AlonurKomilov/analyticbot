...
class PredictiveAnalyticsService:
    ...
    async def generate_predictive_analytics(
        ...
        try:
            ...
            # Note: ML engine should be injected via dependency injection from apps layer
            if use_ml_models:
                logger.info("ML insights requested but ML engine must be provided via DI")
                predictions["ml_insights"] = {
                    "status": "ml_engine_not_injected",
                    "message": "ML engine should be injected from apps layer via dependency injection",
                }
    ...
    async def _predict_engagement_patterns(
        ...
        try:
            ...
            if posts:
                ...
                best_hour = max(hour_avg.keys(), key=lambda h: hour_avg[h]) if hour_avg else 12
    ...
    async def _predict_optimization_opportunities(self, training_data: dict, use_ml: bool) -> dict:
        ...
        try:
            ...
            best_length = (
                max(length_avg.keys(), key=lambda b: length_avg[b]) if length_avg else "medium"
            )
