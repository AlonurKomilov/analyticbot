...
class AIInsightsGenerator:
    ...
    async def generate_comprehensive_insights(self, df: pd.DataFrame, 
    ...
    def _analyze_data_overview(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            if overview['missing_data_percentage'] > 10:
                quality_issues.append(f"High missing data: {overview['missing_data_percentage']:.1f}%")
            ...
            overview['data_quality_score'] = max(0, 100 - overview['missing_data_percentage'] - (overview['duplicated_records'] / len(df) * 100))
            ...
            if len(categorical_cols) > 0:
                ...
                for col in categorical_cols:
                    ...
                    categorical_patterns[col] = {
                        ...
                        'distribution_type': 'uniform' if len(set(value_counts.values)) == 1 else 'skewed',
    ...
    async def _analyze_trends(self, df: pd.DataFrame, 
                            datetime_column: Optional[str] = None) -> Dict[str, Any]:
            ...
            if STATSMODELS_AVAILABLE and len(ts) >= 24:  # Need at least 2 periods
                try:
                    ...
                    trend_info['seasonal_decomposition'] = {
                        ...
                        'seasonal_strength': float(np.var(decomposition.seasonal.dropna()) / np.var(ts)),
        ...
        try:
            ...
            if len(datetime_cols) > 0:
                recommendations.append({
                    ...
                    'description': f'Temporal data detected. Consider time series analysis and forecasting.',
    ...
    async def _generate_insight_summary(self, insights: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if 'isolation_forest' in anomalies:
                ...
                if anomaly_pct > 5:
                    summary_parts.append(f"Detected {anomaly_pct:.1f}% anomalous data points requiring investigation.")
            
            # Pattern summary
            patterns = insights.get('patterns', {})
            if patterns:
                pattern_count = len(patterns.get('categorical', {})) + len(patterns.get('numeric', {}))
            ...
            if recommendations:
                ...
                summary_parts.append(f"Generated {len(recommendations)} recommendations ({high_priority} high priority).")
    ...
    def _calculate_insight_confidence(self, insights: Dict[str, Any]) -> float:
        ...
        try:
            ...
            analysis_sections = ['statistical_insights', 'anomalies', 'patterns', 'trends', 'correlations']
            completed_sections = sum(1 for section in analysis_sections if section in insights and insights[section])
...
if __name__ == "__main__":
    ...
    sample_data = {
        'date': dates,
        'sales': 1000 + np.cumsum(np.random.normal(10, 50, 365)) + 200 * np.sin(np.arange(365) * 2 * np.pi / 30),
