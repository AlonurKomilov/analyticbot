...
class ContentProtectionService:
    ...
    async def add_image_watermark(self, image_path: str | Path, config: WatermarkConfig) -> Path:
    ...
    async def add_video_watermark(self, video_path: str | Path, config: WatermarkConfig) -> Path:
        ...
        try:
            ...
            cmd = [
                ...
                str(input_path),
                "-vf",
                f"drawtext=text='{config.text}':fontcolor={config.color}@{alpha}:fontsize={config.font_size}:fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf:{position}",
                "-codec:a",
            ...
            raise RuntimeError(
                "FFmpeg not installed. Please install FFmpeg to use video watermarking."
            )
        except Exception as e:
            raise RuntimeError(f"Failed to add watermark to video: {e}")

    async def create_custom_emoji_message(
        self, text: str, emoji_ids: list[str]
    ) -> tuple[str, list[dict]]:
...
class PremiumEmojiService:
    ...
    async def format_premium_message(
        text: str, user_tier: str, include_signature: bool = True
    ) -> tuple[str, list[dict]]:
