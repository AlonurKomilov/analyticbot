...
from bot.models.payment import (
    PaymentCreate, PaymentResponse, PaymentStatus, SubscriptionStatus,
    BillingCycle, PaymentProvider, PaymentMethodCreate, PaymentMethodResponse,
    SubscriptionCreate, SubscriptionResponse, WebhookPayload
)
...
class StripeAdapter(PaymentGatewayAdapter):
    ...
    async def create_subscription(
        ...
        return {
            ...
            "current_period_end": int((now + timedelta(days=30 if billing_cycle == BillingCycle.MONTHLY else 365)).timestamp()),
            "trial_end": int((now + timedelta(days=trial_days)).timestamp()) if trial_days else None,
    ...
    async def handle_webhook_event(self, event_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if event_type == "payment_intent.succeeded":
            ...
            return {
                ...
                "payment_id": event_data["data"]["object"]["id"],
                "failure_code": event_data["data"]["object"].get("last_payment_error", {}).get("code"),
                "failure_message": event_data["data"]["object"].get("last_payment_error", {}).get("message")
            }
...
class PaymentService:
    ...
    async def create_payment_method(
        ...
        try:
            ...
            provider_response = await adapter.create_payment_method(
                user_id, 
                payment_method_data.provider_data
            )
    ...
    async def process_payment(
        ...
        try:
            ...
            status = PaymentStatus.SUCCEEDED if provider_response.get("status") == "succeeded" else PaymentStatus.FAILED
    ...
    async def create_subscription(
        ...
        if subscription_data.payment_method_id:
            payment_method = await self.repository.get_payment_method(subscription_data.payment_method_id)
            if payment_method:
                provider = payment_method["provider"]
                adapter = self.get_adapter(provider)
    ...
    async def handle_webhook(
        ...
        try:
            ...
            if 'event_id' in locals():
                await self.repository.mark_webhook_processed(
                    event_id, False, str(e)
                )
            raise
