# infra/telegram/aiogram/telegram_service.py
"""
Telegram service implementation using aiogram.

This module provides the Telegram Bot API client implementation using aiogram.
Used for:
- Sending text messages to users/channels
...
class AiogramTelegramService(TelegramService):
    ...
    async def send_message(self, chat_id: str, text: str, **kwargs) -> dict[str, Any]:
        ...
        try:
            result = await self._bot.send_message(chat_id=int(chat_id), text=text, **kwargs)
            return {
    ...
    async def send_media(
        self, chat_id: str, media_url: str, caption: str = "", **kwargs
    ) -> dict[str, Any]:
