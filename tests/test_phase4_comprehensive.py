    ...
    business_df = pd.DataFrame(
        {
            ...
            "product_category": np.random.choice(
                ["Electronics", "Clothing", "Books", "Home"], 1000
            ),
    ...
    financial_df = pd.DataFrame(
        {
            ...
            "sector": np.random.choice(
                ["Tech", "Finance", "Healthcare", "Energy"], 365
            ),
    ...
    iot_df = pd.DataFrame(
        {
            ...
            "sensor_1": 25
            + 5 * np.sin(np.arange(2000) * 2 * np.pi / 48)
            ...
            "device_status": np.random.choice(
                ["active", "inactive", "maintenance"], 2000, p=[0.8, 0.15, 0.05]
            ),
            "location": np.random.choice(
                ["Building_A", "Building_B", "Building_C"], 2000
            ),
        }
    )

    return {"business": business_df, "financial": financial_df, "iot": iot_df}
...
async def test_module_4_1_data_processor():
    ...
    try:
        ...
        from bot.analytics import AdvancedDataProcessor
        ...
        print(
            f"   ✅ Missing values detected: {quality_report['missing_values_count']}"
        )
        ...
        print(
            f"   ✅ Missing values removed: {business_df.isnull().sum().sum() - cleaned_df.isnull().sum().sum()}"
        )
        ...
        print(
            f"   ✅ Categorical columns analyzed: {len(stats['categorical_summary'])}"
        )
        ...
        print(
            f"   ✅ Transformations applied: {len(['sales', 'customers', 'conversion_rate'])}"
        )
...
async def test_module_4_2_predictive_engine():
    ...
    try:
        ...
        from bot.analytics import PredictiveAnalyticsEngine
        ...
        ml_results = await engine.auto_predict(
            business_df, target_column="sales", task_type="regression", test_size=0.2
        )
        ...
        features_df = datasets["business"][
            ["sales", "customers", "marketing_spend", "temperature"]
        ].dropna()
        ...
        load_success = engine.load_model(
            f"{model_name}_loaded", f"/tmp/{model_name}.pkl"
        )
...
async def test_module_4_3_dashboard():
    ...
    try:
        ...
        line_chart = viz_engine.create_line_chart(
            business_df.head(100), "timestamp", "sales", title="Sales Over Time"
        )
        ...
        corr_chart = viz_engine.create_correlation_matrix(
            numeric_df, title="Feature Correlations"
        )
        ...
        export_path = viz_engine.export_chart(
            line_chart, "/tmp/test_chart", format="html"
        )
...
async def test_module_4_4_ai_insights():
    ...
    try:
        ...
        from bot.analytics import AIInsightsGenerator
        ...
        insights = await insights_gen.generate_comprehensive_insights(
            business_df, target_column="sales", time_column="timestamp"
        )
        print(
            f"   ✅ Insights generated for {insights['metadata']['dataset_shape'][0]} records"
        )
        print(f"   ✅ Insight types: {list(insights['insights'].keys())}")
        print(f"   ✅ Recommendations: {len(insights['recommendations'])}")
        print(
            f"   ✅ Confidence scores available: {len(insights['confidence_scores'])}"
        )
        ...
        print(
            f"   ✅ Missing data analysis: {len(quality_issues['missing_data'])} insights"
        )
        print(
            f"   ✅ Outlier analysis: {len(quality_issues['outliers'])} columns analyzed"
        )
        print(
            f"   ✅ Quality recommendations: {len(quality_issues['recommendations'])}"
        )
        ...
        trend_predictions = await insights_gen.predict_future_trends(
            ts_df, time_column="date", value_column="stock_price", prediction_periods=30
        )
        print("   ✅ Historical analysis completed")
        print(
            f"   ✅ Trend predictions: {len(trend_predictions['trend_predictions'])} periods"
        )
        print(
            f"   ✅ Trend changes detected: {len(trend_predictions['trend_changes'])}"
        )
        print(
            f"   ✅ Strategic recommendations: {len(trend_predictions['recommendations'])}"
        )
        ...
        alerts = await insights_gen.generate_automated_alerts(
            business_df, sensitivity="medium"
        )
...
async def test_module_4_5_reporting():
    ...
    try:
        ...
        from bot.analytics import AutomatedReportingSystem
        ...
        print(
            f"   ✅ Custom styling applied: {custom_template.styling['colors']['primary']}"
        )
        ...
        print(
            f"   ✅ Multi-dataset report generated: {len(multi_dataset_files)} formats"
        )
...
async def run_integration_tests():
    ...
    try:
        ...
        print(
            f"   ✅ Data processed: {cleaned_df.shape[0]} records, quality: {quality_report['overall_score']:.1f}"
        )

        # Step 2: Predictive modeling
        ml_results = await predictor.auto_predict(cleaned_df, "sales", test_size=0.2)
        print(
            f"   ✅ ML model trained: {ml_results['best_model']} (R²: {ml_results['best_model_score']:.3f})"
        )
        ...
        report_files = await reporter.generate_comprehensive_report(
            cleaned_df, report_title="Integration Test Report", output_formats=["json"]
        )
        ...
        print(
            f"   ✅ Data flow: {original_shape} → {processed_shape} → {predicted_samples} predictions"
        )
...
async def main():
    ...
    test_results.append(
        ("Module 4.1 - Data Processor", await test_module_4_1_data_processor())
    )
    test_results.append(
        ("Module 4.2 - Predictive Engine", await test_module_4_2_predictive_engine())
    )
    test_results.append(("Module 4.3 - Dashboard", await test_module_4_3_dashboard()))
    test_results.append(
        ("Module 4.4 - AI Insights", await test_module_4_4_ai_insights())
    )
