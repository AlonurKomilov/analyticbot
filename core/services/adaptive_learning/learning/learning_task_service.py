...
class LearningTaskService(LearningProtocol):
    ...
    def __init__(self, config: LearningTaskConfig | None = None):
    ...
    async def get_active_tasks(self, model_id: str | None = None) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if status and "progress" in status:
                ...
                return LearningProgress(
                    ...
                    status=UpdateStatus(
                        progress_data.get("status", UpdateStatus.IN_PROGRESS.value)
                    ),
                    ...
                    started_at=datetime.fromisoformat(
                        progress_data.get("started_at", datetime.utcnow().isoformat())
                    ),
                    estimated_completion=None,
                )
    ...
    async def get_learning_progress(self, task_id: str) -> LearningProgress | None:
        ...
        try:
            ...
            return LearningProgress(
                ...
                strategy=LearningStrategy(
                    progress_data.get("strategy", LearningStrategy.INCREMENTAL.value)
                ),
