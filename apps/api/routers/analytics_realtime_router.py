...
router = APIRouter(prefix="/analytics/realtime", tags=["Analytics Real-time"])
...
async def get_live_monitoring_data(
    ...
    try:
        ...
        performance = await get_performance_score(
            channel_id,
            1,
            current_user,
            analytics_client,
...
def generate_performance_recommendations(
    ...
    if overall_score > 80:
        recommendations.append(
            "🎉 Excellent performance! Maintain current strategy and consider scaling content"
        )
