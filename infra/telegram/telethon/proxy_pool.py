...
class ProxyPool:
    ...
    def __init__(
        ...
        self.current_proxy: ProxyState | None = None
    ...
    async def get_current_proxy(self) -> str | None:
        ...
        async with self._lock:
            ...
            if (
                ...
                or not self.current_proxy.is_available
            ):
                await self._rotate_proxy()
        ...
        async with self._lock:
            ...
            if any(word in error_str for word in ["timeout", "connection", "network"]):
                ...
                if proxy.fail_count >= self.fail_score_limit:
                    ...
                    proxy.status = ProxyStatus.UNHEALTHY
                    logger.warning(
                        f"Proxy {proxy_url} marked unhealthy "
    ...
    async def _rotate_proxy(self) -> None:
        ...
        logger.info(
            f"Rotated proxy from {old_proxy} to {self.current_proxy.url} "
    ...
    def _find_proxy(self, url: str) -> ProxyState | None:
    ...
    async def _health_check_loop(self) -> None:
        ...
        while self._started:
            try:
                ...
                if (
                    self.current_proxy
                    and not self.current_proxy.is_available
                    and time.time() - self.last_rotation > 30
                ):  # At least 30s between rotations
                    async with self._lock:
                        await self._rotate_proxy()
