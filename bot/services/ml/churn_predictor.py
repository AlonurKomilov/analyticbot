...
class ChurnPredictor:
    ...
    async def predict_churn_risk(
        ...
        try:
            ...
            if not user_data:
                return await self._create_fallback_assessment(
                    user_id, "insufficient_data"
                )
    ...
    async def analyze_cohort_churn(
        ...
        try:
            # Get user cohorts
            cohorts = await self._get_user_cohorts(
                channel_id, cohort_size, time_period_days
            )
            ...
            for cohort_name, user_ids in cohorts.items():
                ...
                cohort_analysis = await self._analyze_cohort_churn_patterns(
                    user_ids, cohort_name
                )
