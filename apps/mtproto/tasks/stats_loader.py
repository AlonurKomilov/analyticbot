...
from core.ports.tg_client import TGClient, BroadcastStats
...
class StatsLoaderTask:
    ...
    async def load_batch_stats(
        ...
        channels: List[str],
        include_detailed: bool = False
    ) -> Dict[str, Optional[BroadcastStats]]:
        ...
        for channel in channels:
            try:
                stats = await self.load_channel_stats(
                    channel, 
                    include_detailed=include_detailed
                )
        ...
        self.logger.info(
            f"Batch stats loading complete: "
            f"{successful_loads}/{len(channels)} successful"
        )
        
        return results
    
    async def schedule_stats_refresh(
        self,
        channels: List[str],
