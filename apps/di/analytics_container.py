"""
Analytics DI Container
======================

✅ REFACTORED (Nov 18, 2025): Moved from apps/api/di_analytics.py to apps/di/
This file provides dependency injection for Analytics V2 services and repositories.

LOCATION RATIONALE:
- Consolidated with other DI containers (database_container, bot_container, etc.)
- Follows Single Responsibility Principle (DI logic separate from API layer)
- Maintains consistency with project architecture patterns

EXPORTS:
- get_analytics_fusion_service(): Main analytics orchestrator
- get_cache(): Redis cache adapter
- get_channel_management_service(): Channel CRUD operations
- get_telegram_validation_service(): Telegram API validation
- get_database_pool(): AsyncPG connection pool
- get_*_repository(): Direct repository access (5 repositories)
- get_telethon_client(): Cached Telethon client
- cleanup_analytics(): Graceful shutdown

DEPENDENCIES:
✅ Infrastructure imports are acceptable - DI containers MUST import concrete implementations
- infra.db.repositories: 6 AsyncPG repositories
- infra.cache: Redis cache adapter
- infra.tg: Telethon Telegram client
- core.services: Analytics fusion orchestrator
"""
