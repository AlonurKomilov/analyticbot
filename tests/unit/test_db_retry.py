...
from apps.bot.database.db import create_pool
...
async def test_create_pool_retry_once(mock_create_pool):
    ...
    """
    mock_create_pool.side_effect = [OSError("Connection failed"), "fake_pool"]
    pool = await create_pool(max_retries=3, backoff_factor=0.01)
    assert pool == "fake_pool"
...
async def test_create_pool_backoff_timing(mock_create_pool, mock_sleep):
    ...
    mock_sleep.assert_any_call(backoff_factor * 2**0)
    mock_sleep.assert_any_call(backoff_factor * 2**1)
    mock_sleep.assert_any_call(backoff_factor * 2**2)
    mock_sleep.assert_any_call(backoff_factor * 2**3)
