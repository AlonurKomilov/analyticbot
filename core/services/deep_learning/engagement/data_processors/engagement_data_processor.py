...
class EngagementDataProcessor:
    ...
    def __init__(self, sequence_length: int = 30):
        ...
        logger.info(f"📊 EngagementDataProcessor initialized with sequence length: {sequence_length}")
    ...
    async def process_historical_features(
        ...
        target_engagement: Optional[List[float]] = None
        ...
        try:
            ...
            targets_tensor = torch.tensor(targets, dtype=torch.float32).unsqueeze(1) if targets else None
        ...
        try:
            ...
            for feature_name in self.feature_names:
                ...
                if 'min' in params and value < params['min']:
                    logger.warning(f"Feature {feature_name} below minimum: {value} < {params['min']}")
                    return False
                
                if 'max' in params and value > params['max']:
                    logger.warning(f"Feature {feature_name} above maximum: {value} > {params['max']}")
