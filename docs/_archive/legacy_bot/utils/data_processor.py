...
class AdvancedDataProcessor:
    ...
    async def clean_data(
        ...
        try:
            ...
            if cleaning_options.get("fix_data_types", False):
                ...
                if type_fixes:
                    cleaning_report["steps"].append({"step": "fix_data_types", "fixes": type_fixes})
            ...
            if cleaning_options.get("remove_outliers", False):
                ...
                for col in numeric_cols:
                    ...
                    if outlier_count > 0:
                        df = df[~outlier_mask]
                        outlier_info.append({"column": col, "outliers_removed": int(outlier_count)})
                    ...
                    cleaning_report["steps"].append(
                        {
                            ...
                            "changes": dict(
                                zip(original_columns, df.columns.tolist(), strict=False)
                            ),
                        }
                    )
    ...
    async def transform_data(self, data_id: str, transformations: list[dict]) -> dict[str, Any]:
        """
        🔄 Apply data transformations
        """
        try:
