...
down_revision = "0055_add_user_ai_tables"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.String(length=50),
            nullable=False,
            comment="Provider: openai, claude, gemini, grok, etc.",
        ),
        sa.Column(
            "api_key_encrypted",
            sa.Text(),
        ...
        sa.Column(
            ...
            comment="User's default provider",
        ),
        sa.Column(
            "model_preference",
            sa.String(length=100),
            nullable=True,
            comment="Preferred model: gpt-4-turbo, claude-3-opus, etc.",
        ),
        sa.Column(
            "config",
            postgresql.JSONB(astext_type=sa.Text()),
            nullable=True,
            comment="Provider-specific config",
        ),
        sa.Column(
            "monthly_budget_usd",
            sa.Numeric(precision=10, scale=2),
            nullable=True,
            comment="Optional monthly spending limit",
        ),
        sa.Column(
