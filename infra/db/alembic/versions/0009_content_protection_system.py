...
def upgrade():
    ...
    op.create_table('reporting_snapshots',
        sa.Column('id', sa.Integer(), nullable=False),
    ...
    op.add_column('channels', sa.Column('protection_level', sa.String(20), nullable=False, server_default='standard'))
    op.add_column('channels', sa.Column('auto_moderation', sa.Boolean(), nullable=False, server_default='true'))
    op.add_column('channels', sa.Column('whitelist_enabled', sa.Boolean(), nullable=False, server_default='false'))
    op.add_column('channels', sa.Column('last_content_scan', sa.TIMESTAMP(timezone=True), nullable=True))
    
    # Create indexes for performance
    op.create_index('ix_content_violations_channel_id', 'content_violations', ['channel_id'])
    op.create_index('ix_content_violations_violation_type', 'content_violations', ['violation_type'])
    ...
    op.create_index('ix_reporting_snapshots_snapshot_date', 'reporting_snapshots', ['snapshot_date'])
    ...
    op.create_index('ix_content_analysis_tags', 'content_analysis', ['tags'], postgresql_using='gin')
