...
class AnalyticsOrchestratorService:
        ...
        try:
            ...
            core_tasks = [
                self._statistical.calculate_statistical_significance(
                    channel_id, "views", analysis_period_days // 2, analysis_period_days // 2
                ),
                self._ai_insights.generate_ai_insights(
                    channel_id, "comprehensive", analysis_period_days
                ),
                self._trend.analyze_advanced_trends(
                    channel_id, "views", analysis_period_days
                ),
            ...
            results = {
                ...
                "statistical_analysis": statistical_analysis if not isinstance(statistical_analysis, Exception) else {"error": str(statistical_analysis)},
                "ai_insights": ai_insights if not isinstance(ai_insights, Exception) else {"error": str(ai_insights)},
                "trend_analysis": trend_analysis if not isinstance(trend_analysis, Exception) else {"error": str(trend_analysis)},
                "live_metrics": live_metrics if not isinstance(live_metrics, Exception) else {"error": str(live_metrics)}
            }
    ...
    async def cross_service_correlation_analysis(
        ...
        try:
            ...
            # Add AI insights
            tasks.append(self._ai_insights.generate_ai_insights(channel_id, "correlation", analysis_days))
        ...
        try:
            ...
            if stats and "p_value" in stats:
                if stats["p_value"] < 0.05:
                    insights["key_strengths"].append("Statistically significant performance improvements detected")
    ...
    async def _execute_ai_insights_step(self, channel_id: int, params: Dict):
        ...
        return await self._ai_insights.generate_ai_insights(
        ...
        for step_name, result in step_results.items():
            if "error" not in result:
                ...
                if "significance" in result:
                    summary["key_findings"].append(f"{step_name}: Statistical significance detected")
    ...
    def _calculate_cross_metric_correlations(self, results: List) -> Dict:
        ...
        try:
            ...
            if len(valid_results) >= 2:
                ...
                correlations["strong_correlations"].append("Views and engagement show positive correlation")
            else:
