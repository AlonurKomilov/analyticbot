...
def complete_module_monolith_migration():
    ...
    repo_distributions = [
        ...
        (
            "infra/db/repositories/reports_repository.py",
            "src/analytics/infrastructure/persistence/",
        ),
        ...
        (
            "infra/db/repositories/subscription_repository.py",
            "src/payments/infrastructure/persistence/",
        ),
        # Scheduling module repositories
        (
            "infra/db/repositories/schedule_repository.py",
            "src/scheduling/infrastructure/persistence/",
        ),
        (
            "infra/db/repositories/delivery_repository.py",
            "src/scheduling/infrastructure/persistence/",
        ),
        # Channels module repositories
        (
            "infra/db/repositories/channel_repository.py",
            "src/channels/infrastructure/persistence/",
        ),
        (
            "infra/db/repositories/telegram_repository.py",
            "src/channels/infrastructure/persistence/",
        ),
        # Bot service repositories (if any specific ones exist)
        (
            "infra/db/repositories/bot_repository.py",
            "src/bot_service/infrastructure/persistence/",
        ),
    ]
    ...
    shared_db_moves = [
        ...
        (
            ...
            "src/shared_kernel/infrastructure/database/connection_manager.py",
        ),
        ...
        (
            ...
            "src/shared_kernel/infrastructure/database/__init__.py",
        ),
    ]
    ...
    for src_path, dst_path in cross_cutting_moves:
        if os.path.exists(src_path):
            ...
            if os.path.exists(f"src/shared_kernel/infrastructure/{existing_shared}"):
                print(
                    f"   ⚠️  {existing_shared} already exists in shared_kernel, skipping {src_path}"
                )
                continue
    ...
    print("📊 Next: Update imports and test integration")
