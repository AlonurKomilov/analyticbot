...
from ..protocols.learning_protocols import LearningTask, UpdateStatus, LearningProgress
...
class ProgressTracker:
    ...
    async def update_progress(
        ...
        try:
            ...
            logger.debug(f"📊 Updated progress for task {task_id}: {progress_percent:.1f}% ({current_phase})")
            return True
    ...
    async def update_intermediate_results(
        self,
        task_id: str,
    ...
    async def get_tracking_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.active_contexts:
                ...
                self.tracking_stats['avg_progress_rate'] = total_progress / len(self.active_contexts)
            ...
            return {
                ...
                'total_progress_records': sum(len(history) for history in self.progress_history.values()),
                'config': {
    ...
    async def _check_stale_tasks(self) -> None:
        ...
        try:
            ...
            for task_id in stale_tasks:
                logger.warning(f"⚠️ Task {task_id} appears stale (no heartbeat for {self.config.stale_threshold_minutes} minutes)")
