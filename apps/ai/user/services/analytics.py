        ...
        try:
            ...
            if current.avg_views_per_post > previous.avg_views_per_post * 1.2:
                trends.append({
                    "type": "views",
                    "direction": "up",
                    "change": (current.avg_views_per_post - previous.avg_views_per_post) / previous.avg_views_per_post * 100,
                ...
                trends.append({
                    "type": "views",
                    "direction": "down",
                    "change": (previous.avg_views_per_post - current.avg_views_per_post) / previous.avg_views_per_post * 100,
            ...
            if current.engagement_rate > previous.engagement_rate * 1.1:
                trends.append({
                    "type": "engagement",
                    "direction": "up",
                    "change": (current.engagement_rate - previous.engagement_rate) / previous.engagement_rate * 100 if previous.engagement_rate > 0 else 0,
        ...
        try:
            report = {
                ...
                "key_insights": [
                    ...
                    for insight in sorted(insights, key=lambda x: {"high": 0, "medium": 1, "low": 2}[x.importance])[:5]
                ],
                "top_recommendations": [],
            }
