...
class ContentDataProcessor:
    ...
    def build_vocabulary(self, texts: List[str]) -> Dict[str, Any]:
        ...
        for word, count in word_counts.most_common():
            if count >= self.min_word_freq and len(vocab_words) < (self.vocab_size - len(self.special_tokens)):
        ...
        logger.info(f"✅ Vocabulary built: {vocab_stats['vocab_size']} words, {vocab_stats['coverage']:.2%} coverage")
        return vocab_stats
    ...
    def process_batch(
        ...
        input_ids = pad_sequence(input_ids, batch_first=True, padding_value=self.special_tokens['<PAD>'])
    ...
    def analyze_text_complexity(self, text: str) -> Dict[str, float]:
        ...
        """
        if not text or pd.isna(text):
            return {metric: 0.0 for metric in ['readability', 'complexity', 'diversity', 'formality']}
    ...
    def validate_inputs(self, texts: List[str]) -> Dict[str, Any]:
        ...
        if too_long_count > 0:
            ...
            report["recommendations"].append(f"Texts will be truncated to {self.max_seq_length} tokens")
