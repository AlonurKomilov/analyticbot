...
class VisualizationEngine:
    ...
    async def create_line_chart(self, df: pd.DataFrame, x_column: str, y_column: str,
        ...
        try:
            ...
            # Apply theme
            theme_config = self.theme_settings.get(theme, self.theme_settings['default'])
            fig.update_layout(
    ...
    async def create_bar_chart(self, df: pd.DataFrame, x_column: str, y_column: str,
                              title: str = None, orientation: str = 'vertical') -> Dict[str, Any]:
    ...
    async def create_scatter_plot(self, df: pd.DataFrame, x_column: str, y_column: str,
    ...
    async def create_histogram(self, df: pd.DataFrame, column: str, bins: int = 30,
                              title: str = None) -> Dict[str, Any]:
        """Create histogram"""
        try:
            if not PLOTLY_AVAILABLE:
    ...
    async def create_box_plot(self, df: pd.DataFrame, y_column: str, x_column: str = None,
                             title: str = None) -> Dict[str, Any]:
        """Create box plot"""
        try:
            if not PLOTLY_AVAILABLE:
            ...
            fig = px.box(
                ...
                y=y_column, 
                x=x_column,
                title=title or f"Box Plot of {y_column}"
            )
                ...
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            ...
                            dbc.Label("X Column:", className="mt-2"),
                            dcc.Dropdown(id='x-column'),
                            dbc.Label("Y Column:", className="mt-2"),
                            dcc.Dropdown(id='y-column'),
                            dbc.Button("Generate Chart", id="generate-btn", className="mt-3", color="primary")
                        ])
                    ])
                ], width=4),
...
if __name__ == "__main__":
    ...
    async def test_dashboard():
        ...
        scatter_plot = await viz_engine.create_scatter_plot(sample_data, 'visitors', 'sales', color_column='category')
