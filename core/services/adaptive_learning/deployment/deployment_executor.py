...
class DeploymentExecutor:
    ...
    def __init__(self, monitoring_service: MonitoringServiceProtocol):
        ...
        self.deployment_implementations: Dict[DeploymentStrategy, Callable] = self._initialize_deployment_implementations()
    ...
    async def execute_deployment(self, plan: DeploymentPlan) -> Optional[str]:
    ...
    async def get_execution_status(self, execution_id: str) -> Optional[ExecutionProgress]:
    ...
    async def cancel_deployment(self, execution_id: str, reason: str = "User requested") -> bool:
        ...
        try:
            ...
            if execution.progress.phase in [ExecutionPhase.DEPLOYING, ExecutionPhase.ROLLING_BACK]:
                logger.warning(f"⚠️ Cancellation during {execution.progress.phase.value} may be unsafe")
    ...
    async def _monitor_deployment(self, execution: DeploymentExecution) -> bool:
        ...
        try:
            ...
            for i in range(checks_needed):
                ...
                try:
                    metrics = await self.monitoring_service.get_current_metrics(execution.plan.model_id)
    ...
    async def _execute_rollback_strategy(self, execution: DeploymentExecution, strategy: str) -> bool:
        """Execute rollback strategy"""
        try:
    ...
    async def _update_progress(self, execution: DeploymentExecution, progress_percent: float) -> None:
